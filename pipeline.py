#!/usr/bin/env python3
"""Canonical NarrowGate data command dispatcher.

Use ``data`` for the current acquisition and common-input contract. Historical
imports, preprocessing and feature tools require explicit ``legacy`` selection;
they are not current defaults or automatic source fallbacks.
"""
...
LEGACY_COMMANDS: dict[str, tuple[str, str]] = {
    "unify-raw": (
        "data.daily_raw",
        "Convert selected daily market sources to one supplier-neutral Parquet layout",
    ),
    "download-agg-trades": (
        "data.downloaders.binance_vision",
        "Download native reference aggTrades (BTCUSDC perpetual is retired)",
    ),
    "download-raw-trades": (
        "data.downloaders.binance_vision",
        "Download Binance USD-M daily raw trades",
    ),
    "download-secondary-trades": (
        "data.downloaders.cryptohft_trades",
        "Acquire ID-preserving CryptoHFT individual trades for source union",
    ),
    "aggregate-trades": (
        "data.trade_aggregation",
        "Prepare verified individual-trade union and synthetic 100ms flow buckets",
    ),
    "download-metrics": (
        "data.downloaders.binance_vision",
    ...
    "download-orderbook": (
        "data.downloaders.cryptohft_orderbook",
        "Download and normalize CryptoHFT orderbook BBO/L2",
    ),
    "download-tardis": (
        "data.downloaders.tardis_archive",
    ...
    "download-bybit": (
        "data.downloaders.bybit_reference",
    ...
    "bars": ("features.preprocess", "Build 1s bars, preferring canonical individual trades"),
    "preprocess-metrics": (
...
COMMANDS = {"data": ("data.__main__", "Current market-data acquisition, inventory and fact normalization")}
...
def _print_help() -> None:
    ...
    print(__doc__.strip())
    print("\ncommands:")
    print("  legacy <command>      Explicit historical tooling; not current data admission")
...
def run_features_all(argv: list[str]) -> None:
    ...
    _run_or_exit("bars", LEGACY_COMMANDS["bars"][0], bars_args)
    _run_or_exit(
        "preprocess-metrics",
        LEGACY_COMMANDS["preprocess-metrics"][0],
        metrics_args,
        allow_failure=not args.strict_metrics,
    )
    _run_or_exit("engineer", LEGACY_COMMANDS["engineer"][0], engineer_args)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not args or args[0] in {"-h", "--help", "help"}:
        _print_help()
        return
    legacy = args[0] == "legacy"
    if legacy:
        args = args[1:]
        if not args:
            raise SystemExit("Specify an explicit historical command")
        if args[0] in {"-h", "--help", "help"}:
            print("Historical commands only; not current-source admission:")
            for name, (_, description) in LEGACY_COMMANDS.items():
                print(f"  {name}: {description}")
            print("  features-all: Historical bars/metrics/features pipeline")
            return
    command = ALIASES.get(args[0], args[0]) if legacy else args[0]
    commands = LEGACY_COMMANDS if legacy else COMMANDS
    if legacy and command == "features-all":
