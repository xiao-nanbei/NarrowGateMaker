...
def test_10s_cache_requires_explicit_features_and_rebuilds(tmp_path, monkeypatch):
    from research.families.f04_external_market_alpha import external_venue_model as model

    day = "2026-01-01"
    cache = model._cache_path(tmp_path, "10s", day)
    cache.parent.mkdir(parents=True)
    pd.DataFrame({"value": [-1]}).to_parquet(cache)
    with pytest.raises(ValueError, match="explicit"):
        model._build_cache_day(str(tmp_path), "10s", day, False)
    with pytest.raises(ValueError, match="explicit"):
        model.build_cache(tmp_path, "10s", [day], workers=1, force=False)
    source = tmp_path / "selected"
    source.mkdir()
    pd.DataFrame({"value": [7]}).to_parquet(source / f"features_{day}.parquet")
    monkeypatch.setattr(model, "enrich_10s_feature_day", lambda frame, root, date: frame)
    result = model._build_cache_day(str(tmp_path), "10s", day, False, str(source))
    assert result["status"] == "built"
    assert pd.read_parquet(cache)["value"].tolist() == [7]
