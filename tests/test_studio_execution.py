...
def test_unavailable_lan_stays_queued_without_automatic_mac_fallback(store):
    ...
    assert store.job(job["id"])["queue_reason"] == (
        "waiting_for_selected_resource_worker_inputs_memory_or_capacity"
    )
...
@pytest.mark.parametrize("ready_resource", ["mac", "lan"])
def test_auto_without_preferences_uses_each_registered_ready_resource(
    tmp_path, manifest, ready_resource,
):
    rewrite(manifest, lambda data: data["plans"][0].update(preferred_resources=[]))
    store = studio.Store(tmp_path / "auto-control", manifest)
    registered(store, ready_resource)
    job = submitted(store)
    claimed = store.claim(f"{ready_resource}-worker", "session")
    assert claimed["id"] == job["id"]
    assert claimed["resource_id"] == ready_resource


def test_auto_preferences_keep_declared_order_and_explicit_target_wins(tmp_path, manifest):
    rewrite(manifest, lambda data: data["plans"][0].update(preferred_resources=["lan", "mac"]))
    store = studio.Store(tmp_path / "preferred-control", manifest)
    registered(store, "mac")
    registered(store, "lan")
    job = submitted(store)
    assert store.claim("mac-worker", "session") is None
    assert store.claim("lan-worker", "session")["id"] == job["id"]
    contract = store.execution.contract(store.execution.plans["audit"])
    assert store.execution.candidates(contract, "mac") == ["mac"]


def test_auto_does_not_add_local_without_registered_target(manifest):
    def change(data):
        data["plans"][0]["preferred_resources"] = []
        data["plans"][0]["targets"].pop("mac")
    rewrite(manifest, change)
    catalog = execution.Catalog(manifest)
    contract = catalog.contract(catalog.plans["audit"])
    assert catalog.candidates(contract, "auto") == ["lan"]


@pytest.mark.parametrize("preferences,expected", [([], ["cloud", "mac"]), (["mac", "cloud"], ["mac", "cloud"])])
def test_training_defaults_to_role_preference_but_honors_explicit_order(
    manifest, preferences, expected,
):
    def change(data):
        data["resources"][0]["roles"]["training"] = "allowed"
        data["resources"][1].update(id="cloud", kind="azure")
        data["resources"][1]["roles"]["training"] = "preferred"
        plan = data["plans"][0]
        plan.update(role="training", preferred_resources=preferences)
        plan["targets"]["cloud"] = plan["targets"].pop("lan")
    rewrite(manifest, change)
    catalog = execution.Catalog(manifest)
    contract = catalog.contract(catalog.plans["audit"])
    assert catalog.candidates(contract, "auto") == expected
