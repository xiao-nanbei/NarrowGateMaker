...
def test_public_registry_contains_only_family_routing() -> None:
    ...
    required = {"id", "directory", "package"}
    allowed = required | {"public_input_module"}
    for family in payload["families"]:
        assert required <= set(family) <= allowed
        if "public_input_module" in family:
            assert family["public_input_module"] == f"{family['package']}.public_input"
