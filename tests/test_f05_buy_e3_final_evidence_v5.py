...
from scripts import f05_buy_e3_final_evidence_v5 as subject
...
def _freeze_transport(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    values: dict[str, Any] = {
        ...
        "FROZEN_FINAL_CONFIG_CORRECTION_FILE_SHA256": "e" * 64,
        "FROZEN_FINAL_CONFIG_CORRECTION_CANONICAL_SHA256": "f" * 64,
        ...
        "FROZEN_CONFIG_CORRECTION_PATH_PROVENANCE": str(tmp_path / "correction.json"),
...
def test_content_projection_accepts_exact_artifact_field_only() -> None:
    ...
    with pytest.raises(subject.FinalEvidenceV5Error, match="malformed"):
...
def _portable() -> dict[str, Any]:
    ...
    receipts = {
        "config_correction": {
            **_content(
                subject.transport.resource_v7.config_successor.SCHEMA_VERSION,
                subject.transport.resource_v7.config_successor.STATUS,
                "e",
                "f",
                canonical_field=(
                    subject.transport.resource_v7.config_successor.CANONICAL_FIELD
                ),
            ),
...
def _superseded_v4_history() -> dict[str, Any]:
    return {
...
def test_all_five_receipt_schemas_are_new_v5_identities() -> None:
    schemas = {
    ...
    assert all(schema.endswith(".v5") for schema in schemas)
    statuses = {
        subject.ENVELOPE_STATUS,
        subject.COMPLETION_STATUS,
        subject.COMPOSITION_STATUS,
        subject.ATTEMPT_FINAL_STATUS,
        subject.EVIDENCE_RELEASE_STATUS,
    }
    assert len(statuses) == 5
    assert all("external_venues_disabled" in status for status in statuses)
    assert "config_correction" in subject.PORTABLE_SOURCE_ROLES
...
def test_frozen_lifecycle_and_rejected_epoch_exact7_identities() -> None:
    ...
    assert set(subject.SUPERSEDED_V4_PROOF_CONTENT) == set(subject.CONTENT_BINDING_FIELDS)
    assert (
        subject.SUPERSEDED_V4_PROOF_CONTENT["file_sha256"]
        == "0f85849289cb9e42de7333117c7719e1a95a1561cf42e8a00366e0e8500df28f"
    )
    assert (
        subject.SUPERSEDED_V4_PROOF_CONTENT["canonical_sha256"]
...
def test_portable_v4_accepts_release_v2_and_rejects_v3_execution() -> None:
    release, binding, execution, artifact = _release_bundle()
    portable = _portable()
    observed = subject._validate_portable_v5(  # noqa: SLF001
        portable,
        release=release,
    ...
    with pytest.raises(subject.FinalEvidenceV5Error, match="not direct-v4"):
        subject._validate_portable_v5(  # noqa: SLF001
            portable,
            release=release,
            release_binding=binding,
            execution=execution,
...
def test_portable_v5_rejects_missing_or_tampered_config_correction() -> None:
    release, binding, execution, artifact = _release_bundle()
    portable = _portable()
    portable["source_receipts"]["config_correction"]["canonical_sha256"] = "0" * 64
    with pytest.raises(subject.FinalEvidenceV5Error, match="config_correction exact7"):
        subject._validate_portable_v5(  # noqa: SLF001
            portable,
            release=release,
            release_binding=binding,
    ...
    portable = _portable()
    del portable["source_receipts"]["config_correction"]
    with pytest.raises(subject.FinalEvidenceV5Error, match="source receipt roles"):
        subject._validate_portable_v5(  # noqa: SLF001
            portable,
            release=release,
            release_binding=binding,
...
def test_superseded_v4_proof_cannot_be_current_authority() -> None:
    historical = _superseded_v4_history()
    assert subject._validate_superseded_v4_historical(historical) == historical  # noqa: SLF001
    historical["current_runtime_authority"] = True
    with pytest.raises(subject.FinalEvidenceV5Error, match="historical identity"):
...
def test_activation_keeps_v3_attempt_as_history_while_v4_is_final(
    ...
    monkeypatch.setattr(
        subject,
        "_superseded_v4_proof_context",
        lambda *_args, **_kwargs: (
            {"status": subject.historical_final_v4.EVIDENCE_RELEASE_STATUS},
            dict(subject.SUPERSEDED_V4_PROOF_CONTENT),
            _superseded_v4_history(),
        ),
    )

    observed = subject.build_activation_envelope(
        historical_operational_attempt_v10_path=Path("attempt.json"),
        superseded_v4_proof_path=Path("proof-v4.json"),
    ...
    assert observed["superseded_v4_proof"] == _superseded_v4_history()
    assert observed["config_correction"]["file_sha256"] == "e" * 64
    assert observed["checks"]["superseded_v4_proof_is_current_authority"] is False
...
def test_lifecycle_supplement_semantic_tamper_rejected(mutate: Any, message: str) -> None:
    ...
    mutate(payload)
    with pytest.raises(subject.FinalEvidenceV5Error, match=message):
...
def test_rejected_predecessor_semantic_tamper_rejected(field_value: tuple[str, Any]) -> None:
    ...
    with pytest.raises(subject.FinalEvidenceV5Error, match="predecessor lifecycle semantics"):
...
def test_lifecycle_admission_requires_new_epoch_and_exact_active_files(
    ...
    with pytest.raises(subject.FinalEvidenceV5Error, match="epoch/config/runtime files"):
...
def _write_attempt_final_fixture(
    ...
    correction = subject._validate_config_correction_content(  # noqa: SLF001
        subject._content_projection(  # noqa: SLF001
            _portable()["source_receipts"]["config_correction"],
            "fixture config correction",
        )
    )
    superseded = _superseded_v4_history()
    composition = {
        ...
        "exact_artifact": artifact,
        "evidence": {
            "external_venues_disabled_config_correction": correction,
            "superseded_v4_proof": superseded,
        },
...
def test_proof_release_revalidates_v4_and_never_calls_base_direct_helpers(
    ...
    assert payload["config_correction"]["file_sha256"] == "e" * 64
    assert payload["superseded_v4_proof"] == _superseded_v4_history()
    assert payload["authority_provenance"]["superseded_v4_proof_used_as_authority"] is False
    assert payload["evidence_state"]["external_venues_disabled_active_config_exact"] is True
    ...
    with pytest.raises(subject.FinalEvidenceV5Error, match="receipt creation failed"):
...
@pytest.mark.parametrize(
    ...
    (
        ...
        lambda attempt: attempt["config_correction"].__setitem__(
            "canonical_sha256", "0" * 64
        ),
        lambda attempt: attempt["superseded_v4_proof"].__setitem__(
            "current_runtime_authority", True
        ),
    ),
)
def test_proof_release_rejects_reused_or_tampered_source_identity(
    ...
    with pytest.raises(subject.FinalEvidenceV5Error):
