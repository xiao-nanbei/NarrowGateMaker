...
def test_f03_final_requires_all_101_days_and_preserves_end_inventory_mtm():
    rows = _utc_results(independent=False)["inf"].rows[:101]
    ...
    assert report["days"] == 101
    assert report["net_pnl_usdc"] == 101
...
def test_f03_independent_shards_preserve_full_calendar_and_original_labels():
    ...
    assert len(final) == 51 and [len(spec.calendar_days) for spec in final] == [2] * 50 + [1]
    ...
    assert final[-1].calendar_days == ("2026-09-05",)
...
@pytest.mark.parametrize("phase,days,shards", [("development", 300, 150), ("final", 101, 51)])
...
def test_f03_final_pairing_accepts_only_same_full_51_shards_without_selection():
    results = _utc_results(phase="final")
    pair = {name: results[name] for name in ("inf", "120")}
    assert len(tw_eval.validate_paired_shards(pair, phase="final")) == 51
    with pytest.raises(ValueError, match="calendar"):
