...
from research.families.f06_placement_fill_cif.audit import (
    evaluate_role_calibrated_competing_curve_cif as v5,
    evaluate_nested_role_competing_curve_cif as v6,
)
...
@pytest.mark.parametrize(
    "evaluator,spec_name,family_id,oof_field",
    [
        pytest.param(v5, "V5_SPEC", "placement_fill_role_calibrated_competing_cif_v5",
                     "oof_role_calibrated_predictions", id="v5-role-calibrated"),
        pytest.param(v6, "V6_SPEC", "placement_fill_nested_role_competing_cif_v6",
                     "oof_nested_role_predictions", id="v6-nested-role"),
    ],
)
def test_evaluator_reuses_v4_gate_and_fails_closed(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, evaluator, spec_name, family_id, oof_field
) -> None:
    spec = load_placement_fill_spec(getattr(evaluator, spec_name))
    base = (
    ...
    migrated_spec = tmp_path / f"{spec_name.lower()}_migrated_spec.json"
    migrated_spec.write_text(json.dumps(spec), encoding="utf-8")
    monkeypatch.setattr(evaluator, spec_name, migrated_spec)
    rows = []
