...
def test_retired_july_inputs_have_no_implicit_research_default(tmp_path, monkeypatch):
    args = ["--config", "config.yaml", "--latency-telemetry", "latency.csv"]
    with pytest.raises(SystemExit):
        placement_fill_panel.parse_args(args)
    assert placement_fill_panel.parse_args(args + ["--feature-context-dir", str(tmp_path)]).feature_context_dir == tmp_path
    monkeypatch.setattr(toxicity_gate, "TRADE_MANIFEST", None)
    with pytest.raises(ValueError, match="retired"):
        toxicity_gate.build_params("2026-07-01", tmp_path / "missing-config")
    with pytest.raises(FileNotFoundError):
        toxicity_gate.bind_trade_manifest(tmp_path / "missing")
    manifest = tmp_path / "manifest.json"
    manifest.write_text("{}")
    toxicity_gate.bind_trade_manifest(manifest)
    assert toxicity_gate.require_trade_manifest() == manifest.resolve()


def test_explicit_trade_manifest_survives_spawn(tmp_path):
    ...
    with concurrent.futures.ProcessPoolExecutor(
        ...
        initializer=toxicity_gate.bind_trade_manifest, initargs=(manifest,),
    ) as pool:
        assert pool.submit(toxicity_gate.require_trade_manifest).result(timeout=30) == manifest.resolve()
