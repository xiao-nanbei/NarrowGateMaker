...
def test_native_cache_identity_excludes_receive_clock_fallback(tmp_path: Path) -> None:
    assert _identity(_source(tmp_path))["exchange_clock"] == (
        "transaction_with_event_fallback_no_receive"
    )
...
def test_cryptohft_tape_reuses_hour_cache_across_iterations(
    ...
    unrelated_daily = tmp_path / "unrelated-daily.parquet"
    unrelated_daily.write_bytes(b"unrelated global canonical must not be read")
    monkeypatch.delenv("NARROWGATE_DAILY_ORDERBOOK_ROOT", raising=False)
    monkeypatch.setattr(
        "data.downloaders.cryptohft_orderbook.daily_market_path", lambda *args, **kwargs: unrelated_daily,
    )
    calls: list[Path] = []
    ...
    assert first == second
    assert len(first) == 24
    assert [event.event_type for event in first].count("snapshot") == 1
    assert [event.event_type for event in first].count("source_gap") == 23
    assert len(tape.missing_paths) == 23
    assert tape.source_paths == (source,)
    assert calls == [source]
...
def test_cryptohft_tape_binds_explicit_d_plus_one_continuation(
    ...
    assert [event.event_type for event in events].count("snapshot") == 2
    assert [event.event_type for event in events].count("source_gap") == 46
    assert len(tape.missing_paths) == 46
