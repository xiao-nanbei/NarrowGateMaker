...
def test_queue_calibration_has_no_ambient_fallback(tmp_path, monkeypatch):
    from models.queue_calibration import load_daily_queue_calibration

    artifact = tmp_path / "ambient.json"
    artifact.write_text('{"days": {"2026-06-26": {"queue_base": 999}}}')
    monkeypatch.setenv("MM_QUEUE_CALIBRATION_PATH", str(artifact))
    assert load_daily_queue_calibration(symbol="BTCUSDC", root=tmp_path) == {}
    params = {}
    add_queue_calibration_params(params, symbol="BTCUSDC")
    assert params["queue_calibration_loaded"] is False
    assert params["queue_calibration_path"] == ""
    assert "_queue_calibration" not in params
    with pytest.raises(ValueError, match="explicit artifact path"):
        add_queue_calibration_params({}, symbol="BTCUSDC", strict=True)
    with pytest.raises(FileNotFoundError):
        load_daily_queue_calibration(path=tmp_path / "missing.json")
    assert load_daily_queue_calibration(path=artifact)["days"]
    assert not (tmp_path / "queue_calibration").exists()
