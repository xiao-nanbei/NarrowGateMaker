...
def test_manifest_has_fixed_four_files_and_shared_funding_not_retired_early(tmp_path):
    ...
    assert {p.name for p in expected.iterdir()} == {f"{channel}.parquet" for channel in CHANNEL_SOURCES}
...
def test_vision_defaults_convert_retire_and_repeat_without_network(tmp_path, monkeypatch, channel, symbol):
    from data import download_binance_vision as vision
