...
from narrowgate.runtime.baseline_epoch_manifest import (
...
def test_design_contract_binds_implementation_and_keeps_authority_closed() -> None:
    ...
    # The frozen design binds the pre-migration import spelling. Preserve that
    # evidence rather than rehashing it as if a historical run used new code.
    # Only this declared path-only change is normalized; any other byte change
    # still fails the historical identity check. Behavioral tests above test
    # the actual current implementation.
    historical_source = PREFLIGHT_PATH.read_text().replace(
        "from narrowgate.runtime.baseline_epoch_manifest import (",
        "from models.replay.baseline_epoch_manifest import (",
    )
    assert implementation["sha256"] == hashlib.sha256(historical_source.encode()).hexdigest()
