...
def test_exact_native_probe_bridge_preserves_original_bundle(tmp_path: Path) -> None:
    # Controlled authority bytes exercise the bridge, not a retired market dataset.
    manifest = tmp_path / "source-manifest.json"
    quality = tmp_path / "source-quality.csv"
    manifest.write_text('{"fixture": "controlled"}\n', encoding="utf-8")
    quality.write_text("fixture,valid\ncontrolled,true\n", encoding="utf-8")
    built = prep.source_specs.BuiltSourceSpec(
        bundle=SimpleNamespace(identity_sha256=lambda: "controlled-bundle"),
        market_data_root=tmp_path,
        profile_id=prep.EXPECTED_PROFILE,
        probe={
            "physical_materialization_eligible": True,
            "execution_l2_quality_authority": {
                "authority_mode": prep.source_specs.REGISTRY_MANIFEST_CSV_QUALITY_AUTHORITY,
                "valid": True,
                "errors": [],
                "manifest_path": str(manifest),
                "manifest_sha256": _sha256(manifest),
                "daily_quality_path": str(quality),
                "daily_quality_sha256": _sha256(quality),
            },
        },
    )
