...
def test_utc_accounting_slices_keep_cross_day_inventory_fees_and_funding():
    ...
    funding_trace = []
    ledger = campaign_audit._fills_to_trade_rows(
    ...
    rows = campaign_audit._utc_accounting_rows(
...
def test_utc_accounting_unmodeled_funding_is_unknown_and_reconciliation_is_required():
    ...
    with pytest.raises(ValueError, match="reconcile"):
        campaign_audit._utc_accounting_rows(
            [], marks, initial_inventory=0., initial_entry_price=0., funding_trace=[],
...
def test_f01_continuous_utc_accounting_output_uses_real_tick_state(monkeypatch, tmp_path):
    ...
    midnight = int(campaign_audit._day_start_ts(days[1]) * 1000)
    ...
    def load(day, params):
        lower = int(campaign_audit._day_start_ts(day) * 1000)
    ...
    monkeypatch.setattr(campaign_audit, "build_configured_cooldown_policy_adapter", lambda **_kw: None)
    ...
    result = campaign_audit._run_day_campaign_audit(
        ...
        arms=[campaign_audit.smoke.SmokeArm("B0", "test", {}, "")], engine="python",
    ...
    campaign_audit._write_partial_day_outputs(out_dir, "sample", result)
...
def test_modeled_trade_batch_cli_needs_explicit_diagnostic_source_and_timing(extra):
    with pytest.raises(SystemExit, match="causal_100ms_batches requires"):
        campaign_audit_main(["--days", "2025-08-01", "--trade-visibility-model", "causal_100ms_batches", *extra])
...
def test_f01_modeled_batches_do_not_read_retired_parents_and_bind_checkpoint_mode(monkeypatch):
    ...
    monkeypatch.setattr(campaign_audit.smoke, "_load_window", lambda *_: window)
    monkeypatch.setattr(campaign_audit, "build_configured_cooldown_policy_adapter", lambda **_: None)
    ...
    monkeypatch.setattr(campaign_audit.data_windows, "_execution_source_content_identity",
    ...
    kwargs = dict(day="1970-01-01", continuous_days=["1970-01-01"], symbol="BTCUSDC", base=params,
                  arms=[campaign_audit.smoke.SmokeArm("uniform", "test", {}, "")], engine="python",
                  day_initial={}, day_live_state=None, use_initial_state=False,
    ...
    result = campaign_audit._run_day_campaign_audit(**kwargs)
    assert len(result["utc_accounting_rows"]) == 1
    assert result["utc_accounting_rows"][0]["start_ts_ms"] == 1_000_000
    partial = campaign_audit._run_day_campaign_audit(**kwargs, checkpoint_at_ts_ms=1_000_050,
    ...
    resumed = campaign_audit._run_day_campaign_audit(
    ...
    with pytest.raises(ValueError, match="prior runtime checkpoint"):
        campaign_audit._run_day_campaign_audit(**kwargs, resume_checkpoint=checkpoint)
...
def test_campaign_model_arms_use_their_own_inference_and_horizon(
    ...
    arms = [campaign_audit.smoke.SmokeArm(str(index), "synthetic", delta, "")
...
def test_campaign_model_alias_conflicts_are_not_silently_ignored(overrides):
    arm = campaign_audit.smoke.SmokeArm("candidate", "synthetic", overrides, "")
    with pytest.raises(ValueError, match="model"):
        campaign_audit._params_for_arm({"model_dir": "baseline"}, arm)
...
def test_funding_history_reads_canonical_daily_directory(tmp_path):
    ...
    assert [row["fundingTime"] for row in campaign_audit._load_funding_history(tmp_path, symbol="BTCUSDC")] == [100, 200]
    assert len(campaign_audit._load_funding_history(path.parent, symbol="BTCUSDC")) == 2
...
def test_campaign_runtime_checkpoint_defers_finalizer_and_resumes_funding(monkeypatch, tmp_path, rotate_inputs, consume, source_plan, start_offset):
    from dataclasses import replace
