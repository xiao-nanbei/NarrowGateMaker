...
@pytest.mark.parametrize("argument_padding", ["", "x" * 512])
def test_actual_local_metadata_probe_reads_only_explicit_status_json(
    resource_manifest, tmp_path, status, terminal, still_running, argument_padding, monkeypatch
):
    from narrowgate import studio_resources as resources
    ...
    process = (
        subprocess.Popen(
            [sys.executable, "-c", "import time; time.sleep(30)", argument_padding, marker]
        )
        if still_running
        else None
    )
