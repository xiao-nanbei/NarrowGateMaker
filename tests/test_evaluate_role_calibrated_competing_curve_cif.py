...
from research.families.f06_placement_fill_cif.audit import (
    evaluate_role_calibrated_competing_curve_cif as evaluator,
)
...
def test_v5_evaluator_reuses_v4_gate_and_fails_closed(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    spec = load_placement_fill_spec(evaluator.V5_SPEC)
    base = (
    ...
    migrated_spec = tmp_path / "v5_migrated_spec.json"
    migrated_spec.write_text(json.dumps(spec), encoding="utf-8")
    monkeypatch.setattr(evaluator, "V5_SPEC", migrated_spec)
    rows = []
    ...
    report_path.write_text(
        json.dumps(
            {
                "family_id": "placement_fill_role_calibrated_competing_cif_v5",
                "outputs": {
