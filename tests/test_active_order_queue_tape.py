...
@pytest.mark.parametrize("include_levels", [True, False])
@pytest.mark.parametrize("transaction,event,expected", [
    (1_767_225_600_001, 1_767_225_600_002, 1_767_225_600_001),
    (0, 1_767_225_600_002, 1_767_225_600_002),
    (None, None, None),
    (0, 0, None),
])
def test_native_parser_never_substitutes_receive_clock(
    tmp_path, include_levels, transaction, event, expected,
):
    received = 1_767_225_600_003_123_456
    path = tmp_path / "hour.parquet"
    pd.DataFrame([dict(
        event_time=event, transaction_time=transaction, received_time=received,
        event_type="snapshot", first_update_id=None, final_update_id=None,
        prev_final_update_id=None, last_update_id=123, side="bid",
        price="100", quantity="2",
    )]).to_parquet(path)
    messages = iter_cryptohft_logical_messages(path, .1, include_levels=include_levels)
    if expected is None:
        with pytest.raises(ValueError, match="receiver clock cannot substitute"):
            list(messages)
    else:
        result = list(messages)
        assert len(result) == 1
        assert result[0].exchange_ts_ms == expected
        assert result[0].receive_time_ns == received
