...
def _write_binding(root: Path) -> tuple[Path, Path]:
    ...
    p3.write_text("{}\n", encoding="utf-8")
    for artifact in (bundle_meta, training_summary, p3):
    ...
    # Authority permissions are part of the fixture contract, not the host umask.
...
def _write_split_v2_binding(root: Path) -> tuple[Path, Path, Path]:
    ...
    for authority in (v12, private_boundary, pointer):
        authority.chmod(0o644)
    return pointer, archive, alias


@pytest.mark.parametrize("mask", [0o002, 0o022, 0o077])
@pytest.mark.parametrize("split_v2", [False, True])
def test_binding_fixture_permissions_are_independent_of_umask(
    tmp_path: Path, mask: int, split_v2: bool,
) -> None:
    previous = os.umask(mask)
    try:
        if split_v2:
            pointer, config, _alias = _write_split_v2_binding(tmp_path)
        else:
    ...
    assert binding["config_path"] == config.resolve()
