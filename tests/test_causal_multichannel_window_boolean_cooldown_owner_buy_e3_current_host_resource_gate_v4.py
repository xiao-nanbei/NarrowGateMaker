...
from research.families.f05_fill_quality_quote_ev.audit import (
    causal_multichannel_window_boolean_cooldown_owner_buy_e3_current_host_resource_gate_v4 as subject,
)
...
def test_schema_surface_and_cycle_break_are_explicit() -> None:
    assert subject.RESOURCE_SCHEMA.endswith("current_host_concurrent_resource_gate.v4")
    assert subject.RESOURCE_STATUS == "fresh_v4_disabled_same_pid_concurrent_gate_passed"
...
def test_disabled_config_requires_no_shadow_and_exact_artifact(
    ...
    path.write_text(yaml.safe_dump({"strategy": strategy, "logging": logging}), encoding="utf-8")
