...
from scripts import f05_buy_e3_active_capture_v6 as subject
...
def _release() -> dict[str, Any]:
    deployed = subject.resource_v6.EXACT_DEPLOYED_FILE_SHA256
    return {
        "exact_artifact": {
...
def _resource_binding() -> dict[str, Any]:
    return _content_binding(
        subject.resource_v6.RESOURCE_SCHEMA,
        subject.resource_v6.RESOURCE_STATUS,
        subject.resource_v6.RESOURCE_CANONICAL_FIELD,
    )


def _config_correction_binding() -> dict[str, Any]:
    return _content_binding(
        subject.resource_v6.config_successor.SCHEMA_VERSION,
        subject.resource_v6.config_successor.STATUS,
        subject.resource_v6.config_successor.CANONICAL_FIELD,
    )
...
def _resource() -> dict[str, Any]:
    files = {
        ...
        for role, frozen in subject.resource_v6.CURRENT_V4_RUNTIME_SOURCE_SHA256.items()
    }
    return {
        "config_correction": _config_correction_binding(),
        "host": {
            "instance_id": subject.resource_v6.CURRENT_INSTANCE_ID,
        ...
        "fresh_disabled_process": {
            ...
            "disabled_config_sha256": subject.resource_v6.EXPECTED_DISABLED_CONFIG_SHA256,
            "fresh_pid": True,
...
def _active_sources() -> dict[str, Any]:
    ...
    return {
        ...
        "files": files,
        "runtime_source_manifest_sha256": subject.resource_v6.canonical_sha256(files),
...
def _process(*, pid: int = 202, start_ticks: int = 2_000) -> dict[str, Any]:
    ...
    raw["canonical_process_identity_sha256"] = subject.resource_v6.document_sha256(
...
def _semantics() -> dict[str, Any]:
    startup = _startup()
    return {
        "startup_attestation_sha256": subject.resource_v6.canonical_sha256(startup),
...
def _patch_build_dependencies(
    ...
    *,
    ...
    monkeypatch.setattr(
        subject.resource_v6, "file_sha256", lambda _path: subject.ACTIVE_CONFIG_SHA256
    )
...
def test_build_binds_content_only_v4_authority_and_all_runtime_sources(
    ...
    assert payload["schema_version"].endswith(".v4")
    ...
    assert (
        ...
        == (subject.resource_v6.CURRENT_V4_RUNTIME_SOURCE_SHA256["buy_e3_runtime"]["sha256"])
    )
...
def test_active_source_capture_rejects_lifecycle_source_tamper(
    ...
    for frozen in subject.resource_v6.CURRENT_V4_RUNTIME_SOURCE_SHA256.values():
...
def test_validate_rejects_tamper_even_with_recomputed_document_hash(
    ...
    payload["active_process"]["canonical_process_identity_sha256"] = (
        subject.resource_v6.canonical_sha256(process_body)
    )
    payload[subject.CANONICAL_FIELD] = subject.resource_v6.document_sha256(
...
def test_create_only_writer_refuses_overwrite(tmp_path: Path) -> None:
    ...
    with pytest.raises(subject.resource_v6.BuyE3CurrentHostResourceGateError):
        subject.resource_v6.atomic_write_receipt(target, {"status": "must_not_replace"})
