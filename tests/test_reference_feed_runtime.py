...
@pytest.mark.parametrize("falsey_roles", [False, True])
def test_ws_handler_legacy_start_preserves_all_stream_compatibility(monkeypatch, falsey_roles):
    handler = WSHandler(SimpleNamespace(), Config())
    calls = []
    rest = object()
    class Client:
        def __bool__(self):
            return not falsey_roles

    market_snapshot_client = Client()
