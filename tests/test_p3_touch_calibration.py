...
from research.families.f02_empirical_p3_touch.audit.p3_touch_calibration import (
    ...
    survival_curve,
    window_reaches,
    public_window_reaches,
)


def test_public_p3_uses_delivered_new_book_and_strict_outcome_boundary(tmp_path):
    ...
    book.write_text("exchange,symbol,timestamp,local_timestamp,is_snapshot,side,price,amount\n"
                    "binance-futures,BTCUSDC,1,9000000,true,bid,100,1\n"
                    "binance-futures,BTCUSDC,1,9000000,true,ask,102,1\n"
                    "binance-futures,BTCUSDC,10000000,19000000,false,bid,101,1\n")
    trade = tmp_path / "trade.csv"
    trade.write_text("exchange,symbol,timestamp,local_timestamp,id,side,price,amount\n"
                     "binance-futures,BTCUSDC,11000000,22000000,1,sell,98,1\n"
                     "binance-futures,BTCUSDC,12000000,23000000,2,buy,105,1\n")
    ...
    profile = ObservationProfile("synthetic", "source_timestamp_proxy", 1_000_000_000,
                                 0, 0, 100_000_000_000, trade_coverage="observed")
    def stream(selected=profile):
        return PublicInputStream([tmp_path / "facts"], profile=selected, start_ns=0,
            end_ns=40_000_000_000, market_id="binance_futures:perpetual:BTCUSDC",
            input_contract_id=CONTRACT)
    result = public_window_reaches(stream(), start_ns=0, end_ns=40_000_000_000)
    assert result["decision_ns"].tolist() == [10_000_000_000, 20_000_000_000]
    # The bid change at10s is not delivered until11s: use100, not101.
    assert result["BUY"].tolist() == [2, float("-inf")]
    assert result["SELL"].tolist() == [3, float("-inf")]
    assert max(result["actual_outcome_end_ns"]) < 40_000_000_000
    assert result["unsupported_or_boundary_windows"] == 2
    with pytest.raises(ValueError, match="unknown trade coverage"):
        public_window_reaches(stream(replace(profile, trade_coverage="unknown")),
                              start_ns=0, end_ns=40_000_000_000)
