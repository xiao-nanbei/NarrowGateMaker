...
def test_startup_source_plan_binds_every_no_shadow_evaluator_module() -> None:
    ...
    assert set(plan["files"]) == set(subject.STARTUP_SOURCE_ROLE_MAP)
    assert {
        "signal_engine",
        "global_flow",
        "global_reference",
    ...
    for startup_role, resource_role in subject.STARTUP_SOURCE_ROLE_MAP.items():
        assert plan["files"][startup_role]["repository_relative_path"] == (
...
def _runtime() -> dict[str, Any]:
    ...
    return {
        ...
        "f05_buy_e3_active_release_canonical_sha256": (subject.DIRECT_SUCCESSOR_RELEASE_CANONICAL_SHA256),
    }
...
def test_build_binds_content_only_v4_authority_and_all_runtime_sources(
    ...
    assert payload["schema_version"].endswith(".v6")
    ...
    assert payload["startup_semantics"]["shadow_runtime"][
