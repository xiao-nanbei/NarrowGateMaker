...
def test_v1_2_distinguishes_frozen_spec_from_retired_amendment() -> None:
    assert study._spec_sha256() != study._sha256_file(study.SPEC)
    amendment = json.loads(study.EXECUTION_AMENDMENT.read_text(encoding="utf-8"))
    assert amendment["execution_availability"] == "retired_not_runnable"
    assert study._execution_amendment_sha256() == study._sha256_file(study.EXECUTION_AMENDMENT)
    assert study._execution_amendment_sha256() != amendment["projection_retirement"][
        "historical_private_source_sha256"
    ]
