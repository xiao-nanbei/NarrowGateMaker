...
def test_recompute_plan_covers_families_with_scoped_result_status() -> None:
    registry = _payload(REGISTRY)
    plan = _payload(ROOT / str(registry["current_recompute_plan"]))
    assert registry["historical_status_blocks_new_work"] is False
    assert plan["previous_use_reset"] is False
    assert plan["automatic_execution"] is False
    packages = plan["packages"]
    assert {p["id"] for p in packages} == set(range(1, 13))
    assert {f for p in packages for f in p["families"]} == {f["id"] for f in registry["families"]}
    for package in packages:
        result = package["public_results"]
        if result is not None:
            path = Path(result)
            assert not path.is_absolute() and "private" not in path.parts
            assert (ROOT / "research" / path).is_file()
    f05 = next(package for package in packages if package["id"] == 7)
    assert f05["first_batch"]["evaluation_independent_two_day_accounts_remaining"] == 0
    assert f05["first_batch"]["whole_strategy_fresh_holdout"] is False
    assert "family_open" in f05["execution_status"]
    f03 = plan["f03"]
    assert f03["total_strategy_shards"] == f03["original_strategy_shards"] + f03["ml_off_strategy_shards"]
    assert f03["ml_off_is_selection_candidate"] is False
    assert f03["final_requires_b_selection"] is True
