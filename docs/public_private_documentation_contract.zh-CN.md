...
历史实施回执（不是当前规范）：[公开/私有证据治理交接](public_private_governance_handoff_20260812.md)。

本仓库应当能够直接在 GitHub 上被理解，而不要求读者访问所有者的工作站、存储卷、云账户或 live 交易凭证。除非 Markdown 位于[非研究类私有证据所有者映射](#证据归属与本地目录)定义的 Git 忽略私有根目录，或具体研究单元自身被忽略的 `private/` 目录下，否则面向人的 Markdown 默认属于公开文档。
...
跨项目私有运营细节属于被忽略的 `docs/private/`；live、data、model 和 execution 组件的证据属于[非研究类私有证据所有者映射](#证据归属与本地目录)中定义的对应私有根目录；研究专用定位器和证据索引属于[公开研究与私有证据布局](#证据归属与本地目录)所定义的所属单元被忽略 `private/` 目录。私有 Markdown 以 `Local only — do not publish.` 开头。私有 JSON 或 YAML 使用顶层 `visibility` 或 `documentation_scope` 值 `local_only_do_not_publish`。位于 `private/original_public_machine_records/` 下、按原字节保存的历史源，以及 consumer 拒绝未知字段的 schema 受限 runtime/config 记录，应保持不变，并从其被忽略目录、`README.local.md` 和 catalog 条目继承私有分类。当前主机 pointer、物理存储映射、含 secret 的配置和仅供所有者使用的产物 catalog 均位于这些私有表面。由于它们不属于公开仓库，所以可以包含精确的本地定位器。
...
| 被忽略的根目录 | 规范职责 |
| --- | --- |
| `docs/private/` | 仓库级 runtime/config 指针、历史运营配置、脱敏映射及仓库目录。 |
| `live/private/` | 组件诊断和运行证据索引，不拥有当前主机或配置权威。 |
| `data/private/` | 受限来源映射、准入定位器和来源质量索引；原始行情保留在仓库外。 |
| `models/private/` | 模型、checkpoint、评估定位器及私有工作树 projection 索引；大型产物保留在受治理的存储中。 |
| `execution/private/` | 生命周期、队列、订单、成交及 journal 索引；原始账户记录保留在仓库外。 |
| `research/**/private/` | 具体研究族、共享层或系统工程单元的目录及未公开研究上下文；上层汇总目录不另建重复目录。 |

每个根目录由 Git 忽略，包含以 `Local only — do not publish.` 开头的 `README.local.md`，以及遵循[目录 schema](../research/private_artifact_catalog.schema.json) 的 `catalog.current.local.json`。Git 忽略不是加密或 secret 管理器。先读研究单元的公开文档；只有任务授权使用所有者侧证据时才读取私有目录。

字段和模板以 schema 为唯一权威：条目记录稳定 `artifact_id`、私有解析位置、角色、可用性、`panel_role`、`read_gate`、相关公开文档；已准入证据还须具有验证过的 SHA256 和字节数。可选验证时间、源区间和公开 projection 描述同一产物。目录本身不授予开发、验证、holdout、动作或 live 权限。

旧分类 `historical_or_operational_unspecified` 保持 fail closed，直到所有者在不查阅受限结果的前提下完成分类。哈希或字节数为空的条目只是隔离定位记录，不是已准入证据。历史字节缺失仍为不可用，不得因路径相似而用当前可变文件替代。获得授权的私有验证检查忽略目录、标记、schema、唯一归属、解析、哈希/大小及读取门；它遵循下文限定范围的审查门，不是普通源码交付前置条件。

研究报告可公开协议、特征/标签定义、逻辑来源、时间划分、代码/测试、汇总结果、限制及使用占位符的复现命令。小型证据包须允许合法再分发、移除私有字段并提供直接公开链接。私有原件和冻结身份保持不变，展示整理不是新结果。

## 审查门

源码发布前，应对预定发布的文件树运行 [`audit_public_documentation.py`](../scripts/audit_public_documentation.py)，并检查实际上传内容中的凭据、私有定位器、购买数据及其他非公开内容。公开审计会扫描面向人的文档、机器文档、源代码、公开 archive、结构化进程标识符、projection 可用性和仓库链接。同时还应验证 Markdown reflow 具有幂等性，并确保受保护的代码块、表格、公式、front matter 和显式 hard-break block 保持不变。

根据所有者 2026-09-19 的指令，[`audit_private_evidence.py`](../scripts/audit_private_evidence.py) 不再是普通源码发布的前置条件，包括向公开仓库发布。仅在获得授权的私有证据治理或验证任务中运行它。与上传内容无关的所有者侧 catalog、权限、哈希或 read-gate 问题不得阻挡源码更新。此项分离不授权上传私有证据、绕过研究使用限制或宣称未经验证的经济结果；证据主张仍需对应任务的支持，源码发布不等于证据准入。
