...
## 七月份生成输入退役（2026-09-14）

用户另行退役了五组派生产物。历史文档和机制实现继续保留；这是输入退役，不是新的经济实验，也不代表这些研究族整体失效。

| 退役产物 | 受影响研究及限制 |
| --- | --- |
| `features_btcusdc_causal_v10_minimal141_context_20260728` | F06 挂单特征上下文；旧特征支持／标签不能代替新日历的重建输入。命令行现在必须显式提供兼容上下文。 |
| `trade_features_causal_v3_20260727` | F09 毒性／P3 动作诊断；旧成交节奏输入和结果仅保留历史含义，不证明新来源已经验证。主进程及新启动的工作进程都必须绑定显式成交 manifest。 |
| `trade_features_causal_v5_expanded_20250801_20260725` | F03 历史来源覆盖／profile 输入；带日期路径仅保留为历史描述，已移除的数据载荷不可用。不得把新来源重跑冒称旧结果。 |
| `model_features` | 现存 F04 外部市场 v2/v4 特征缓存，不是当前13-head模型包。保留生成和机制代码；重新评价预测或执行结论前，需生成并绑定新输入。 |
| `replay_l2_deep250_snapshot100ms_probe_v1` | 历史深盘口工程探针；不属于当前队列、特征或经济证据。 |

不因删除而额外推断每组数据都有时间泄漏。先前记载的失真、时钟和标签缺陷维持原有适用范围。新数据研究应解释相关旧缺陷，重建特征和标签、检查实际结果时间边界，并按声明的协议比较仿真／拟合／经济表现变化，之后才更新公开结论或博客。previous-use 和封存结果权限不重置。规范原始行情、购买归档、正在运行的盘口重建和已分发到计算节点的输入均不在本次退役范围内。
