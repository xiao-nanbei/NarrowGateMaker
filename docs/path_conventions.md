...
The stable BTCUSDC raw naming contract is:

```text
${NARROWGATE_RAW_DATA_ROOT}/binance_futures/BTCUSDC/YYYY-MM-DD/
    incremental_book_L2.parquet
    trades.parquet
    funding.parquet
```

Use `data_paths.daily_market_path(day, symbol, channel)` rather than constructing provider-specific paths. `NARROWGATE_RAW_DATA_ROOT` is independently configurable even when `NARROWGATE_DATA_ROOT` points to derived products. An explicit `root=` passed to that helper denotes a storage workspace whose child is `raw/`; migration uses that explicit root without rebinding global configuration. Date strings are UTC; channel names and case are exact. Reference BTCUSDT uses a separate symbol directory, never an execution-market alias.

Temporary downloads may retain provider syntax only beneath `raw/.incoming/`, until validated daily conversion. Provider, original timing and native capability remain provenance, not alternative research paths. Do not recreate retired `cryptohftdata`, `tardis`, `raw_trades` or `tardis_compatible` aliases. Historical manifests retain their original names as evidence, not current directory instructions.

Separate auxiliary containers use `raw/binance_spot/<SYMBOL>/YYYY-MM-DD/aggTrades.parquet` and `raw/history/binance_futures/<SYMBOL>/YYYY-MM-DD/metrics.parquet`. Their original timestamp units and metrics observation strings remain unchanged; readers apply the channel's existing timestamp rules, not the execution-book clock by assumption.

BTCUSDC execution `trades.parquet` is the native-ID union of individual sources. Self-generated fixed UTC 100ms price/side groups live at `derived/binance_futures/BTCUSDC/YYYY-MM-DD/trade_aggregates_100ms.parquet`, never under raw `aggTrades.parquet`. They have synthetic IDs and bucket-completion feature times, not native parent membership or receive clocks. Current 1s bars are built directly from individuals. Authorized retirement of BTCUSDC native aggregate originals is recorded separately; historical native-message contracts remain input-unavailable rather than rebound to these derived groups. BTCUSDT/spot reference native aggregates are unchanged.

Processed names such as `bars_1s`, `features_btcusdc` and versioned `normalized_l2_100ms_v2/{bbo,l2}` live under the derived root. Old retained-registry counts are historical; use the current continuous-calendar manifest for availability and separate previous-use records for research permission. Additional external-venue processed products use:

```text
${NARROWGATE_DATA_ROOT}/external_venues/<venue>/<instrument>/<symbol>/<dataset>
```

Current values are `<instrument>=perp|spot` and processed `<dataset>=bars_1s|features_1s`. External raw `trades`, `raw`, and `archive_source_utc8` use the matching `${NARROWGATE_RAW_DATA_ROOT}/external_venues/...` tree; ingestion manifests live under `${NARROWGATE_RAW_DATA_ROOT}/.provenance/external_venues/.../manifests`. Exchange product names such as `linear` and `USDT-FUTURES` belong in metadata, not in physical directory names. Do not create parallel aliases such as `usdt-futures`, `linear`, `swap`, or venue-specific top-level data roots.
