...
日文件的交易所时间可能重叠。如果下一文件以真实快照开头，显式来源计划在该快照的原始交易所时间交接，排除旧文件重叠的尾部；最多同时打开两个流式读取器，不改写原始时间戳。如果同供应商下一文件以增量开头，则保留这些更新，继续执行既有序号／时间检查；没有快照时不能切换供应商。输入批次必须在处理交接之前包含下一文件，不能先消费重叠尾部并保存断点，再发现更早的快照。处理后 BBO/L2 也必须遵守相同来源选择；盘口组件相等不能单独证明跨供应商账户回放通过。
