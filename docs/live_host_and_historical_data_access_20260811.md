...
The public repository does not identify a current live host or an operational endpoint. `<current-live-host>`, `<current-live-instance>`, `<current-live-ssh-target>`, `${NARROWGATE_REMOTE_ROOT}`, and `<current-live-epoch>` are logical fields that become meaningful only after the owner-private pointer has been validated. Operational scripts must resolve `${NARROWGATE_LIVE_REMOTE_POINTER}` or an explicitly supplied `NARROWGATE_LIVE_REMOTE`; they must never copy an address, instance identifier, epoch identifier, or repository path from a public or dated report. A cloud or region label in historical evidence does not turn a placeholder into current routing authority.
...
| --- | --- |
| `[<current-live-epoch-start>, now)` | The source resolved and revalidated from `${NARROWGATE_LIVE_REMOTE_POINTER}`; the public placeholder is not a query endpoint |
...
The mutable [`operational_baseline_current.json`](../research/families/f10_live_replay_attribution/docs/operational_baseline_current.json) resolves the public [`operational_baseline_identity_20260825_v13`](../research/families/f10_live_replay_attribution/docs/operational_baseline_identity_20260825_v13.json). v13 is a governance locator, not a source of live authority. Its `current_live` binding summarizes the private owner-active BUY E3 release-v3 with BUY E3 enabled, the SELL owner policy unchanged, and no active shadow or companion. Backtest mechanics now resolve independently through [`operational_backtest_mechanics_current.json`](../research/families/f10_live_replay_attribution/docs/operational_backtest_mechanics_current.json) to the owner-private mechanics-safety successor. That successor grants mechanics availability only; the immutable v12 50-day economics remain a stale historical comparator. The current live alias and v12 are both forbidden replay fallbacks.

The exact current endpoint, instance, epoch, host-key pin, archive locators, current process status, and lifecycle admission receipts live only in `${NARROWGATE_LIVE_REMOTE_POINTER}` and the private catalogs. Public automation and analysis code must resolve that pointer; public prose and v13 never grant remote-control authority. The frozen post-lifecycle capture is explicitly not latest-liveness evidence, and neither v13 nor the live E3 chain grants economic or backtest authority. The separate safety successor grants only fail-closed backtest mechanics availability.
