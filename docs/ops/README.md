...
`build-envelope` binds the artifacts actually required by the selected config. With ML enabled, retain `--model-authorization` and the complete 13-head schema/private-authorization checks. Without `--model-authorization`, `--p3 <artifact>` is required to bind the P3 artifact independently for ML-OFF; disabling inference does not remove P3 validation. Supply `--state-conditioned-policy <artifact>` when state-conditioned policy mode is `shadow` or `active`. The existing envelope binds these bytes; it does not require another manifest or approval file.

Policy permission is part of the existing deployment envelope, not an environment switch or a research verdict. For an explicitly approved action, repeat `build-envelope --approve-policy <policy>` using `q90_action`, `f05_boolean_cooldown`, `f05_buy_e3`, or `state_conditioned_quote_policy` as appropriate. State-conditioned policy requires action approval only in `active` mode; `disabled` and `shadow` grant no action authority, and documenting an existing shadow mode does not authorize enabling a new shadow mechanism. Never generate approval arguments automatically from config fields. The envelope binds this approval list to the exact config and artifact bundles; startup compares enabled actions with the verified list once, before constructing the engine, and passes the admitted result to runtime identity/logging. No additional receipt or hash chain is introduced.

Config validation and ordinary `preflight_live_deploy.py` check configuration/artifact compatibility; ordinary preflight reports policy admission as not evaluated and cannot authorize startup. The existing `candidate-verify` transaction additionally uses `--check-policy-approval` to consume a verified deployment envelope and call the same admission function, so a missing approval fails before the old service is stopped. That pre-stop diagnostic is not a startup credential: the new process admits its own verified release once and never trusts a previous preflight output. BUY E3 loaders do not interpret `research_supported`, `owner_risk_accepted`, or historical `evidence_route` descriptions as permission, and state-conditioned policy's research `promotion_status` is not a live grant. Research conclusions remain in their research records. Legacy `NARROWGATE_ALLOW_*_PRIVATE_DEPLOY` and `NARROWGATE_ALLOW_STATE_CONDITIONED_POLICY_LIVE` flags have no admission effect. State-policy artifact structure, supported actions, overlap/uplift/freshness limits, and runtime numerical checks remain enforced; its mode and artifact path remain restart-only, and reload preserves the startup-loaded bytes. An older envelope without `policy_approvals` grants no optional policy: disabled-policy releases remain valid, while enabling one requires a newly approved envelope, not mutation of an existing immutable release. Publishing this change as a new source commit does not deploy or restart live.

Envelope construction performs the complete nested build validation, including the original lock, wheelhouse, and wheel archives. Ordinary envelope loading and startup do not reopen those build inputs. They still validate the bound native-build and install receipts and the current native module; startup also verifies the complete installed `RECORD` inventory, interpreter, and ABI. This separates build evidence from installed runtime verification without removing the runtime integrity checks.
