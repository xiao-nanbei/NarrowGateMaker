...
The historical audit used `models/replay_cache_audit.py` and `scripts/audit_legacy_replay_cache.py`. These tools and their version-specific prune chain were retired on 2026-09-20. This report preserves historical findings, not a current disk inventory or an executable cleanup procedure. See the [current cache contract](replay_cache_materialization_contract.md).
