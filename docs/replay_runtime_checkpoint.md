...
A CryptoHFT plan row may explicitly select `recorder_snapshot_clock=preceding_update_id`. A recorder snapshot stamped at an exact hour, without a transaction timestamp, is anchored to the immediately preceding actual update only when their final sequence IDs match. Its original event/receive timestamps, levels and all deltas remain intact; unmatched clocks still fail the ordinary monotonic check. The opening snapshot can read the previous raw hour's terminal update, whose identity is recorded, so input rotation retains the same anchor. This is an explicit modeled snapshot-state clock, not a measured exchange snapshot time or live arrival time. The default remains `original`; raw hourly caches remain unchanged. Component tests cover checkpoint and prefix-rotation behavior. Normalization does not yet expose this option: matching prepared inputs and full-account validation are still required before using it for a complete calendar baseline.

With this explicit plan, continuous input concatenation permits different daily `book_source_authority` labels and reports `mixed_explicit_daily_sources`, retaining each original label and eligibility in `book_quality_by_day`. Without the plan, mismatched source labels are still rejected. Trade-source, dataset-version and schema compatibility checks remain unchanged; a mixed window does not inherit native eligibility from one of its days. A real ten-minute provider-transition diagnostic matched 203 decisions, 67 quote records, funding and aggregate accounting after an automatic midnight checkpoint. It produced no fills, so it does not qualify active campaign or filled-order behavior across that transition; separate native-midnight and synthetic state tests cover other paths. Its intentionally gap-containing window used relaxed historical coverage admission, not relaxed live stale-book protection, and is not a full-period baseline.
