...
The isolated Linux candidate at the source version recorded in the work list has a newly built native extension, passing dependency checks and 270 cooldown, quote and public-signal tests. Normal installed imports were checked outside the checkout. This closes only the candidate runtime/build check, not real-market target-configuration assembly or live activation. The current Tardis account evidence disables both cooldown policies; the retained enabled-policy account uses a retired model/input protocol. Neither can be relabelled as a completed current native-enabled account restoration. These outstanding checks do not block independent from-start offline parameter comparisons that do not use restoration or AWS.
