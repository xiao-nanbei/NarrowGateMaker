...
def _validate_price_quantity(frame: pd.DataFrame, source: str) -> None:
    for name in ("price", "quantity"):
        values = frame[name].to_numpy(dtype=float)
        if not np.isfinite(values).all() or (values <= 0.0).any():
            raise ValueError(f"{source} {name} must be finite and positive")


def _order_diagnostics(frame: pd.DataFrame, identity: str) -> dict[str, int]:
    ids = frame[identity].diff()
    clocks = frame["exchange_ts_ms"].diff()
    return {
        "id_reversals": int(ids.lt(0).sum()),
        "id_adjacent_duplicates": int(ids.eq(0).sum()),
        "exchange_time_reversals": int(clocks.lt(0).sum()),
    }


def _id_gap_diagnostics(starts: np.ndarray, ends: np.ndarray) -> dict[str, Any]:
    gaps = starts[1:] - ends[:-1] - 1
    positions = np.flatnonzero(gaps > 0)
    return {
        "interval_count": int(len(positions)),
        "unrepresented_id_count": int(gaps[positions].sum()),
        "examples": [
            {"first_id": int(ends[i] + 1), "last_id": int(starts[i + 1] - 1),
             "count": int(gaps[i])}
...
def normalize_individual_trades(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    _validate_price_quantity(out, "individual")
    out.attrs["input_order_diagnostics"] = _order_diagnostics(out, "trade_id")
...
def normalize_aggtrades(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    _validate_price_quantity(out, "aggregate")
    if "normal_quantity" in frame.columns or "nq" in frame.columns:
        ...
        normal = out["normal_quantity"].to_numpy(dtype=float)
        if not np.isfinite(normal).all() or (normal < 0.0).any():
            raise ValueError("supplied aggregate normal_quantity must be finite and nonnegative")
    else:
    ...
    out.attrs["input_order_diagnostics"] = _order_diagnostics(out, "agg_trade_id")
...
def build_individual_aggtrade_mapping(
    ...
    *,
    ...
    the aggregate message's feature-ready timestamp. A mismatch fails closed by
    default. Diagnostic mode retains all denominators and reports findings;
    malformed schemas, duplicate identities and overlapping ranges still raise.
    """
    ...
    if raise_on_findings and require_full_individual_coverage and not bool(covered.all()):
    ...
    checks = (
        "trade_ids_contiguous", "price_match", "aggressor_side_match",
        "aggregate_timestamp_in_child_span", "feature_ready_causal",
    )
    reason_counts = {
        name: int((~aggregate_audit[name].astype(bool)).sum()) for name in checks
    }
    reason_counts["quantity_match"] = int((~(total_match | normal_match)).sum())
    if failures and raise_on_findings:
    ...
    child_counts = aggregate_audit["mapped_individual_count"]
    has_child = child_counts.gt(0)
    partial = has_child & child_counts.lt(aggregate_audit["expected_individual_count"])
    span_offsets = (
        (aggregate_audit["aggregate_exchange_ts_ms"] - aggregate_audit["first_child_exchange_ts_ms"]).clip(upper=0)
        + (aggregate_audit["aggregate_exchange_ts_ms"] - aggregate_audit["last_child_exchange_ts_ms"]).clip(lower=0)
    ).loc[has_child]
    diagnostic_findings = bool(failures) or not bool(covered.all())
    example_rows = aggregate_audit.loc[
        ~aggregate_audit["mapping_valid"] | ~aggregate_audit["trade_ids_contiguous"]
    ].head(8)
    examples = []
    for row in example_rows.to_dict("records"):
        first = row["first_child_exchange_ts_ms"]
        last = row["last_child_exchange_ts_ms"]
        parent_time = int(row["aggregate_exchange_ts_ms"])
        span_offset = (
            min(0, parent_time - int(first)) + max(0, parent_time - int(last))
        ...
        examples.append({
            ...
            "mapped_individual_count": int(row["mapped_individual_count"]),
            "expected_individual_count": int(row["expected_individual_count"]),
            "first_child_exchange_ts_ms": (
                int(row["first_child_exchange_ts_ms"])
                if pd.notna(row["first_child_exchange_ts_ms"]) else None
            ),
            "last_child_exchange_ts_ms": (
                int(row["last_child_exchange_ts_ms"])
                if pd.notna(row["last_child_exchange_ts_ms"]) else None
            ),
            "aggregate_exchange_ts_ms": int(row["aggregate_exchange_ts_ms"]),
            "aggregate_timestamp_span_offset_ms": span_offset,
            **{
                name: float(row[name]) if pd.notna(row[name]) and np.isfinite(row[name]) else None
                for name in ("aggregate_quantity", "individual_quantity", "aggregate_normal_quantity",
                             "aggregate_price", "child_price_min", "child_price_max")
            },
            "quantity_identity": str(row["quantity_identity"]),
            **{name: bool(row[name]) for name in checks},
        })
    summary = {
        "schema_version": SCHEMA_VERSION,
        "source_contract_id": SOURCE_CONTRACT_ID,
        "status": "findings" if not raise_on_findings and diagnostic_findings else "passed",
        "individual_rows": int(len(children)),
        "mapped_individual_rows": int(len(mapped)),
        "unmapped_individual_rows": int((~covered).sum()),
        "unmapped_individual_id_examples": [int(value) for value in trade_ids[~covered][:8]],
        "aggregate_rows": int(len(parents)),
        # Preserve the historical field's meaning; use the new field for
        # parents with at least one observed child, not merely an audit row.
        "mapped_aggregate_rows": int(len(aggregate_audit)),
        "actual_matched_aggregate_rows": int(has_child.sum()),
        "zero_child_aggregate_rows": int((~has_child).sum()),
        "partial_child_aggregate_rows": int(partial.sum()),
        "valid_aggregate_rows": int(aggregate_audit["mapping_valid"].sum()),
        "invalid_aggregate_rows": int((~aggregate_audit["mapping_valid"]).sum()),
        "reason_counts": reason_counts,
        "reason_denominator_aggregate_rows": int(len(parents)),
        "reason_unobserved_aggregate_rows": int((~has_child).sum()),
        "reason_counts_semantics": "failed or unobservable checks over all supplied parents; zero-child parents are not successful checks",
        "aggregate_finding_examples": examples,
        "aggregate_timestamp_span_offset_ms_min": int(span_offsets.min()) if len(span_offsets) else None,
        "aggregate_timestamp_span_offset_ms_max": int(span_offsets.max()) if len(span_offsets) else None,
        "aggregate_timestamp_span_offset_semantics": "negative before child span, positive after child span, zero within span, null without observed children",
        "source_order_diagnostics": {
            "individual": children.attrs["input_order_diagnostics"],
            "aggregate": parents.attrs["input_order_diagnostics"],
        },
        "id_gap_diagnostics": {
            "individual": _id_gap_diagnostics(trade_ids, trade_ids),
            "aggregate_parent_ranges": _id_gap_diagnostics(starts, ends),
            "semantics": "observed ID discontinuities, not proof of missing capture or permission to fabricate trades",
        },
        ...
        ) and (raise_on_findings or not diagnostic_findings),
