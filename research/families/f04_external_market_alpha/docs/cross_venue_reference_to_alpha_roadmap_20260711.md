...
The first frozen profile is `live/profiles/latency/aws_tokyo_t3_medium_amzn2023_native_public_ws_20260711_3600s.json`. It contains 682,110 events from AWS Tokyo `t3.medium` / Amazon Linux 2023 / native compute / synchronous gateway. External BBO median lag was about 5-6ms for Bitget, 29-36ms for OKX, and 38-40ms for Bybit; external BBO p99 widened to roughly 268-701ms. A four-fill wiring smoke showed that p99 replay effectively removed the 10ms flow state and weakened 100ms direction agreement. This proves the visibility simulator is active, not that any action has positive uplift.
