...
owner 授权公开的[首批 Development 证据包](risk_selection_development_20260907/README.zh-CN.md)包含实际研究模型、全部 43 个训练标签、18 个后续标签、单独的 horizon 检查及完整实验臂汇总。E 改变了动作，但未超过复用的 B0；C 没有选择撤单。这是精简研究披露，不是附带的实盘模型或可独立重跑的原始行情包。
