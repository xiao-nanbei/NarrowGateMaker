#!/usr/bin/env python3
"""Frozen Development runner for multiscale EMA ADD-vs-WAIT value evidence."""
...
SPEC = ROOT / (
    ...
    "multiscale_ema_add_wait_incremental_value_v1_spec_20260809.json"
)
EXECUTION_AMENDMENT = ROOT / (
    "research/families/f05_fill_quality_quote_ev/docs/"
    "multiscale_ema_add_wait_incremental_value_v1_execution_amendment_20260809.json"
)
PLAN = DATA_ROOT / (
    "cache/replay_dag/"
...
OUTPUT = DATA_ROOT / (
    "reports/"
    "multiscale_ema_add_wait_incremental_value_v1_20260809"
)
...
def _validate_cached_table(
    *,
    ...
    if expected_amendment_sha256 is not None and (
        manifest.get("execution_amendment_sha256")
    ...
    if int(manifest.get("row_count", manifest.get("eligible_row_count", -1))) != len(
        frame
    ):
...
def _spec_and_plan() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for key, role in (
        ...
        _validate_hash(path, row["sha256"], role=role)
    plan = _load_json(PLAN)
    days = tuple(spec["development_denominator"]["ordered_utc_days"])
    plan_days = tuple(
        row["utc_day"] for row in plan["identity_payload"]["days"]
    )
...
def _offline_params(spec: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    config_path = ROOT / str(config_row["path"])
...
def _day_row(plan: Mapping[str, Any], day: str) -> dict[str, Any]:
    rows = {
        row["utc_day"]: row for row in plan["identity_payload"]["days"]
    }
    if day not in rows:
...
def _generation_from_row(row: Mapping[str, Any]) -> MarketGeneration:
    return MarketGeneration(
        ...
        feature_ready_index=int(
            row[GENERATION_COLUMNS["feature_ready_index"]]
        ),
        prediction_index=int(row[GENERATION_COLUMNS["prediction_index"]]),
        snapshot_mid_tick_x2=int(
            row[GENERATION_COLUMNS["snapshot_mid_tick_x2"]]
        ),
    )
...
def _attach_external_release(
    ...
    clock = clock.sort_values(
        ["market_event_generation", "ts_ms", "side"]
    )
    generation_source_columns = tuple(GENERATION_COLUMNS.values())
    for event_index, group in clock.groupby(
        "market_event_generation", sort=False, observed=True
    ):
        if group["ts_ms"].nunique(dropna=False) != 1:
            raise StudyError(
                f"market event {event_index} has multiple decision timestamps"
            )
        if len(group.loc[:, generation_source_columns].drop_duplicates()) != 1:
            raise StudyError(
                f"market event {event_index} has side-dependent market generation"
            )
    clock = clock.drop_duplicates("market_event_generation", keep="first")
    clock_records = clock.to_dict("records")
    clock_generations = [_generation_from_row(row) for row in clock_records]
    clock_identities: list[str] = []
    for position, generation in enumerate(clock_generations):
        generation.validate()
        if position:
            generation.is_strictly_after(clock_generations[position - 1])
        clock_identities.append(generation.identity)
    next_changed_position = [-1] * len(clock_records)
    for position in range(len(clock_records) - 2, -1, -1):
        next_position = position + 1
        if clock_identities[next_position] == clock_identities[position]:
            next_position = next_changed_position[next_position]
        next_changed_position[position] = next_position
    clock_position_by_event = {
        int(row["market_event_generation"]): position
        for position, row in enumerate(clock_records)
    }
    release_payloads: list[dict[str, Any]] = []
    for row in frame.to_dict("records"):
        position = clock_position_by_event.get(
            int(row["market_event_generation"]), -1
        )
        release_position = (
            next_changed_position[position] if position >= 0 else -1
        )
        payload: dict[str, Any] = {
            "external_epoch_support_valid": 0,
            "external_epoch_support_reason": "missing_next_external_generation",
            "release_ts_ms": 0,
            "release_market_event_generation": -1,
            "release_market_generation_identity": "",
        }
        for name in GENERATION_COLUMNS:
            payload[f"release_{name}"] = -1
        if position < 0:
            payload["external_epoch_support_reason"] = (
                "target_not_in_canonical_external_clock"
            )
        elif clock_identities[position] != row["market_generation_identity"]:
            raise StudyError("opportunity generation drifted from external clock")
        elif release_position >= 0:
            release_row = clock_records[release_position]
            release_generation = clock_generations[release_position]
            if not release_generation.is_strictly_after(
                clock_generations[position]
            ):
                raise StudyError("frozen release generation did not advance")
            if int(release_row["ts_ms"]) <= int(row["ts_ms"]):
                raise StudyError("frozen release timestamp did not advance")
            payload.update(
                {
                    "external_epoch_support_valid": 1,
                    "external_epoch_support_reason": "supported",
                    "release_ts_ms": int(release_row["ts_ms"]),
                    "release_market_event_generation": int(
                        release_row["market_event_generation"]
                    ),
                    "release_market_generation_identity": (
                        release_generation.identity
                    ),
                }
            )
            for name, source in GENERATION_COLUMNS.items():
                payload[f"release_{name}"] = int(release_row[source])
        release_payloads.append(payload)
    release_frame = pd.DataFrame(release_payloads, index=frame.index)
    output = frame.copy()
    for column in release_frame.columns:
        output[column] = release_frame[column]
    return output
...
def _eligible_opportunities(
    ...
    *,
    ...
    exact_action = (
        (frame["baseline_action"].eq(ADD_NOW) & frame["action"].eq("place"))
        | (
            frame["baseline_action"].eq(WAIT_ONE_EPOCH)
            & frame["action"].eq("pause")
        )
    )
        ...
        _canonical_sha256(
            {
                ...
                "prospective_campaign_side_id": row[
                    "prospective_campaign_side_id"
                ],
                "decision_ts_ns": int(row["decision_ts_ns"]),
                "market_generation_identity": row[
                    "market_generation_identity"
                ],
            }
        )
        for row in frame.to_dict("records")
    ...
    frame["campaign_max_abs_inventory_units"] = (
        frame["campaign_max_abs_qty_so_far"].abs() / float(lot_size)
    )
    ...
    frame["log1p_l2_near_depth_total"] = np.log1p(
        frame["l2_near_depth_total"].clip(lower=0.0)
    )
    signed_families = (
        "volume_imbalance",
        "price_change",
...
def _add_ema_features(
    ...
    *,
    ...
    for row in ordered.to_dict("records"):
        ...
        mid = float(row["mid"])
        volatility_bps = abs(
            float(row.get("decision_feature_volatility_5s", 0.0))
        ) * 10_000.0
        ...
        if int(ema_row["ema_surface_feature_ready_ts_ns"]) > int(
            row["decision_ts_ns"]
        ):
        ...
        queue_source = str(
            row.get("decision_queue_ahead_source", "trace_queue_unavailable")
        )
...
def census_day(day: str, *, output: Path = OUTPUT) -> dict[str, Any]:
    ...
    if manifest_path.is_file() and data_path.is_file():
        ...
        _validate_cached_table(
            ...
            output=output,
            day=day,
            expected_spec_sha256=_sha256_file(SPEC),
...
def run_census(days: Sequence[str], *, workers: int, output: Path = OUTPUT) -> None:
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as pool:
        futures = {
            pool.submit(census_day, day, output=output): day for day in days
        }
...
def freeze_panel(*, output: Path = OUTPUT) -> dict[str, Any]:
    ...
    support_valid = (
        census["ema_surface_support_valid"].astype(bool)
        & census["external_epoch_support_valid"].astype(bool)
    )
    ...
    representatives = (
        ...
        .sort_values(
            ["utc_day", "side", "cooldown_phase", "stable_sample_hash"]
        )
    )
    selected = (
    ...
    if len(selected) > int(
        spec["opportunity_contract"]["maximum_total_sampled_opportunities"]
    ):
    ...
    manifest = {
        ...
        "selected_campaign_count": int(
            selected["prospective_campaign_side_id"].nunique()
        ),
...
def _validate_execution_amendment() -> dict[str, Any]:
    ...
    if missing_roles:
        raise StudyError(
            f"execution amendment is missing required roles: {missing_roles}"
        )
    for row in rows:
        path = _resolve_bound_path(str(row["path"]))
        _validate_hash(path, row["sha256"], role=row["role"])
    source = _spec_and_plan()[0]["source_contract"]
    role_rows = {str(row["role"]): row for row in rows}
    expected_role_sources = {
        "frozen_spec": {"path": str(SPEC), "sha256": _sha256_file(SPEC)},
...
def _checkpoint_payload(
    *,
    ...
    return {
        ...
        "action": str(action),
        "spec_sha256": _sha256_file(SPEC),
        "execution_amendment_sha256": _sha256_file(EXECUTION_AMENDMENT),
...
def _run_arm(
    ...
    *,
    ...
    params.update(
        {
            ...
            "ema_add_wait_fork_target_baseline_action": str(
                opportunity["baseline_action"]
            ),
            "ema_add_wait_fork_target_market_event_index": int(
                opportunity["market_event_generation"]
            ),
            "ema_add_wait_fork_target_generation": {
                target: int(opportunity[source])
                for target, source in GENERATION_COLUMNS.items()
            },
            "ema_add_wait_fork_release_ts_ms": int(
                opportunity["release_ts_ms"]
            ),
            ...
            "ema_add_wait_fork_release_generation": {
                target: int(opportunity[f"release_{target}"])
    ...
    if (
        int(trace.get("frozen_release_ts_ms", 0))
        != int(opportunity["release_ts_ms"])
        or int(trace.get("frozen_release_market_event_index", -1))
    ...
    if (
        int(trace.get("terminal_ts_ms", 0))
        >= int(opportunity["release_ts_ms"])
        and not bool(trace.get("external_epoch_observed", False))
    ):
...
def label_day(day: str, *, output: Path = OUTPUT) -> dict[str, Any]:
    ...
    if day_path.is_file() and day_manifest_path.is_file():
        ...
        _validate_cached_table(
            ...
            expected_spec_sha256=_sha256_file(SPEC),
            expected_plan_sha256=_sha256_file(PLAN),
            expected_amendment_sha256=_sha256_file(EXECUTION_AMENDMENT),
    ...
    for opportunity in day_panel.to_dict("records"):
        ...
        if observed != bool(
            add["arm_washout_complete"] and wait["arm_washout_complete"]
        ):
        ...
        paired.update(
            {
                ...
                "joint_washout_ts_ms": int(
                    max(add["terminal_ts_ms"], wait["terminal_ts_ms"])
                ),
                ...
                "censor_time_marking_delta_lower_usdc": (
                    add_marking_lower - wait_marking_upper
                    if not observed
                    else math.nan
                ),
                "censor_time_marking_delta_upper_usdc": (
                    add_marking_upper - wait_marking_lower
                    if not observed
                    else math.nan
                ),
                ...
                "add_descendant_submit_count": int(
                    add["descendant_submit_count"]
                ),
                "wait_descendant_submit_count": int(
                    wait["descendant_submit_count"]
                ),
                "add_terminal_inventory_btc": float(
                    add["terminal_inventory_btc"]
                ),
                "wait_terminal_inventory_btc": float(
                    wait["terminal_inventory_btc"]
                ),
                "add_boundary_mid_value_usdc": float(
                    add["boundary_mid_value_usdc"]
                ),
                "wait_boundary_mid_value_usdc": float(
                    wait["boundary_mid_value_usdc"]
                ),
                "add_boundary_executable_value_usdc": float(
                    add["boundary_executable_value_usdc"]
                ),
                "wait_boundary_executable_value_usdc": float(
                    wait["boundary_executable_value_usdc"]
                ),
                "add_active_or_pending_order_count": int(
                    add["active_or_pending_order_count"]
                ),
                "wait_active_or_pending_order_count": int(
                    wait["active_or_pending_order_count"]
                ),
                ...
                "joint_terminal_identity": (
                    "absorbing_flat_quarantine_until_later_arm_washout.v1"
                ),
            }
        )
...
def _transform(
    ...
    for phase in allowed_phases:
        train_columns.append(
            train["cooldown_phase"].eq(phase).to_numpy(dtype=np.float64)
        )
        test_columns.append(
            test["cooldown_phase"].eq(phase).to_numpy(dtype=np.float64)
        )
    ...
    *,
    ...
    for draw in range(draws):
        ...
        for day in rng.choice(days, size=len(days), replace=True):
            ...
            for campaign in rng.choice(
                campaigns, size=len(campaigns), replace=True
            ):
                rows = day_frame.loc[
                    day_frame["prospective_campaign_side_id"].eq(campaign)
                ]
                sampled_values.extend(rows[column].to_numpy(dtype=np.float64))
                sampled_weights.extend(
                    rows["campaign_weight"].to_numpy(dtype=np.float64)
                )
