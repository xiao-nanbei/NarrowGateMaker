"""Current-host concurrent resource gate for the exact BUY E3 live path.

This additive successor leaves the historical v1/v2 gates unchanged.  It is
run from a clean, annotated collector checkout while the live repository stays
on the immutable direct-v3 runtime authority.  The live process is freshly
restarted with BUY E3 disabled and remains the same PID throughout a concurrent
four-file benchmark.  Only aggregate timing/resource evidence is persisted;
the collector never connects to a market stream and never persists benchmark
actions or economic values.

The immutable direct-v3 release is an input authority, not an output of this
...
PROCESS_SCHEMA: Final = f"{OWNER_IDENTITY}.current_host_process_snapshot.v3"
PROCESS_STATUS: Final = "exact_runtime_process_snapshot_complete"
PROCESS_CANONICAL_FIELD: Final = "canonical_process_snapshot_sha256"
BENCHMARK_SCHEMA: Final = f"{OWNER_IDENTITY}.exact_four_file_host_benchmark.v3"
BENCHMARK_STATUS: Final = "exact_four_file_aggregate_benchmark_passed"
BENCHMARK_CANONICAL_FIELD: Final = "canonical_benchmark_receipt_sha256"
RESOURCE_SCHEMA: Final = f"{OWNER_IDENTITY}.current_host_concurrent_resource_gate.v3"
...
DIRECT_V3_EXECUTION_COMMIT: Final = "1be0e062fe2c8ac12a34d5fc2193ca166898105a"
DIRECT_V3_EXECUTION_TREE: Final = "ec54a9fbe5a4e476af4d6e58cc323804f0a2f275"
DIRECT_V3_ANNOTATED_TAG: Final = "f05-owner-buy-e3-direct-live-v3-20260824"
DIRECT_V3_TAG_OBJECT: Final = "00b5d8bb9078a04dee7e2ae2b3ecdec332698106"
DIRECT_V3_RELEASE_FILE_SHA256: Final = (
    "aacf30f0abc978b9a14570cb0082c3858b0f022c2f0cc9daa8a687d71932f396"
)
DIRECT_V3_RELEASE_CANONICAL_SHA256: Final = (
    "b5baea19a925b8fe8b1a8a8f1d387bfcc0c1aa0124b51108556e3df46ab59384"
)
DIRECT_V3_RELEASE_SCHEMA: Final = (
    "causal_multichannel_window_boolean_cooldown_owner_buy_e3_direct_owner_active_release.v1"
)
DIRECT_V3_RELEASE_STATUS: Final = "owner_authorized_direct_live_with_incomplete_evidence"
...
EXACT_DEPLOYED_FILE_SHA256: Final = {
    ...
    "direct_active_release": DIRECT_V3_RELEASE_FILE_SHA256,
}
EXPECTED_DISABLED_CONFIG_SHA256: Final = (
    "d08df3958f4243109036555ba60d58c2599d88560990305f176744d62959c7ef"
)
CURRENT_V3_RUNTIME_SOURCE_SHA256: Final = {
    "buy_e3_runtime": {
        "path": "strategy/boolean_cooldown_buy_e3.py",
        "sha256": "f5cbcae9f6c69fc20be79516e35b3d966ea32bf973ea0ede1797cbe02f501a9d",
    },
    "maker_engine": {
...
RESOURCE_CHECK_NAMES: Final = (
    ...
    "runtime_checkout_exact_direct_v3",
    "collector_is_clean_annotated_direct_v3_descendant",
    "exact_four_deployed_files_bound",
    "immutable_direct_v3_authority_has_no_resource_dependency",
...
AUTHORITY_DESIGN: Final = {
    ...
    "runtime_authority_release_file_sha256": DIRECT_V3_RELEASE_FILE_SHA256,
    "runtime_authority_release_canonical_sha256": DIRECT_V3_RELEASE_CANONICAL_SHA256,
...
def capture_git_execution(
    ...
    *,
    ...
    if not ancestor:
        raise BuyE3CurrentHostResourceGateError("execution is not a direct-v3 descendant")
    if runtime_authority and (
        commit != DIRECT_V3_EXECUTION_COMMIT
        or tree != DIRECT_V3_EXECUTION_TREE
        or annotated_tag != DIRECT_V3_ANNOTATED_TAG
        or tag_object != DIRECT_V3_TAG_OBJECT
    ):
        raise BuyE3CurrentHostResourceGateError("live runtime checkout is not exact direct-v3")
    return {
...
def bind_current_v3_runtime_sources(
    *,
    ...
    for role, frozen in CURRENT_V3_RUNTIME_SOURCE_SHA256.items():
        ...
        runtime_git = hashlib.sha256(
            _run_git(
                runtime_root,
                "show",
                f"{DIRECT_V3_EXECUTION_COMMIT}:{relative}",
            ).stdout.encode()
        ).hexdigest()
        ...
        if {runtime_working, collector_working, runtime_git, collector_git} != {expected}:
            raise BuyE3CurrentHostResourceGateError(
                f"current-v3 runtime source drifted or imported pre-sparse bytes: {role}"
            )
        rows[role] = {
            "role": role,
    ...
    return {
        "direct_v3_execution_commit": DIRECT_V3_EXECUTION_COMMIT,
...
def _validate_direct_release_payload(payload: Mapping[str, Any]) -> None:
    if (
        payload.get("schema_version") != DIRECT_V3_RELEASE_SCHEMA
        or payload.get("status") != DIRECT_V3_RELEASE_STATUS
        or payload.get("canonical_active_release_sha256") != DIRECT_V3_RELEASE_CANONICAL_SHA256
        or document_sha256(payload, "canonical_active_release_sha256")
        != DIRECT_V3_RELEASE_CANONICAL_SHA256
    ):
        raise BuyE3CurrentHostResourceGateError("direct-v3 release identity drifted")
    expected_execution = {
        "execution_commit": DIRECT_V3_EXECUTION_COMMIT,
        "execution_tree": DIRECT_V3_EXECUTION_TREE,
        "annotated_operational_tag": DIRECT_V3_ANNOTATED_TAG,
        "annotated_operational_tag_object": DIRECT_V3_TAG_OBJECT,
        "tag_peeled_commit": DIRECT_V3_EXECUTION_COMMIT,
    }
    if payload.get("execution") != expected_execution:
        raise BuyE3CurrentHostResourceGateError("direct-v3 release execution drifted")
    artifact = _mapping(payload.get("exact_artifact"), "direct-v3 exact artifact")
    roles = _mapping(artifact.get("roles"), "direct-v3 artifact roles")
    if artifact.get("artifact_sha256") != EXACT_ARTIFACT_SHA256 or set(roles) != {
        "manifest",
        "policy",
        "predicate_bundle",
    }:
        raise BuyE3CurrentHostResourceGateError("direct-v3 artifact identity drifted")
    for role in ("manifest", "policy", "predicate_bundle"):
        binding = _mapping(roles.get(role), f"direct-v3 {role} binding")
        if (
            binding.get("role") != role
            or binding.get("file_sha256") != EXACT_DEPLOYED_FILE_SHA256[role]
            or binding.get("mode") != "0600"
        ):
            raise BuyE3CurrentHostResourceGateError(f"direct-v3 {role} binding drifted")
    if roles["manifest"].get("canonical_sha256") != EXACT_ARTIFACT_SHA256:
        raise BuyE3CurrentHostResourceGateError("direct-v3 manifest canonical drifted")
...
def bind_exact_deployed_files(
    *,
    ...
    runtime = LiveBuyE3CooldownPolicy.from_files(
        ...
        active_release_file_sha256=DIRECT_V3_RELEASE_FILE_SHA256,
        active_release_canonical_sha256=DIRECT_V3_RELEASE_CANONICAL_SHA256,
        warmup_s=2048.0,
    ...
    if (
        ...
        or identity.get("file_sha256") != DIRECT_V3_RELEASE_FILE_SHA256
        or identity.get("file_canonical_sha256") != DIRECT_V3_RELEASE_CANONICAL_SHA256
        or identity.get("execution_commit") != DIRECT_V3_EXECUTION_COMMIT
        or identity.get("execution_tree") != DIRECT_V3_EXECUTION_TREE
        or identity.get("annotated_operational_tag") != DIRECT_V3_ANNOTATED_TAG
        or identity.get("annotated_operational_tag_object") != DIRECT_V3_TAG_OBJECT
    ):
        raise BuyE3CurrentHostResourceGateError("four-file runtime loader identity drifted")
    return {
        "artifact_sha256": EXACT_ARTIFACT_SHA256,
        "files": bindings,
        "direct_release_canonical_sha256": DIRECT_V3_RELEASE_CANONICAL_SHA256,
...
def validate_process_snapshot(path: Path) -> dict[str, Any]:
    ...
    expected_execution = {
        ...
        "execution_commit": DIRECT_V3_EXECUTION_COMMIT,
        "execution_tree": DIRECT_V3_EXECUTION_TREE,
        "annotated_tag": DIRECT_V3_ANNOTATED_TAG,
        "annotated_tag_object": DIRECT_V3_TAG_OBJECT,
        "tag_peeled_commit": DIRECT_V3_EXECUTION_COMMIT,
        "direct_v3_commit_is_ancestor": True,
...
def run_exact_four_file_benchmark(
    *,
    ...
    runtime_sources = bind_current_v3_runtime_sources(
        runtime_repository_root=runtime_repository_root,
...
def _validate_exact_deployed_binding(raw: Any) -> dict[str, Any]:
    ...
    if (
        ...
        or binding["direct_release_canonical_sha256"] != DIRECT_V3_RELEASE_CANONICAL_SHA256
...
def _validate_runtime_source_binding(raw: Any) -> dict[str, Any]:
    binding = _exact_mapping(
        ...
        "current-v3 runtime sources",
    )
    files = _exact_mapping(
        binding["files"], tuple(CURRENT_V3_RUNTIME_SOURCE_SHA256), "runtime source roles"
    )
    for role, frozen in CURRENT_V3_RUNTIME_SOURCE_SHA256.items():
        row = _exact_mapping(
            files[role],
            (
                ...
                "sha256",
                "runtime_working_matches_direct_v3",
                "collector_working_matches_direct_v3",
                "collector_head_matches_direct_v3",
            ),
        ...
        if (
            ...
            or row["runtime_working_matches_direct_v3"] is not True
            or row["collector_working_matches_direct_v3"] is not True
            or row["collector_head_matches_direct_v3"] is not True
        ):
            raise BuyE3CurrentHostResourceGateError(f"runtime source binding drifted: {role}")
    if (
        binding["direct_v3_execution_commit"] != DIRECT_V3_EXECUTION_COMMIT
...
def build_resource_receipt(
    *,
    ...
    checks = {
        ...
        "runtime_checkout_exact_direct_v3": (
            runtime_execution.get("execution_commit") == DIRECT_V3_EXECUTION_COMMIT
            and runtime_execution.get("execution_tree") == DIRECT_V3_EXECUTION_TREE
            and runtime_execution.get("annotated_tag") == DIRECT_V3_ANNOTATED_TAG
            and runtime_execution.get("annotated_tag_object") == DIRECT_V3_TAG_OBJECT
        ),
        "collector_is_clean_annotated_direct_v3_descendant": (
            collector_execution.get("direct_v3_commit_is_ancestor") is True
        ...
        == 4,
        "immutable_direct_v3_authority_has_no_resource_dependency": True,
...
def _validate_execution_shape(raw: Any, *, runtime: bool) -> dict[str, Any]:
    ...
    if (
        ...
        or peeled != commit
        or row["direct_v3_commit_is_ancestor"] is not True
    ...
    if runtime and (
        commit != DIRECT_V3_EXECUTION_COMMIT
        or tree != DIRECT_V3_EXECUTION_TREE
        or row["annotated_tag"] != DIRECT_V3_ANNOTATED_TAG
        or tag_object != DIRECT_V3_TAG_OBJECT
    ):
        raise BuyE3CurrentHostResourceGateError("runtime execution is not direct-v3")
    return row
...
def validate_resource_receipt(
    ...
    *,
    ...
    if (
        ...
        or runtime_execution["execution_commit"] != DIRECT_V3_EXECUTION_COMMIT
        or runtime_sources["direct_v3_execution_commit"] != DIRECT_V3_EXECUTION_COMMIT
        or deployed["direct_release_canonical_sha256"] != DIRECT_V3_RELEASE_CANONICAL_SHA256
...
def capture_concurrent_resource_gate(
    *,
    ...
    disabled = capture_process_snapshot(
        runtime_repository_root=runtime_root,
        runtime_annotated_tag=DIRECT_V3_ANNOTATED_TAG,
        pid_file=pid_file,
...
def _parser() -> argparse.ArgumentParser:
    ...
    process.add_argument("--runtime-annotated-tag", default=DIRECT_V3_ANNOTATED_TAG)
