...
本族负责[统一计划](../../RECOMPUTE_407.zh-CN.md)中的重算包 8, 12，详见[机器可读任务清单](../../recompute_407.json)。计划不是执行回执。

## 当前实现与新结果

带时间边界的动作/OPE API 已存在；配对仿真不制造 propensity，OPE 需要真实有支持的行为日志。新动作网格和完整路径评价待完成。
