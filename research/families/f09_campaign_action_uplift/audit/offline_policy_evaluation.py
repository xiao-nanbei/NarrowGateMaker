...
@dataclass(frozen=True)
class OPEConfig:
    actual_outcome_end_col: str | None = None
    day_col: str = "day"
...
def _fold_predictions(
    ...
    if cfg.actual_outcome_end_col is not None:
        if cfg.split_mode != "chronological":
            raise ValueError("actual outcome boundary purge requires chronological OPE")
        if cfg.actual_outcome_end_col not in train:
            raise ValueError("actual outcome endpoints required for OPE training")
        end = pd.to_numeric(train[cfg.actual_outcome_end_col], errors="raise")
        if end.isna().any() or not np.isfinite(end).all():
            raise ValueError("unknown outcome endpoint in OPE training")
        boundary = pd.Timestamp(min(fold.test_days), tz="UTC").value
        train = train[(end < boundary) & (train[cfg.decision_ts_col] < boundary)].copy()
