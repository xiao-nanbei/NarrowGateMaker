...
def bind_trade_manifest(path: Path) -> None:
    """Bind an explicit input in the parent and spawned workers, never a retired default."""
    global TRADE_MANIFEST
...
def require_trade_manifest() -> Path:
    if TRADE_MANIFEST is None:
        raise ValueError("July trade inputs retired; supply --trade-manifest with a compatible input")
    return TRADE_MANIFEST


def build_params(day: str, config_path: Path) -> dict[str, Any]:
    trade_manifest = require_trade_manifest()
...
def prepare_baseline(*, workers: int) -> None:
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max(1, workers),
            initializer=bind_trade_manifest, initargs=(require_trade_manifest(),)) as pool:
...
def freeze_spec() -> None:
    ...
    payload: dict[str, Any] = {
        ...
        "data_and_model": {
            ...
            "trade_manifest": _artifact(require_trade_manifest()),
...
def main() -> int:
    ...
    parser.add_argument("--trade-manifest", type=Path, required=True,
                        help="Explicit compatible individual-trade manifest; no retired July default.")
    ...
    bind_trade_manifest(args.trade_manifest)
