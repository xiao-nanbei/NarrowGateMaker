#!/usr/bin/env python3
"""Development-only randomized replay for maker-only multi-SHORT repair."""
...
SCHEMA_VERSION = "multi_short_reducing_buy_aggression.v1"
FAMILY_ID = "multi_short_reducing_buy_aggression_v1"
...
def _load_spec(path: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if spec.get("schema_version") != SCHEMA_VERSION:
        raise ValueError("unexpected multi-SHORT reducing-BUY aggression spec schema")
    if spec.get("family_id") != FAMILY_ID:
        raise ValueError("unexpected multi-SHORT reducing-BUY aggression family id")
    if canonical_spec_sha256(spec) != str(
        spec.get("canonical_spec_identity_sha256", "")
    ):
        raise ValueError("multi-SHORT reducing-BUY aggression canonical spec hash mismatch")
    ...
    if list(spec["panels"]["development_days"]) != list(
        ...
        raise ValueError("multi-SHORT Development denominator differs from F09 base")
    if len(spec["panels"]["development_days"]) != 40:
        raise ValueError("multi-SHORT v1 requires exactly 40 Development days")
    ...
    if spec.get("scorecard_profile") != score_profile_contract(
        ...
        raise ValueError("action_defense_v1 score profile was not frozen exactly")
...
def _validate_identities(
    ...
    budget_execution.require_identity(
        ...
        "multi-SHORT randomized evaluator",
    )
    ...
    if not bool(junit["passed"]) or missing:
        raise ValueError(f"multi-SHORT repair action contract tests are incomplete: {missing}")
    ...
    params.update(
        {
            ...
            "multi_short_reducing_buy_aggression_family_id": FAMILY_ID,
            "multi_short_reducing_buy_aggression_probabilities": dict(
                spec["behavior_policy"]["probabilities"]
            ),
            "multi_short_reducing_buy_aggression_trigger_inventory_btc": float(
                actions["candidate"]["trigger_inventory_btc"]
            ),
            "multi_short_reducing_buy_aggression_release_inventory_btc": float(
                actions["candidate"]["release_inventory_btc"]
            ),
            "trace_multi_short_reducing_buy_aggression_max": int(
...
def _run_day(
    ...
    raw_trace = pd.DataFrame(
        result.get("_multi_short_reducing_buy_aggression_trace", ())
    )
    raw_events = pd.DataFrame(
        result.get("_multi_short_reducing_buy_aggression_event_journal", ())
    )
    if raw_trace.empty:
        raise RuntimeError(f"{day}: no multi-SHORT repair action assignments")
    ...
    return {
        ...
        "summary": {
            ...
            "control_assignments": int(
                result.get(
                    "multi_short_reducing_buy_aggression_control_assignments", 0
                )
                or 0
            ),
            "candidate_assignments": int(
                result.get(
                    "multi_short_reducing_buy_aggression_candidate_assignments", 0
                )
                or 0
            ),
            ...
            "full_cpp_tick_replay_authority": int(
                bool(
                    result.get(
                        "multi_short_reducing_buy_aggression_full_cpp_tick_replay_authority",
                        False,
                    )
                )
            ),
...
def validate_panel(frame: pd.DataFrame, spec: Mapping[str, Any]) -> None:
    required = {
        ...
        "reducing_buy_fill_count",
        "sell_exposure_fill_count",
        "actual_final_action_change_count",
        "candidate_quote_count",
        "price_action_change_count",
        "defense_pause_observation_count",
        "defense_override_attempt_count",
        "defense_override_effective_count",
    ...
    if missing:
        raise ValueError("multi-SHORT randomized panel is missing: " + ", ".join(missing))
    if frame.empty or frame.duplicated(["day", "campaign_id"]).any():
        raise ValueError("multi-SHORT randomized campaign denominator is invalid")
    if set(frame["action"].astype(str)) != {CONTROL_ACTION, CANDIDATE_ACTION}:
        raise ValueError("both multi-SHORT repair actions require support")
    if not np.allclose(
        pd.to_numeric(frame["behavior_propensity"], errors="coerce"),
        0.5,
        atol=1e-12,
    ):
        raise ValueError("multi-SHORT behavior propensity differs from 0.5")
    development = set(str(day) for day in spec["panels"]["development_days"])
    if set(frame["day"].astype(str)) != development:
        raise ValueError("multi-SHORT run did not cover exact Development")
    if int(pd.to_numeric(frame["maker_violation_count"], errors="coerce").sum()) != 0:
        raise ValueError("multi-SHORT candidate emitted a non-maker quote")
    if int(
        pd.to_numeric(
            frame["action_generated_ioc_or_taker_count"], errors="coerce"
        ).sum()
    ) != 0:
        raise ValueError("multi-SHORT candidate emitted IOC/taker activity")
...
def _daily_policy_value(
    ...
    *,
    ...
    for _, day_frame in frame.groupby("day", sort=True):
        ...
        # Exact 0.5/0.5 assignment makes this the day-level Horvitz-Thompson
        # estimate of rolling the candidate out to every eligible campaign.
        policy_value = 2.0 * candidate_reward - 2.0 * control_reward
        daily_rows.append((policy_value, float(len(day_frame))))
    values = np.asarray(daily_rows, dtype=float)
    if values.size == 0:
        raise ValueError("policy-value estimation requires at least one day")
...
def evaluate_scope(
    ...
    *,
    ...
    total_fill_retention = _retention(derived, "intervention_fill_count")
    reducing_buy_fill_ratio = _retention(derived, "reducing_buy_fill_count")
    sell_exposure_fill_retention = _retention(
        derived, "sell_exposure_fill_count"
    )
    ...
    effective_policy_candidate_rate = float(0.5 * action_change["estimate"])
    maker_violation_count = int(
        pd.to_numeric(candidate["maker_violation_count"], errors="coerce").sum()
    )
    action_generated_ioc_or_taker_count = int(
        pd.to_numeric(
            candidate["action_generated_ioc_or_taker_count"], errors="coerce"
        ).sum()
    )
    defense_override_effective_count = int(
        pd.to_numeric(
            candidate["defense_override_effective_count"], errors="coerce"
        ).sum()
    )
    ...
    if maker_violation_count > 0:
        family_failures.append("maker_quote_invariant_violated")
    if action_generated_ioc_or_taker_count > 0:
        family_failures.append("candidate_generated_ioc_or_taker_activity")
    if (
    ...
    if (
        float(outcomes["inventory_time_avoidance_btc_s"]["interval"]["p025"])
        < float(gates["inventory_time_avoidance_lcb_min"])
    ):
        family_failures.append("inventory_time_increase_not_excluded")
    if (
        float(outcomes["campaign_mae_avoidance"]["interval"]["p025"])
        < float(gates["campaign_mae_avoidance_lcb_min"])
    ):
        family_failures.append("campaign_mae_worsening_not_excluded")
    ...
    if policy_value["lcb95_usdc_per_day"] <= float(
        gates["policy_value_lcb_usdc_per_day_must_exceed"]
    ):
        family_failures.append("policy_value_lcb_not_positive")
    ...
    report = {
        ...
        "candidate_assignment_rate": candidate_assignment_rate,
        "effective_policy_candidate_rate": effective_policy_candidate_rate,
        "actual_action_change": action_change,
        "total_fill_retention": total_fill_retention,
        "reducing_buy_fill_ratio": reducing_buy_fill_ratio,
        "sell_exposure_fill_retention": sell_exposure_fill_retention,
        "maker_only_diagnostics": {
            "candidate_quote_count": int(candidate_quote_count),
            "maker_violation_count": maker_violation_count,
            "action_generated_ioc_or_taker_count": (
                action_generated_ioc_or_taker_count
            ),
            "defense_override_effective_count": defense_override_effective_count,
        },
        "outcomes": outcomes,
    ...
    metrics["fills_retention"] = {
        "estimate": total_fill_retention,
        "source": "candidate_vs_control_mean_post_assignment_total_fill_count",
    }
    evidence = {
        ...
        "candidate_rate": effective_policy_candidate_rate,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(days) != 40:
        raise ValueError("formal multi-SHORT repair action identity requires exactly 40 days")
    ...
    if grade_b_report["total_fill_retention"] < float(
        sensitivity["minimum_total_fill_retention"]
    ):
        grade_b_failures.append("grade_b_total_fill_retention_below_gate")

    primary_pass = bool(scorecard["ranking_eligible"])
    decision = (
        "development_passed_validation_locked"
        if primary_pass and not grade_b_failures
        else "close_multi_short_reducing_buy_aggression_on_development"
    )
    ...
    manifest = {
        "schema_version": "multi_short_reducing_buy_aggression_manifest.v1",
        "family_id": FAMILY_ID,
        "decision": decision,
