...
def _load_funding_history(path: Path, *, symbol: str) -> list[dict[str, Any]]:
    ...
    if path.is_dir():
        ...
        directory = path
        if not any(directory.glob("*/funding.parquet")):
            directory = daily_market_path("2000-01-01", symbol, "funding", root=path).parent.parent
        files = sorted(directory.glob("*/funding.parquet"))
        if not files:
            raise FileNotFoundError("No canonical daily funding files found")
        payload = [row for file in files for row in pd.read_parquet(file).to_dict("records")]
    elif path.suffix == ".parquet":
        payload = pd.read_parquet(path).to_dict("records")
    else:
...
def _utc_accounting_rows(
    trades: list[TradeRow], marks: list[dict[str, Any]], *,
    initial_inventory: float, initial_entry_price: float,
    funding_trace: list[dict[str, Any]], funding_modeled: bool,
    terminal_equity_usdc: float,
) -> list[dict[str, Any]]:
    """Slice the existing fill/funding cash ledger, without resetting any arm.

    UTC boundaries are half-open: midnight fills and funding belong to the new
    day. Fees/funding below are explanatory cashflow columns already in equity.
    Terminal inventory is marked, not liquidated or charged another fee.
    """
    if len(marks) < 2:
        raise ValueError("UTC accounting requires initial and terminal price marks")
    times = [round(float(row.ts) * 1000) for row in trades]
    if any(a > b for a, b in zip(times, times[1:], strict=False)):
        raise ValueError("UTC accounting requires chronological execution/settlement rows")
    q = float(initial_inventory)
    cash = -q * float(initial_entry_price) if initial_entry_price > 0 else 0.0
    fees = funding = 0.0
    cursor = settlement_cursor = 0
    states = []
    previous_boundary = -1
    for mark in marks:
        boundary = int(mark["boundary_ts_ms"])
        if boundary <= previous_boundary:
            raise ValueError("UTC accounting boundaries must be strictly increasing")
        previous_boundary = boundary
        px = mark["mark_price"]
        clock = mark["mark_clock_ts_ms"]
        if px is not None and (not math.isfinite(float(px)) or float(px) <= 0):
            raise ValueError("UTC accounting mark is not positive/finite")
        if clock is not None and (int(clock) > boundary or (states and int(clock) == boundary)):
            raise ValueError("UTC accounting cannot use a future mark")
        while cursor < len(trades) and times[cursor] < boundary:
            row = trades[cursor]
            cash, q = float(row.realized_pnl), float(row.position)
            fees += float(row.fee_usdc)
            cursor += 1
        while (settlement_cursor < len(funding_trace)
               and int(funding_trace[settlement_cursor]["funding_ts_ms"]) < boundary):
            funding += float(funding_trace[settlement_cursor]["funding_cashflow_usdc"])
            settlement_cursor += 1
        if px is None and abs(q) > 1e-12:
            raise ValueError("non-flat UTC accounting boundary lacks a causal mark")
        states.append({
            **mark, "cash_usdc": cash, "inventory_btc": q,
            "equity_usdc": cash + q * float(px or 0.0),
            "fees_usdc": fees, "funding_usdc": funding,
        })
    if cursor != len(trades) or not math.isclose(
        states[-1]["equity_usdc"], terminal_equity_usdc, rel_tol=1e-10, abs_tol=1e-7,
    ):
        raise ValueError("UTC accounting does not reconcile to the complete segment equity")
    rows = []
    for start, end in zip(states, states[1:], strict=False):
        a, b = start["boundary_ts_ms"], end["boundary_ts_ms"]
        rows.append({
            "schema_version": "f01_utc_equity_slices.v1",
            "calendar_date": datetime.fromtimestamp(a / 1000, tz=UTC).date().isoformat(),
            "start_ts_ms": a, "end_ts_ms_exclusive": b,
            "complete_utc_day": a % 86_400_000 == 0 and b - a == 86_400_000,
            **{f"{side}_{field}": state[field] for side, state in (("start", start), ("end", end))
               for field in ("cash_usdc", "inventory_btc", "equity_usdc", "mark_price", "mark_clock_ts_ms")},
            "net_equity_change_usdc": end["equity_usdc"] - start["equity_usdc"],
            "fees_usdc": end["fees_usdc"] - start["fees_usdc"],
            "funding_cashflow_usdc": end["funding_usdc"] - start["funding_usdc"] if funding_modeled else None,
            "funding_mode": "frozen_settlement_tape" if funding_modeled else "unmodeled",
            "mark_basis": end["mark_basis"],
    ...
    return {
        ...
        **{
            ...
            or key in {
                ...
                "initial_live_state_enabled", "initial_live_state_orders_requested",
                "initial_live_state_orders_restored",
            }
        },
    }
...
def _write_partial_day_outputs(out_dir: Path, stem: str, day_result: dict[str, Any]) -> None:
    ...
    if day_result.get("utc_accounting_rows"):
        pd.DataFrame(day_result["utc_accounting_rows"]).to_csv(
            out_dir / f"{stem}.partial.{safe_day}.utc_accounting.csv", index=False,
        )
...
def _params_for_arm(base: dict[str, Any], arm: smoke.SmokeArm) -> dict[str, Any]:
    params = {**base, **arm.overrides}
    model_keys = ("model_dir", "resolved_model_dir", "model_dir_override")
    selected = {}
    for key in model_keys:
        if key in arm.overrides:
            raw = arm.overrides[key]
            if not isinstance(raw, (str, Path)) or not str(raw).strip():
                raise ValueError(f"arm {arm.name!r} requires a non-empty {key}")
            path = Path(raw).expanduser()
            selected[key] = str((ROOT / path if not path.is_absolute() else path).resolve())
    if selected:
        if len(set(selected.values())) != 1:
            raise ValueError(f"arm {arm.name!r} has conflicting model directory overrides")
        # A candidate's directory supersedes the baseline's resolved alias.
        # Keep all loader aliases bound to the same bundle without reloading P3.
        model_dir = next(iter(selected.values()))
        for key in model_keys:
            params[key] = model_dir
    return params
...
def _run_day_campaign_audit(
    *,
    ...
    consume_resume_checkpoint: bool = False,
    ...
    checkpoint_options = {}
    trade_visibility_model = base.get("trade_visibility_model", "retained_native_agg")
    if resume_checkpoint is not None and resume_checkpoint["runtime"].quote_core_params.get(
    ...
    if base.get("record_utc_accounting") and (engine != "python" or not continuous_days):
        raise ValueError("UTC accounting currently requires continuous Python replay")
    ...
    window_cache: dict[tuple, dict[str, Any]] = {}
    ...
    def _window_for_params(params: dict[str, Any]) -> dict[str, Any]:
        ...
        key = (
            raw_model,
            bool(params.get("ml_enabled", True)),
            int(params.get("toxicity_horizon_s", 10)),
        ...
        if key not in window_cache:
            ...
            for source_day in input_days:
                ...
                if runtime_input_bounds is not None:
                    ...
                    # Push the bounded data request below the daily cache/load
                    # boundary; post-load slicing alone materializes a full
                    # D-1/D depth matrix before releasing most of its rows.
                    input_params["_runtime_input_bounds_ms"] = (input_start, input_end + 1)
                ...
                if runtime_input_bounds is not None and params.get("exec_message_delivery_profile_path"):
                    # Full packet composition survives an execution cut. The
                    # modeled batch also keeps its last actual price, not a
                    # cropped prefix's different mark or invented parent ID.
                    frame = loaded["trades"]
                    midnight = int(_day_start_ts(source_day) * 1000)
                    columns = ["trade_id", "transact_time"]
                    if trade_visibility_model == "causal_100ms_batches":
                        columns.extend(("price", "quantity", "is_buyer_maker"))
                        # Include real pre-roll children too. Overlapping
                        # daily inputs are reconciled by identity+all values.
                        completion_parts.append(frame.loc[:, columns].copy())
                    else:
            ...
            if (runtime_input_bounds is not None and resume_checkpoint is None
                    and params.get("exec_message_delivery_profile_path")):
                # Initial transport sampling used the historical market prefix.
                # Preserve those narrow clocks/ordinals, not D-1's depth matrix.
                cached = window_cache[key]
                prefix_days = data_windows._causal_context_days(input_days[0], market_context_warmup_days)
                cached["_book_delivery_prefix"] = {}
                for feed, kind in (("bbo", "bbo"), ("depth", "l2")):
                    book = cached.get(f"{kind}_data")
                    if book is not None and len(book.ts_ms):
                        cached["_book_delivery_prefix"][feed] = bt.load_book_delivery_prefix(
                            prefix_days, kind, int(book.ts_ms[0]),
                            quality_allowed_days=data_windows._formal_quality_allowed_days(params),
                        )
            if completion_parts:
                completion = pd.concat(completion_parts, ignore_index=True)
                if trade_visibility_model == "causal_100ms_batches":
                    # Never timestamp dedup. A conflicting repeated ID remains
                    # duplicated and the batch identity validator rejects it.
                    completion = completion.drop_duplicates().sort_values("trade_id", kind="stable")
                window_cache[key]["_parent_completion_trades"] = completion
    ...
    if execution_message_profile:
        ...
        if trade_visibility_model == "causal_100ms_batches":
            parent_source_identity = [identity for source_day in input_days for identity in
                                      data_windows._execution_source_content_identity(source_day, "trades")]
        else:
    ...
    utc_accounting_rows: list[dict[str, Any]] = []
    ...
    for idx, arm in enumerate(arms, 1):
        params = _params_for_arm(base, arm)
        if runtime_input_bounds is not None:
        ...
        if params.get("record_utc_accounting"):
            utc_accounting_rows.extend({"arm": arm.name, "group": arm.group, **row} for row in _utc_accounting_rows(
                trades, result["_utc_accounting_marks"],
                initial_inventory=initial_inventory, initial_entry_price=initial_entry_price,
                funding_trace=arm_funding_trace, funding_modeled=funding_events is not None,
                terminal_equity_usdc=float(result["pnl"]) + funding_value,
            ))
        if risk_pair_baseline_arm:
    ...
    return {
        ...
        "utc_accounting_rows": utc_accounting_rows,
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--save-utc-accounting", action="store_true",
                        help="Save true UTC equity/cash/inventory slices from continuous Python state; no daily reset")
    ...
    parser.add_argument("--trade-visibility-model",
                        choices=("retained_native_agg", "causal_100ms_batches"),
                        default="retained_native_agg",
                        help="Explicit diagnostic delivery model; 100ms batches use actual individuals and a transferred historical aggTrade latency prior, never native parent IDs")
    parser.add_argument(
    ...
    if args.trade_visibility_model == "causal_100ms_batches" and (
        args.replay_purpose != "diagnostic" or args.engine != "python"
        or args.symbol.upper() != "BTCUSDC" or args.execution_trade_source != "trades"
        or args.runtime_timing_samples is None or args.market_data_latency_profile is None
        or args.market_data_latency_mode != "profile_empirical"
    ):
        raise SystemExit("causal_100ms_batches requires diagnostic Python BTCUSDC individual trades and explicit runtime/empirical market profiles")
    ...
    base["record_utc_accounting"] = bool(args.save_utc_accounting)
    ...
    base["queue_regime_calibration_enabled"] = bool(base.get("_queue_calibration")) and not args.no_queue_regime_calibration
    ...
    base["trade_visibility_model"] = args.trade_visibility_model
    if any(arm.overrides.get("trade_visibility_model", args.trade_visibility_model)
           != args.trade_visibility_model for arm in arms):
        raise ValueError("all arms must share the frozen trade visibility model")
    if (
        active_multi_market
    ...
    def _merge_day_result(day_result: dict[str, Any]) -> bool:
        ...
        utc_accounting_rows.extend(day_result.get("utc_accounting_rows", []))
        ...
        for day in (days[:1] if args.continuous else days):
            ...
            if _merge_day_result(
                _run_day_campaign_audit(
                    ...
                    consume_resume_checkpoint=loaded_runtime_checkpoint is not None,
    ...
    utc_accounting_path = out_dir / f"{stem}.utc_accounting.csv"
    ...
    if args.save_utc_accounting:
        pd.DataFrame(utc_accounting_rows).to_csv(utc_accounting_path, index=False)
    ...
    meta = {
        ...
        **({"runtime_input_batches": runtime_input_batches} if runtime_input_batches else {}),
        ...
        "market_data_latency_mode": args.market_data_latency_mode,
        "trade_visibility_model": args.trade_visibility_model,
        ...
        "save_quote_trace": bool(args.save_quote_trace),
        "save_utc_accounting": bool(args.save_utc_accounting),
        ...
        "outputs": {
            ...
            "utc_accounting_csv": str(utc_accounting_path) if args.save_utc_accounting else "",
