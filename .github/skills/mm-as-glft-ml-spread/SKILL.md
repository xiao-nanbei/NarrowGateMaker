...
- Owner dataset policy (2026-09-12) supersedes the supplier-metadata presentation rules below: all 401 dates in 2025-08-01..2026-09-05 are the current dataset, including the 142 dates whose recovery is closed. Remove supplier names, historical processing classes and retired duplicate entries from current catalogs/UI/summaries, not merely move those distinctions into another displayed metadata layer. Keep raw and derived directories separate. Remove physical provenance fields only after replacing their actual reader dependencies with anonymous stream/priority, reset/observation-only and causal clock semantics; catalog cleanup alone is not physical migration. Do not fabricate original exchange messages, queue progress or new observations by changing labels. Frozen results and previous-use remain historical facts, not current dataset classifications. During gaps, new unsupported quoting/risk stays silent while time, inventory, orders, fees and funding remain accounted for. BBO supplementation changes only top price/quantity and its own observation clock, never unseen deep levels; verify replacement and consumers before deleting the authorized originals.
- Owner orderbook policy (2026-09-11): fuse existing same-market Tardis and CryptoHFT observations into one daily Tardis-compatible canonical book; use the sole available source on single-provider days, and check actual purchased coverage rather than assuming its end. Format conversion, whole-day provider selection or two parallel source folders are not fusion. Complementary observations must reach the unified reader/rebuilt book; preserve exchange/source clocks, native IDs where available, causal ages and coherent snapshot/incremental state. Do not equate timestamp-only matches with native identity or fabricate freshness/queue progress. After verified durable replacement and current-reader/index cutover, delete explicitly authorized superseded provider book originals and staging inputs, retaining compact provenance/retirement identities rather than redundant data trees. This overrides older blanket raw-preservation wording for these replaced book inputs only, not unique unconverted observations, previous-use, frozen results, trades or live/account archives. Future CryptoHFT downloads immediately enter the same conversion/fusion path and retire verified scratch. Complete fusion, consumer wiring, validation and retirement together, with no supplier aliases or symlinks; data readiness is not economic validation.
- Project-maintained source/data/output paths must not use symbolic links, supplier aliases or compatibility link farms. Keep actual market records in a real `raw/` directory and generated bars/features/replay products in a separate real `derived/` directory. Update readers and current catalogs when moving files; resolve immutable historical locators at the reader boundary, not with filesystem links. Do not rewrite installed third-party runtime links or confuse temporary link-rejection fixtures with the maintained project layout.
- Canonical execution data uses `raw/binance_futures/BTCUSDC/YYYY-MM-DD/{incremental_book_L2,trades,funding}.parquet`, with one maintained schema per channel, exchange event clocks and optional source/receive provenance. Supplier names are ingestion metadata, not separate research roots. Convert new hourly book downloads into that daily format, verify values/order/clocks/IDs, then retire verified redundant hourly/staging sources under the owner's unification authorization. Preserve unique data until converted; missing channels are not empty placeholders. Reference symbols keep only their actual required channels. Complete directory migration, reader configuration, UI indexing and redundant-source cleanup together.
- Owner trade policy (2026-09-10): for the fixed 2025-08-01..2026-09-05 BTCUSDC interval, union Vision/CryptoHFT individual records by native trade ID, require exact shared price/quantity/side, report clock conflicts with Vision owning the shared canonical clock/date, and keep adjacent-day context. Generate exact-price/side fixed UTC 100ms flow buckets in `derived/binance_futures/BTCUSDC/YYYY-MM-DD/trade_aggregates_100ms.parquet`; synthetic group IDs are not native aggregate IDs or contiguous child ranges. Feature availability starts at bucket completion, not inferred network receipt. Keep individual execution/queue events and build 1s bars directly from individuals. Following verified output and current-reader/catalog cutover, retire the user-authorized BTCUSDC native aggTrades files with a deletion manifest; old native-message evidence stays historical/input-unavailable, not silently rebound to synthetic data. Do not delete BTCUSDT/spot aggregates or unique individual-source provenance. New updates must maintain this union/derived workflow, not restore native execution aggTrades by default. This data change alone does not prove live feed/feature parity.
- Owner follow-up (2026-09-10): do not download/restore BTCUSDC perpetual native aggTrades, including for retired historical contracts; use individual-source union and local derived aggregates. This offline rule does not change live streams or reference-market inputs. Keep only the latest complete routine data-validation report per actual market/channel/calendar scope, or the latest required per-day record. Remove superseded, duplicate and obsolete failed/smoke checks after verifying the replacement and updating current links; retain only compact necessary identities for retired native-aggregate checks. Preserve market data, current reader manifests/clock sidecars, previous-use, uniquely required current quality evidence, frozen research and live/account archives. Update one current summary rather than adding another cleanup report each time.
- For new work, the continuous-calendar/source-repair rules below supersede older retained-good-day, fresh-start, and physical bad-day deletion defaults. Preserve frozen historical results and strict evidence requirements; data readability never grants research-use or live authority.
...
- Verify actual CryptoHFTData/Binance Vision dataset capabilities before same-venue/instrument/symbol/semantics/unit bars/trades repair; a book source need not supply trades or bars. Prefer existing actual book sources for L2. Deduplicate trades by reliable venue identity, not timestamp alone; retain source conflicts. A bar cannot restore individual fills or queue events, and BTCUSDT/spot is not BTCUSDC execution perpetual.
- Carry only prior valid observations forward; no future-fill. Distinguish verified no-update/no-trade from missing collection. Under an explicit convention a confirmed no-trade bar can inherit close with zero volume/count; collection gaps retain unknown volume/count, not copied prior volume/high/low. Preserve output_ts, last_observed_ts/source clock, source_id, observed/secondary/carried_forward/unknown and stale_age. Without prior valid state emit unknown. New output is not a fresh observation or exact queue progress.
...
- Independent arms and state-independent data tasks may run in parallel. Shards of the same continuous arm must consume that arm's predecessor checkpoint, preserving inventory/orders/FIFO/campaigns/random clocks rather than resetting them. Cross-host continuation requires compatible actual runtime/state; never confuse existing checkpoint support with an implemented automatic fetch/evict scheduler.
...
## CryptoHFT Data Quality and Historical Exclusions
