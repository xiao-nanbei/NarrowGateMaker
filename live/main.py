...
PROSPECTIVE_EPOCH_RUNTIME_CODE_FILES = (
    ...
    "narrowgate/runtime/baseline_epoch_manifest.py",
    "narrowgate/runtime/prospective_baseline_epoch.py",
)
...
def _configured_model_dir(cfg) -> Path:
    ...
    if cfg.ml.enabled:
        ...
        if (model_dir / "public_input_model.json").exists() != (protocol == "execution_v1"):
        ...
        summary.update(
            {
                ...
                "model_contract": {
                    ...
                    "feature_dag_id": (
                        next(iter(model_metadata.values())).get("feature_contract_id")
                        if cfg.ml.enabled and cfg.ml.feature_protocol == "execution_v1"
                        else REQUIRED_FEATURE_DAG_ID if cfg.ml.enabled else None
                    ),
                    "feature_dag_sha256": (
                        next(iter(model_metadata.values())).get("feature_dag_sha256")
                        if cfg.ml.enabled and cfg.ml.feature_protocol == "execution_v1"
                        else REQUIRED_FEATURE_DAG_SHA256 if cfg.ml.enabled else None
                    ),
                    "feature_protocol": cfg.ml.feature_protocol,
                    "p3_path": str(p3_path),
