...
class WSHandler:
    ...
    def __init__(self, engine, cfg):
        ...
        # Compatibility arguments are normalized at startup, never on reconnect.
    ...
    def start(
        ...
        *,
        ...
        if market_snapshot_client is None:
            market_snapshot_client = rest_client
        if listen_key_client is None:
            listen_key_client = rest_client
    ...
    def _start_deep_book_stream(self) -> bool:
        ...
        with self._deep_book_start_lock:
            ...
            if not self._running or not self._deep_book_enabled():
                return True
            snapshot_client = self._market_snapshot_client
            if snapshot_client is None:
                logger.error("Deep book cannot start before snapshot client is bound")
                return False
    ...
    def _record_binance_market_event(
        ...
        *,
        ...
        if event_type == "book":
            ...
            is_individual = source_event_type == "trade" and market_type == PERP_MARKET
            first_trade_id = (
                int(data["f"])
            ...
            row.update(
                {
                    ...
                    "trade_stream_type": (
                        "aggregate" if is_aggregate else "individual" if is_individual else "untyped"
                    ),
                    "trade_payload_schema_version": (
                        ...
                        if is_aggregate
                        else "binance_usdm_trade.v1" if is_individual else "unknown"
                    ),
                    "trade_source_contract_id": (
                        ...
                        if is_aggregate
                        else "binance_usdm_public_individual_trade_receive_time.v1" if is_individual else "unknown"
                    ),
                    "aggregate_trade_id": (
                        int(data["a"])
    ...
    def _subscribe_market_streams(
        ...
        *,
        ...
        """Subscribe the trade granularity required by the selected protocol."""
        ...
        for index, market_symbol in enumerate(ordered_market_symbols):
            try:
                individual = (market_symbol == symbol and
                              getattr(self.cfg.ml, "feature_protocol", "legacy_v12") == "execution_v1")
                stream_kind = "trade" if individual else "aggTrade"
                request_id = self._next_market_request_id(
                    f"SUBSCRIBE {market_symbol}@{stream_kind}"
                )
                if individual:
                    websocket_client.subscribe(stream=f"{market_symbol}@trade", id=request_id)
                else:
                    websocket_client.agg_trade(symbol=market_symbol, id=request_id)
                if market_symbol == symbol:
                    logger.info(
                        f"Subscribe requested id={request_id}: {market_symbol}@{stream_kind}"
                    )
                else:
                    logger.info(
                ...
                if market_symbol == symbol:
                    logger.error(f"Subscribe execution trade stream failed ({market_symbol}): {e}")
                else:
                    logger.error(
    ...
    def restart_user_stream(self, reason: str = ""):
        ...
        listen_key_client = self._listen_key_client
        if not self._running:
            return
        if listen_key_client is None:
            logger.error("User stream cannot restart before listen-key client is bound")
            return
        ...
        try:
            ...
            if event_type == "trade":
                if (str(data.get("s", "")).upper() != self.cfg.symbol.upper()
                        or getattr(self.cfg.ml, "feature_protocol", "legacy_v12") != "execution_v1"):
                    return
                self.engine.signal.on_trade(data, receive_ts_ns=receive_ns)
                self._market_trade_seen[self._stream_key(self.cfg.symbol)] = time.time()
                self._exec_trade_count += 1
                price = float(data["p"])
                if price > 0:
                    self.engine.inventory.update_mark_price(price)
                self._record_binance_market_event(
                    data, market_type=PERP_MARKET, event_type="trade",
                    receive_ts_ns=receive_ns, feature_ready_ts_ns=time.time_ns(),
                )

            elif event_type == "aggTrade":
                if (str(data.get("s", "")).upper() == self.cfg.symbol.upper()
                        and getattr(self.cfg.ml, "feature_protocol", "legacy_v12") == "execution_v1"):
                    return  # Never refresh execution readiness with the wrong granularity.
    ...
    def _on_market_close(self, _, *, session_id: int | None = None):
        ...
        if getattr(self.cfg.ml, "feature_protocol", "legacy_v12") == "execution_v1":
            self.engine.signal.market_disconnected()
        with self._public_market_phase_lock:
            if self._public_market_streams_starting:
    ...
    def validate_config_reload(self, old_cfg, new_cfg) -> None:
        ...
        if (getattr(old_cfg.ml, "feature_protocol", "legacy_v12")
                != getattr(new_cfg.ml, "feature_protocol", "legacy_v12")):
