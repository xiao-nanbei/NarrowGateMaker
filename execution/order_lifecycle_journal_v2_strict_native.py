...
class OrderLifecycleJournalV2SourceCallback:
    ...
    simulator_queue_source: str = "not_recorded"
    exact_queue_path_valid: bool = False
    ...
    def __post_init__(self) -> None:
        ...
        queue_source = _require_id(
            "source callback simulator queue source",
            self.simulator_queue_source,
        )
        if bool(self.exact_queue_path_valid) and queue_source != "native_exchange_book":
            raise ValueError(
                "exact queue path validity requires native_exchange_book source"
            )
...
def validate_order_lifecycle_journal_v2_payload(
    ...
    queue_source = _require_id(
        "simulator queue source",
        payload["simulator_queue_source"],
    )
    if bool(payload["exact_queue_path_valid"]) and queue_source != "native_exchange_book":
        raise ValueError(
            "exact queue path validity requires native_exchange_book source"
        )
    _finite_nonnegative(
        ...
        *,
        ...
        for ordinal, event in enumerate(unseen, start=1):
            ...
            row = OrderLifecycleJournalV2Row(
                ...
                simulator_queue_source=str(callback.simulator_queue_source),
                exact_queue_path_valid=bool(callback.exact_queue_path_valid),
                quantity_time_exposure_visible_btc_s=float(
                    event["quantity_time_exposure_visible_btc_s"]
                ),
