...
Market identity is always `venue:instrument:symbol`; feeds with the same display symbol are not merged before an explicit consensus or conversion step. The table describes acquisition and transport adapters, not separate supplier-labelled research datasets or a claim that every optional source is enabled in production. Compatible observations enter the same daily channel contract after verified conversion. The tracked public template keeps `multi_market`, all external venues, and the independent deep book disabled.
...
| Historical archive | Binance Vision | USD-M `BTCUSDC` / `BTCUSDT`; configured spot symbols | USD-M individual `trades` and `metrics`; aggregates for the retained reference channels | Individual executions for the daily trade union and derived bars/flow; **not historical L2** | No | On-demand downloader; BTCUSDC perpetual native aggregates are retired in favor of locally generated flow groups |
| Historical archive | CryptoHFTData | Binance Futures `BTCUSDC` execution market | Authenticated hourly price-level snapshot/delta input | Observations for the unified daily book and derived BBO/L2; missing or aged intervals remain explicit | No | On-demand acquisition; current conversion and update limitations are documented in the [daily-book contract](data/README.md#one-fused-daily-orderbook) |
| Historical archive | Tardis delivery | Binance Futures `BTCUSDC` execution market | Daily `incremental_book_L2` and native `book_ticker` input | Compatible observations for the same daily book, not a separate class of research days | No | Import adapter; conversion, clock and reconstruction checks still apply, and acquisition alone does not establish economic support |
...
Only Binance USD-M `BTCUSDC` is an execution market. All reference connectors are read-only, and the public configuration does not turn a reference feed into a quote policy. Trade and book observations remain different channels: individual executions can produce bars and flow groups, but cannot reconstruct unobserved quotes or queue changes. A source union requires matching market/contract semantics and reliable event identities, not timestamps alone. Historical BTCUSDT bridge construction uses official individual-trade bars with right-edge visibility and bounded freshness; it does not require a second order-book archive. Live BTCUSDT continues to use book ticker.
...
Raw observations live under `NARROWGATE_RAW_DATA_ROOT`; generated products live under `NARROWGATE_DATA_ROOT`. Private evidence is separately located by `NARROWGATE_PRIVATE_EVIDENCE_ROOT`. Disposable replay caches remain under `NARROWGATE_CACHE_ROOT`. The explicit `NARROWGATE_CACHE_ROOT` override wins; otherwise the portable default is `$XDG_CACHE_HOME/NarrowGate_BTCUSDC`, falling back to `$HOME/.cache/NarrowGate_BTCUSDC` when `XDG_CACHE_HOME` is unset. Public instructions must not assume a macOS-only `~/Library/Caches` path.

Raw and generated data live in these two real directories, without supplier aliases or compatibility symlinks. Raw daily containers use one name and one physical contract per channel across dates: identical field names, column order, types, units and meanings. Books, trades and funding are different channels, not one artificial shared schema:

- `raw/binance_futures/BTCUSDC/YYYY-MM-DD/{incremental_book_L2,trades,funding}.parquet`
- `raw/binance_futures/BTCUSDT/YYYY-MM-DD/{trades,aggTrades}.parquet` (reference channels)

Generated products remain under `derived/`, for example:

- `binance_futures/BTCUSDC/YYYY-MM-DD/trade_aggregates_100ms.parquet` (synthetic price/side flow groups, not native exchange messages)
- `bars_1s/BTCUSDC-1s-YYYY-MM-DD.parquet` (built directly from union individual executions)
...
The maintained BTCUSDC dataset calendar is 2025-08-01 through 2026-09-05 UTC: all 401 dates belong to the same current dataset. New research predeclares a continuous interval instead of selecting isolated good days by coverage, activity or outcome. Keep gaps and blocked dates in that denominator. Repair with compatible real observations where available; otherwise a derived view may causally carry the last valid state with its original observation clock and increasing age, or remain unknown when no prior state exists. This must not fabricate volume, fills, fresh quotes or queue progress, or relax execution freshness/risk limits.

See the [current daily-data contract](data/README.md#one-fused-daily-orderbook) and [path conventions](docs/path_conventions.md#market-data-tree) for the maintained format and layout. The 401-day calendar and common schema are not a claim that every physical file has finished conversion or validation; use the current operation's verified per-day status. Data readability, supported replay mechanisms and economic evidence are separate from previous-use, Development, Validation and holdout permissions. Historical source-specific registries and experiments retain their original meaning; they are not the default date-selection rule for new research. Do not glob historical top-level `bbo/` or `l2/` migration roots.
