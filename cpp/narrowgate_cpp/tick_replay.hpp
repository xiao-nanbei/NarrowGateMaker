...
struct TickReplayParams {
    ...
    // Output rows may carry an older observation. Empty is legacy/unknown;
    // these clocks never replace the matching/output timestamp axis.
    std::vector<std::int64_t> bbo_observation_ts_us;
    std::vector<std::int64_t> l2_observation_ts_us;
...
struct TraceMarkoutDiagnostic {
    double value = std::numeric_limits<double>::quiet_NaN();
    std::int64_t requested_end_ms = 0;
    std::optional<std::int64_t> actual_end_ms;
    std::optional<std::int64_t> gap_ms;
    std::string status = "censored";
    std::string censor_reason;
    bool fixed_horizon_valid = false;
};
...
struct TraceFillRow {
    ...
    double ev_20s = 0.0;
    double ev_30s = 0.0;
    std::optional<bool> toxic_1s;
    std::optional<bool> toxic_5s;
    std::optional<bool> toxic_20s;
    std::optional<bool> toxic_30s;
    std::array<TraceMarkoutDiagnostic, 4> markout_diagnostics;
    TraceOrderRow order;
};
