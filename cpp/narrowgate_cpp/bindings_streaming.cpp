...
CArray<std::int64_t> checked_trade_counts(const py::array& counts) {
    if (counts.ndim() != 1 || !counts.dtype().is(py::dtype::of<std::int64_t>())) {
        throw std::invalid_argument("trade_counts must be a one-dimensional int64 array");
    }
    return CArray<std::int64_t>(counts);
}

CArray<std::uint8_t> checked_trade_sides(const py::array& sides) {
    if (sides.ndim() != 1 ||
        (!sides.dtype().is(py::dtype::of<std::uint8_t>()) &&
         !sides.dtype().is(py::dtype::of<bool>()))) {
        throw std::invalid_argument("is_buyer_maker must be a one-dimensional bool or uint8 array");
    }
    return CArray<std::uint8_t>(sides);
}

void check_weighted_trade_dimensions(
    const CArray<std::int64_t>& ts_ms,
    const CArray<double>& prices,
    const CArray<double>& quantities
) {
    if (ts_ms.ndim() != 1 || prices.ndim() != 1 || quantities.ndim() != 1) {
        throw std::invalid_argument("weighted trade batch arrays must be one-dimensional");
    }
}
...
void bind_streaming_features(py::module_& m) {
    ...
    py::class_<TradeBarAggregator>(m, "TradeBarAggregator")
        ...
        .def(
            "update_weighted_batch",
            [](
                TradeBarAggregator& aggregator,
                CArray<std::int64_t> ts_ms,
                CArray<double> prices,
                CArray<double> quantities,
                py::array is_buyer_maker,
                py::array trade_counts
            ) {
                check_weighted_trade_dimensions(ts_ms, prices, quantities);
                const auto sides = checked_trade_sides(is_buyer_maker);
                const auto counts = checked_trade_counts(trade_counts);
                std::vector<Bar1s> completed;
                {
                    py::gil_scoped_release release;
                    completed = aggregator.update_weighted_batch(
                        view_from_array(ts_ms), view_from_array(prices),
                        view_from_array(quantities), view_from_array(sides),
                        view_from_array(counts)
                    );
                }
                return completed;
            },
            py::arg("ts_ms"), py::arg("prices"), py::arg("quantities"),
            py::arg("is_buyer_maker"), py::arg("trade_counts")
        )
    ...
    py::class_<SignalRefPerpFeatureEngine>(m, "SignalRefPerpFeatureEngine")
        ...
        .def(
            ...
            [](
                ...
                const auto counts = checked_trade_counts(trade_counts);
                py::gil_scoped_release release;
                engine.update_trade_weighted_batch(
                    view_from_array(ts_ms), view_from_array(prices),
                    view_from_array(quantities), view_from_array(sides),
                    view_from_array(counts)
                );
