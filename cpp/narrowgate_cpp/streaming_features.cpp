...
void TradeBarAggregator::apply_trade(
    ...
    bar.volume += qty;
    bar.trade_count += static_cast<double>(trade_count);
    ...
    if (is_buyer_maker) {
        bar.sell_volume += qty;
        bar.sell_count += static_cast<double>(trade_count);
        ...
        bar.buy_volume += qty;
        bar.buy_count += static_cast<double>(trade_count);
...
std::vector<Bar1s> TradeBarAggregator::update_weighted_batch(
    ArrayView<std::int64_t> ts_ms,
    ArrayView<double> prices,
    ArrayView<double> quantities,
    ArrayView<std::uint8_t> is_buyer_maker,
    ArrayView<std::int64_t> trade_counts
) {
    const std::size_t count = ts_ms.size();
    if (prices.size() != count || quantities.size() != count ||
        is_buyer_maker.size() != count || trade_counts.size() != count) {
        throw std::invalid_argument("weighted trade batch arrays must have equal length");
    }

    // Bar counts are exposed as doubles. Check the entire batch, including
    // accumulated current-bar/run counts, before mutating any state.
    constexpr std::int64_t max_exact_count = std::int64_t{1} << 53;
    auto previous_ts = last_trade_ts_ms_;
    auto bucket = current_bucket_ms_;
    double bucket_count = has_current_ ? current_.trade_count : 0.0;
    auto run_len = last_trade_run_len_;
    auto side = last_trade_side_;
    for (std::size_t index = 0; index < count; ++index) {
        const auto ts = ts_ms[index];
        const auto weight = trade_counts[index];
        if (ts <= 0 || ts > std::numeric_limits<std::int64_t>::max() - 1000 ||
            ts < previous_ts) {
            throw std::invalid_argument("weighted trade timestamps must be positive and chronological");
        }
        if (!std::isfinite(prices[index]) || prices[index] <= 0.0 ||
            !std::isfinite(quantities[index]) || quantities[index] <= 0.0 ||
            !std::isfinite(prices[index] * quantities[index]) ||
            is_buyer_maker[index] > 1) {
            throw std::invalid_argument("weighted trade price/quantity/side is invalid");
        }
        if (weight <= 0 || weight > max_exact_count) {
            throw std::invalid_argument("trade_counts must be positive and at most 2**53");
        }
        const auto next_bucket = (ts / 1000) * 1000;
        if (next_bucket != bucket) {
            bucket = next_bucket;
            bucket_count = 0.0;
        }
        if (bucket_count > static_cast<double>(max_exact_count - weight)) {
            throw std::invalid_argument("weighted trade bar count exceeds exact double range");
        }
        bucket_count += static_cast<double>(weight);
        if (track_runs_) {
            const auto next_side = is_buyer_maker[index] ? -1 : 1;
            if (next_side != side) {
                side = next_side;
                run_len = 0;
            }
            if (run_len > max_exact_count - weight) {
                throw std::invalid_argument("weighted trade run count exceeds exact double range");
            }
            run_len += weight;
        }
        previous_ts = ts;
    ...
    completed.reserve(std::min<std::size_t>(count, 16));
    for (std::size_t index = 0; index < count; ++index) {
        update_one(ts_ms[index], prices[index], quantities[index],
                   is_buyer_maker[index] != 0, completed, trade_counts[index]);
    }
    return completed;
}
...
void TradeBarAggregator::update_one(
    ...
    std::int64_t run_len = 1;
    if (track_runs_) {
        if (side == last_trade_side_) {
            last_trade_run_len_ += trade_count;
        } else {
    ...
    apply_trade(current_, price, qty, is_buyer_maker, run_len, trade_count);
    last_trade_ts_ms_ = ts_ms;
}
...
void SignalRefPerpFeatureEngine::update_trade_weighted_batch(
    ArrayView<std::int64_t> ts_ms,
    ArrayView<double> prices,
    ArrayView<double> quantities,
    ArrayView<std::uint8_t> is_buyer_maker,
    ...
    std::lock_guard lock(mutex_);
    const auto completed = trade_aggregator_.update_weighted_batch(
        ts_ms, prices, quantities, is_buyer_maker, trade_counts
    );
    for (const auto& bar : completed) {
        bars_.push_back(bar);
    }
