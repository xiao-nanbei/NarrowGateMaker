...
TraceMarkoutDiagnostic markout_at_horizon(
    const TickReplayInput& input,
    std::int64_t fill_ts,
    ...
    TraceMarkoutDiagnostic observation;
    const std::int64_t target = fill_ts + horizon_ms;
    observation.requested_end_ms = target;
    auto it = std::lower_bound(input.trade_ts_ms.begin(), input.trade_ts_ms.end(), target);
    if (it == input.trade_ts_ms.end()) {
        observation.censor_reason = "no_future_trade";
        return observation;
    }
    const auto index = static_cast<std::size_t>(it - input.trade_ts_ms.begin());
    const double future = input.trade_price[index];
    observation.actual_end_ms = *it;
    observation.gap_ms = *it - target;
    if (!std::isfinite(future) || !std::isfinite(quote_price)) {
        observation.status = "missing";
        observation.censor_reason = "nonfinite_price";
        return observation;
    }
    observation.value = side == Side::Buy ? future - quote_price : quote_price - future;
    observation.fixed_horizon_valid = *it == target;
    observation.status = observation.fixed_horizon_valid ? "exact" : "delayed";
    return observation;
}
...
void append_fill_trace(
    ...
    constexpr std::array<std::int64_t, 4> horizons{1'000, 5'000, 20'000, 30'000};
    for (std::size_t i = 0; i < horizons.size(); ++i) {
        row.markout_diagnostics[i] = markout_at_horizon(
            input, fill_ts, horizons[i], order.price, order.side);
    }
    row.markout_1s = row.markout_diagnostics[0].value;
    row.markout_5s = row.markout_diagnostics[1].value;
    row.markout_20s = row.markout_diagnostics[2].value;
    row.markout_30s = row.markout_diagnostics[3].value;
    ...
    auto toxicity = [](double value) -> std::optional<bool> {
        return std::isfinite(value) ? std::optional<bool>(value < 0.0) : std::nullopt;
    };
    row.toxic_1s = toxicity(row.markout_1s);
    row.toxic_5s = toxicity(row.markout_5s);
    row.toxic_20s = toxicity(row.markout_20s);
    row.toxic_30s = toxicity(row.markout_30s);
...
BookSnapshot book_snapshot_at(
    ...
    bool has_age = false;
    double best_age_ms = std::numeric_limits<double>::infinity();
    const std::int64_t age_ts = age_clock_ts >= 0 ? age_clock_ts : ts;
    const auto age = [age_ts](const auto& observations, const auto& output, std::size_t i) {
        const auto observed_us = observations.empty() || observations[i] == -1
            ? output[i] * 1000 : observations[i];
        return std::max(0.0, static_cast<double>(age_ts * 1000 - observed_us) / 1000.0);
    };
    ...
    if (!params.use_bar_pricing && input.bbo_ts_ms.size() > 0) {
        if (bbo_idx < input.bbo_ts_ms.size() && input.bbo_ts_ms[bbo_idx] <= ts) {
            ...
            best_age_ms = std::min(best_age_ms, age(params.bbo_observation_ts_us, input.bbo_ts_ms, i));
            has_age = true;
        } else {
            has_age = false;
        }
    ...
    if (!params.use_bar_pricing && input.l2_ts_ms.size() > 0 && !input.l2_bid_px.empty()) {
        if (l2_idx < input.l2_ts_ms.size() && input.l2_ts_ms[l2_idx] <= ts) {
            ...
            best_age_ms = std::min(best_age_ms, age(params.l2_observation_ts_us, input.l2_ts_ms, i));
            has_age = true;
        }
    }
...
TickReplayResult simulate_tick_arrays(
    ...
    input.validate();
    const auto validate_observations = [](const auto& observed, const auto& output) {
        if (observed.empty()) return;
        if (observed.size() != output.size()) {
            throw std::invalid_argument("book observation timestamps must align with output rows");
        }
        std::int64_t previous = 0;
        for (std::size_t i = 0; i < observed.size(); ++i) {
            if (observed[i] == -1) continue; // explicit unknown legacy row
            if (observed[i] <= 0 || observed[i] > output[i] * 1000 || observed[i] < previous) {
                throw std::invalid_argument("book observation timestamp is invalid, future, or regressing");
            }
            previous = observed[i];
        }
    };
    validate_observations(params.bbo_observation_ts_us, input.bbo_ts_ms);
    validate_observations(params.l2_observation_ts_us, input.l2_ts_ms);
