...
The downloader preserves the delivery hierarchy only in ingestion staging;
research readers use supplier-neutral daily raw containers. The legacy planner
resolves remote sizes up front. Private --delivery-config --archive-only runs
...
OKX pipelines; Tardis remains a one-off historical delivery adapter.
"""
...
def _publish_downloaded_book(row: Mapping[str, Any], *, keep_archive: bool,
    ...
    from data.download_cryptohft_orderbook import _crypto_boundary_context
    import pyarrow.parquet as pq
