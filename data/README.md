...
`build-calendar --reuse-build-identity <frozen-parser-digest>` explicitly selects the producer identity of existing days when reusing a historical calendar. It does not authorize a new parser to claim that identity: newly missing days use the current build, and each day records its actual source build. Without this selection, the current build identity must match. This command resumes a working calendar and updates its coordinator manifest; do not run it against a frozen calendar merely to relocate raw files. `validate-calendar --raw-root <current-archive-root>` is read-only for the frozen calendar. Neither command changes a frozen daily bundle or consumer manifest. Location checks do not assert equivalence between different parsers.
