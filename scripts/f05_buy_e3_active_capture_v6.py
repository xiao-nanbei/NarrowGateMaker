...
SCHEMA_VERSION: Final = f"{OWNER}.fresh_external_venues_disabled_active_process_capture.v4"
STATUS: Final = "fresh_external_venues_disabled_rate_window_active_process_captured"
...
CHECKS: Final = {
    ...
    "resource_v6_semantic_exact": True,
...
def _require_sha256(value: Any, label: str) -> str:
    try:
        return resource_v6._require_sha256(value, label)  # noqa: SLF001
...
def _content_binding(
    ...
    *,
    ...
    if (
        ...
        or canonical != resource_v6.document_sha256(opened.payload, canonical_field)
    ):
...
def _validate_runtime_repository(root: Path) -> tuple[Path, dict[str, Any]]:
    try:
        ...
        execution = resource_v6.capture_git_execution(
            repository,
...
def _validate_release(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    try:
        resource_v6._validate_direct_release_payload(opened.payload)  # noqa: SLF001
    except Exception as exc:
...
def _validate_resource(
    ...
    *,
    ...
    try:
        validated = resource_v6.validate_resource_receipt(
            path,
    ...
    binding = _content_binding(
        opened,
        canonical_field=resource_v6.RESOURCE_CANONICAL_FIELD,
        expected_schema=resource_v6.RESOURCE_SCHEMA,
        expected_status=resource_v6.RESOURCE_STATUS,
    )
...
def _disabled_process(resource: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if process["config_sha256"] != resource_v6.EXPECTED_DISABLED_CONFIG_SHA256 or any(
...
def _capture_active_runtime_sources(
    ...
    for role, frozen in resource_v6.CURRENT_V4_RUNTIME_SOURCE_SHA256.items():
        ...
        if (
            ...
            or resource_v6.file_sha256(candidate) != expected
        ...
        rows[role] = {
            ...
            "resource_v6_binding_matches": True,
        }
    ...
    return {
        "execution_commit": DIRECT_V4_EXECUTION_COMMIT,
        "files": rows,
        "runtime_source_manifest_sha256": resource_v6.canonical_sha256(rows),
...
def _startup_source_plan() -> dict[str, Any]:
    ...
    for role in STARTUP_SOURCE_ROLES:
        frozen = resource_v6.CURRENT_V4_RUNTIME_SOURCE_SHA256[role]
    ...
    return {
        "files": files,
        "runtime_code_sha256": resource_v6.canonical_sha256(files),
    }
...
def _runtime_semantics(
    ...
    *,
    ...
    return {
        "startup_attestation_sha256": resource_v6.canonical_sha256(startup),
...
def build_active_capture(
    *,
    ...
    if resource_v6.file_sha256(config) != ACTIVE_CONFIG_SHA256:
        raise ActiveCaptureV5Error("active config bytes drifted")
    config_payload = resource_v6._load_yaml(config)  # noqa: SLF001
    external = resource_v6._mapping(  # noqa: SLF001
    ...
    process_row["canonical_process_identity_sha256"] = resource_v6.document_sha256(
    ...
    payload[CANONICAL_FIELD] = resource_v6.document_sha256(payload, CANONICAL_FIELD)
    return payload
...
def validate_active_capture(
    ...
    *,
    ...
    if process_canonical != resource_v6.canonical_sha256(process_body):
    ...
    if (
        ...
        or payload.get(CANONICAL_FIELD) != resource_v6.document_sha256(payload, CANONICAL_FIELD)
    ):
...
def finalize_active_capture(
    *,
    ...
    try:
        file_sha = resource_v6.atomic_write_receipt(output_path, payload)
    except Exception as exc:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "capture":
        ...
        file_sha = resource_v6.file_sha256(args.receipt)
    print(
        json.dumps(
            {
