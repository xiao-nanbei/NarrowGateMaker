...
def _site_packages_directory(prefix: Path, interpreter: dict[str, Any]) -> Path:
    ...
    if len(real_candidates) != 1:
        raise LockedRuntimeError(
            "installed tree requires exactly one real site-packages directory"
        )
    return real_candidates[0]


def _installed_tree_snapshot(
    prefix_path: Path, *, interpreter: dict[str, Any]
) -> dict[str, Any]:
    ...
    for current, directories, files in os.walk(site_packages, followlinks=False):
        ...
        for filename in files:
            ...
            if child.suffix == ".pyc" or "__pycache__" in PurePosixPath(child_relative).parts:
                raise LockedRuntimeError(
                    f"installed bytecode is forbidden: {child_relative}"
                )
    ...
    return {
        ...
        "record_aggregate_sha256": _sha256(
            json.dumps(aggregate_payload, sort_keys=True, separators=(",", ":")).encode(
                "utf-8"
            )
        ),
        "site_packages_relative_path": site_relative,
...
def generate_lock(
    *,
    ...
    return {
        "lock": payload,
        "lock_file_sha256": _sha256(_canonical_json_bytes(payload)),
...
def _wheelhouse_payload(
    *, lock: dict[str, Any], lock_raw: bytes, artifacts: Sequence[dict[str, Any]]
) -> dict[str, Any]:
    wheels = []
    ...
    payload: dict[str, Any] = {
        ...
        "lock_authority": {
            ...
            "lock_file_sha256": _sha256(lock_raw),
        },
...
def _publish_wheelhouse(
    *,
    ...
    lock, lock_raw = load_lock(lock_path, expected_canonical_sha256=expected_lock_sha256)
    ...
    return {
        "manifest": payload,
        "manifest_file_sha256": _sha256(_canonical_json_bytes(payload)),
...
def _validate_static_snapshot_against_receipt(
    *, target: Path, receipt: dict[str, Any]
    ...
    interpreter = _validate_interpreter_shape(
        receipt.get("interpreter"), "static installed tree"
    )
    snapshot = _installed_tree_snapshot(target, interpreter=interpreter)
    expected_versions = {
        row["name"]: row["version"] for row in receipt["installed_distributions"]
    }
    ...
    if receipt["installed_record_aggregate_sha256"] != snapshot[
...
def install_locked_runtime(
    *,
    ...
    try:
        ...
        static_installed = _installed_tree_snapshot(
            target, interpreter=target_before
        )
        ...
        if installed["record_aggregate_sha256"] != static_installed[
            "record_aggregate_sha256"
        ]:
            raise LockedRuntimeError(
                "target interpreter changed the static installed tree"
            )
        ...
        payload: dict[str, Any] = {
            ...
            "wheelhouse_authority": {
                ...
                "manifest_file_sha256": _sha256(_canonical_json_bytes(manifest)),
            },
            "explicit_wheels": {
            ...
            "installed_record_aggregate_sha256": static_installed[
                "record_aggregate_sha256"
            ],
            "pip_check": {
                "passed": True,
        ...
        return {
            "receipt": payload,
            "receipt_file_sha256": _sha256(_canonical_json_bytes(payload)),
...
def validate_startup_runtime(
    *,
    ...
    _validate_static_snapshot_against_receipt(
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    startup = subparsers.add_parser(
        "verify-startup", help="verify live runtime from frozen receipt hashes"
    )
    startup.add_argument("--venv-python", type=Path, required=True)
    startup.add_argument("--pip-runner-python", type=Path)
    startup.add_argument("--receipt", type=Path, required=True)
    startup.add_argument("--expected-receipt-sha256", required=True)
    startup.add_argument("--expected-lock-sha256", required=True)
    startup.add_argument("--expected-wheelhouse-sha256", required=True)
    startup.add_argument("--expected-root-wheel-sha256", required=True)
    startup.add_argument("--expected-native-wheel-sha256", required=True)
    startup.add_argument("--expected-python-version", required=True)
    startup.add_argument("--expected-soabi", required=True)
    startup.add_argument("--expected-compiler", required=True)
    startup.add_argument("--expected-openssl-runtime", required=True)
    startup.add_argument("--expected-interpreter-executable-sha256", required=True)
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "verify-static-tree":
            ...
            _summary(payload, INSTALL_CANONICAL_FIELD)
            return 0
        if args.command == "verify-startup":
            payload = validate_startup_runtime(
                ...
                expected_lock_sha256=args.expected_lock_sha256,
                expected_wheelhouse_sha256=args.expected_wheelhouse_sha256,
                expected_root_wheel_sha256=args.expected_root_wheel_sha256,
                expected_native_wheel_sha256=args.expected_native_wheel_sha256,
                expected_python_version=args.expected_python_version,
                expected_soabi=args.expected_soabi,
                expected_compiler=args.expected_compiler,
                expected_openssl_runtime=args.expected_openssl_runtime,
                expected_interpreter_executable_sha256=(
                    args.expected_interpreter_executable_sha256
                ),
            )
            _summary(payload, INSTALL_CANONICAL_FIELD)
            return 0
