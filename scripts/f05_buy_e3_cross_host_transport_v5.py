#!/usr/bin/env python3
"""Transport external-venues-disabled BUY E3 evidence across hosts, fail closed.
...
from scripts import f05_buy_e3_active_capture_v5 as active_capture_v5
...
REMOTE_ATTESTATION_SCHEMA: Final = f"{OWNER}.remote_active_attestation.v3"
...
ADMISSION_SCHEMA: Final = f"{OWNER}.cross_host_admission.v2"
...
def _validate_config_correction(path: Path) -> tuple[dict[str, Any], dict[str, Any], bytes]:
    try:
        payload, binding = resource_v5.config_successor.validate_content_receipt(path)
    except Exception as exc:
    ...
    if payload != opened.payload or binding != _content_binding(
        opened,
        canonical_field=resource_v5.config_successor.CANONICAL_FIELD,
        expected_schema=resource_v5.config_successor.SCHEMA_VERSION,
        expected_status=resource_v5.config_successor.STATUS,
    ):
...
def _validate_resource(
    ...
    *,
    ...
    try:
        semantically_validated = resource_v5.validate_resource_receipt(
            path,
...
def _active_runtime_semantics(
    ...
    *,
    ...
    expected_startup_sources = {
        ...
        for role, binding in resource_v5.CURRENT_V4_RUNTIME_SOURCE_SHA256.items()
        if role
        in {
            "live_main",
            "live_config",
...
def build_remote_active_attestation(
    *,
    ...
    observed_host = resource_v5.host_identity(
...
def _validate_source_set(
    *,
    ...
    if resource.get("config_correction") != correction_binding:
        raise CrossHostTransportError("resource/config correction cross-binding drifted")
    try:
        semantically_validated_active = active_capture_v5.validate_active_capture(
            active_path,
    ...
    *,
    ...
    payload = {
        ...
        "checks": {
            ...
            "remote_attestation_v3_semantically_valid": True,
            "active_capture_v3_content_exact": True,
