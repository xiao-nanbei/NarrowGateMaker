#!/usr/bin/env python3
"""Freeze an exact-tag Linux native build after ABI and parity checks."""
...
NATIVE_SOURCES = (
    ...
    "research/families/f03_causal_13_head/cpp/causal_v12_1s_features.hpp",
    "research/families/f06_placement_fill_cif/cpp/request_state_features.cpp",
    "research/families/f06_placement_fill_cif/cpp/request_state_features.hpp",
    "research/families/f06_placement_fill_cif/cpp/risk_set_expansion.cpp",
    "research/families/f06_placement_fill_cif/cpp/risk_set_expansion.hpp",
    "research/families/f06_placement_fill_cif/cpp/sparse_order_lifecycle.cpp",
    "research/families/f06_placement_fill_cif/cpp/sparse_order_lifecycle.hpp",
    "research/families/f07_active_order_continuation/cpp/active_order_competing_risk_cif.cpp",
    "research/families/f07_active_order_continuation/cpp/active_order_competing_risk_cif.hpp",
    ...
    *,
    ...
    try:
        ...
        raise NativeBuildReceiptError(
            f"locked Python runtime validation failed: {exc}"
        ) from exc
    lock = _file(runtime_lock_path)
    manifest = _file(
        dependency_wheelhouse_path / locked_runtime.WHEELHOUSE_MANIFEST
    )
    ...
    dependency_lock = {
        ...
        "runtime_lock_file_sha256": lock["sha256"],
        "runtime_lock_canonical_sha256": runtime_lock_sha256,
        "wheelhouse_path": str(
            dependency_wheelhouse_path.expanduser().resolve(strict=True)
        ),
        "wheelhouse_manifest_path": manifest["path"],
        "wheelhouse_manifest_file_sha256": manifest["sha256"],
        "wheelhouse_canonical_sha256": dependency_wheelhouse_sha256,
    }
    installed_distribution_lock = {
        "install_receipt_path": install["path"],
        "install_receipt_file_sha256": install["sha256"],
        "install_receipt_canonical_sha256": install_receipt_sha256,
        ...
        "installed_record_aggregate_sha256": installed[
    ...
    *,
    ...
    with zipfile.ZipFile(wheel) as archive:
        ...
        if len(extension_rows) != 1 or _sha(archive.read(extension_rows[0])) != _sha(
            ...
            raise NativeBuildReceiptError(
                "installed native module does not match the frozen wheel"
            )
    required = (
    ...
    for relative in NATIVE_SOURCES:
        ...
        if working != head:
            raise NativeBuildReceiptError(f"native source differs from HEAD: {relative}")
        sources[relative] = {"sha256": _sha(working), "size_bytes": len(working)}
    try:
        import pybind11
