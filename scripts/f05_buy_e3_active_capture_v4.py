#!/usr/bin/env python3
"""Capture the fresh active BUY E3 direct-v4 process without adding authority.

This additive receipt succeeds the historical direct-v3 active-capture shape.
...
SCHEMA_VERSION: Final = f"{OWNER}.fresh_direct_v4_active_process_capture.v2"
STATUS: Final = "fresh_v4_active_process_captured"
...
ACTIVE_CONFIG_SHA256: Final = "2f61532126cbe633424476cb093c6c978bab1f935f69a30e06677d677008cae6"
...
CHECKS: Final = {
    ...
    "resource_v4_semantic_exact": True,
...
class ActiveCaptureV4Error(RuntimeError):
...
def _reject_duplicate_keys(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in output:
            raise ActiveCaptureV4Error(f"duplicate JSON key: {key}")
        output[key] = value
    return output
...
def _open_private_json(path: Path, label: str) -> OpenedJson:
    ...
    if candidate.is_symlink() or not candidate.is_file():
        raise ActiveCaptureV4Error(f"{label} is not a regular file")
    ...
    if (
        ...
        raise ActiveCaptureV4Error(f"{label} has unsafe identity or permissions")
    try:
        ...
        payload = json.loads(
            ...
            parse_constant=lambda value: (_ for _ in ()).throw(
                ActiveCaptureV4Error(f"non-finite JSON value: {value}")
            ),
        )
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise ActiveCaptureV4Error(f"{label} is unreadable JSON") from exc
    after = target.stat()
    if (
        before.st_dev,
        before.st_ino,
        ...
        raise ActiveCaptureV4Error(f"{label} changed while read")
    if not isinstance(payload, dict):
        raise ActiveCaptureV4Error(f"{label} root is not an object")
    return OpenedJson(target, payload, raw, before)


def _require_sha256(value: Any, label: str) -> str:
    try:
        return resource_v4._require_sha256(value, label)  # noqa: SLF001
    except Exception as exc:
        raise ActiveCaptureV4Error(f"{label} is not SHA256") from exc


def _strict_positive_int(value: Any, label: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
        raise ActiveCaptureV4Error(f"{label} is not a positive integer")
    return value
...
def _timestamp(value: Any, label: str) -> str:
    ...
    if not normalized.endswith("Z"):
        raise ActiveCaptureV4Error(f"{label} is not UTC")
    try:
        observed = datetime.fromisoformat(normalized.removesuffix("Z") + "+00:00")
    except ValueError as exc:
        raise ActiveCaptureV4Error(f"{label} is invalid") from exc
...
def _content_binding(
    ...
    *,
    ...
    if (
        ...
        or canonical != resource_v4.document_sha256(opened.payload, canonical_field)
    ):
        raise ActiveCaptureV4Error("content binding semantic identity drifted")
    return {
...
def _validate_runtime_repository(root: Path) -> tuple[Path, dict[str, Any]]:
    try:
        ...
        execution = resource_v4.capture_git_execution(
            repository,
            annotated_tag=DIRECT_V4_ANNOTATED_TAG,
            runtime_authority=True,
        )
    except Exception as exc:
        raise ActiveCaptureV4Error("active runtime checkout is not exact clean direct-v4") from exc
    expected = {
    ...
    if any(execution.get(field) != value for field, value in expected.items()):
        raise ActiveCaptureV4Error("active runtime Git identity drifted")
...
def _validate_release(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    try:
        resource_v4._validate_direct_release_payload(opened.payload)  # noqa: SLF001
    except Exception as exc:
        raise ActiveCaptureV4Error("direct-v4 release-v2 semantics drifted") from exc
    if (
        binding["file_sha256"] != DIRECT_V4_RELEASE_FILE_SHA256
        or binding["canonical_sha256"] != DIRECT_V4_RELEASE_CANONICAL_SHA256
    ):
        raise ActiveCaptureV4Error("direct-v4 release-v2 byte identity drifted")
    return dict(opened.payload), binding


def _validate_resource(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    try:
        validated = resource_v4.validate_resource_receipt(path)
    except Exception as exc:
        raise ActiveCaptureV4Error("resource-v4 receipt is invalid") from exc
    opened = _open_private_json(path, "resource-v4 receipt")
    binding = _content_binding(
        opened,
        canonical_field=resource_v4.RESOURCE_CANONICAL_FIELD,
        expected_schema=resource_v4.RESOURCE_SCHEMA,
        expected_status=resource_v4.RESOURCE_STATUS,
    )
    ...
    if validated != opened.payload or not all(
        ...
        raise ActiveCaptureV4Error("resource-v4 receipt changed or is incomplete")
    if (
        ...
        raise ActiveCaptureV4Error("resource-v4 authority semantics drifted")
...
def _disabled_process(resource: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(raw, Mapping):
        raise ActiveCaptureV4Error("resource-v4 lacks its disabled process")
    process = {
    ...
    if process["config_sha256"] != resource_v4.EXPECTED_DISABLED_CONFIG_SHA256 or any(
        process[name] is not True
        for name in ("fresh_pid", "fresh_start_ticks", "same_pid_pre_post")
    ):
        raise ActiveCaptureV4Error("resource-v4 disabled process semantics drifted")
    return process
...
def _predecessor_is_quiescent(pid: int, *, proc_root: Path) -> bool:
    ...
    if not root.is_dir():
        raise ActiveCaptureV4Error("active capture requires a process filesystem")
...
def _read_pid(path: Path) -> int:
    ...
    try:
        ...
        raise ActiveCaptureV4Error("active PID file is invalid") from exc
...
def _capture_active_runtime_sources(
    ...
    if not isinstance(resource_files, Mapping):
        raise ActiveCaptureV4Error("resource-v4 runtime source map is missing")
    rows: dict[str, dict[str, Any]] = {}
    for role, frozen in resource_v4.CURRENT_V4_RUNTIME_SOURCE_SHA256.items():
        ...
        if (
            ...
            or resource_v4.file_sha256(candidate) != expected
            or _git_blob_sha256(repository, relative) != expected
        ):
            raise ActiveCaptureV4Error(f"active runtime source drifted: {role}")
        rows[role] = {
            "role": role,
            ...
            "resource_v4_binding_matches": True,
        }
    if not REQUIRED_ACTIVE_SOURCE_ROLES.issubset(rows):
        raise ActiveCaptureV4Error("BUY and lifecycle source roles are incomplete")
    return {
        "execution_commit": DIRECT_V4_EXECUTION_COMMIT,
        "files": rows,
        "runtime_source_manifest_sha256": resource_v4.canonical_sha256(rows),
...
def _startup_source_plan() -> dict[str, Any]:
    ...
    for role in STARTUP_SOURCE_ROLES:
        frozen = resource_v4.CURRENT_V4_RUNTIME_SOURCE_SHA256[role]
    ...
    return {
        "files": files,
        "runtime_code_sha256": resource_v4.canonical_sha256(files),
    }
...
def _runtime_semantics(
    ...
    *,
    ...
    if not PurePosixPath(runtime_release_path).is_absolute() or (
        ...
        raise ActiveCaptureV4Error("active runtime release path drifted")
    try:
        ...
        raise ActiveCaptureV4Error("active runtime/startup attestation is not exact v4") from exc
    ...
    if (
        ...
        raise ActiveCaptureV4Error("active runtime authority, artifact, or shadow state drifted")
    state = startup.get("fill_cooldown_state")
    if not isinstance(state, Mapping):
        raise ActiveCaptureV4Error("active startup cooldown state is missing")
    ...
    if buy_identity not in {"B0", f"BUY_E3:{EXACT_ARTIFACT_SHA256}"} or (
        ...
        raise ActiveCaptureV4Error("active startup imported an unsafe BUY deadline")
    return {
        "startup_attestation_sha256": resource_v4.canonical_sha256(startup),
...
def _capture_process(
    *,
    ...
    try:
        ...
        raise ActiveCaptureV4Error("actual active process identity capture failed") from exc
...
def build_active_capture(
    *,
    ...
    if not _predecessor_is_quiescent(disabled_pid, proc_root=proc_root):
        raise ActiveCaptureV4Error("disabled predecessor PID is still running")
    config = config_path.expanduser().resolve(strict=True)
    if resource_v4.file_sha256(config) != ACTIVE_CONFIG_SHA256:
        raise ActiveCaptureV4Error("active config bytes drifted")
    active_pid = _read_pid(pid_file)
    if active_pid == disabled_pid:
        raise ActiveCaptureV4Error("active restart reused the disabled PID")
    ...
    if active_start <= int(disabled["pid_start_ticks"]):
        raise ActiveCaptureV4Error("active process did not start after its disabled predecessor")
    ...
    if (
        ...
        raise ActiveCaptureV4Error("actual process and runtime identity bytes differ")
    ...
    if _stable_process_projection(recaptured) != _stable_process_projection(process):
        raise ActiveCaptureV4Error("active process changed during capture")
    ...
    process_row["canonical_process_identity_sha256"] = resource_v4.document_sha256(
    ...
    if _utc_datetime(timestamp, "active capture timestamp") < captured_at:
        raise ActiveCaptureV4Error("active capture receipt predates its process observation")
    payload: dict[str, Any] = {
    ...
    payload[CANONICAL_FIELD] = resource_v4.document_sha256(payload, CANONICAL_FIELD)
    return payload
...
def validate_active_capture(
    ...
    *,
    ...
    if (
        ...
        raise ActiveCaptureV4Error("active-capture-v4 structure or content binding drifted")
    if (
        set(payload["runtime_authority"]) != CONTENT_BINDING_FIELDS
        or set(payload["resource_receipt"]) != CONTENT_BINDING_FIELDS
    ):
        raise ActiveCaptureV4Error("active capture contains non-content authority bindings")
    ...
    if (
        ...
        raise ActiveCaptureV4Error("active process transition or v4 source identity drifted")
    ...
    if process_canonical != resource_v4.canonical_sha256(process_body):
        raise ActiveCaptureV4Error("active process canonical identity drifted")
    ...
    if (
        ...
        raise ActiveCaptureV4Error("active runtime identity file binding drifted")
    semantics = _runtime_semantics(
        runtime,
        process=process,
        release=release,
    ...
    if (
        ...
        or payload.get(CANONICAL_FIELD) != resource_v4.document_sha256(payload, CANONICAL_FIELD)
    ):
        raise ActiveCaptureV4Error("active-capture-v4 semantic identity drifted")
...
def finalize_active_capture(
    *,
    ...
    try:
        file_sha = resource_v4.atomic_write_receipt(output_path, payload)
    except Exception as exc:
        raise ActiveCaptureV4Error("create-only active capture write failed") from exc
    ...
    if observed != payload:
        raise ActiveCaptureV4Error("active capture changed after write")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "capture":
        ...
        file_sha = resource_v4.file_sha256(args.receipt)
    print(
        json.dumps(
            {
