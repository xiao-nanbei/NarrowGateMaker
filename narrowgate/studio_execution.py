...
class Catalog:
    ...
    def candidates(self, contract, requested):
        ...
        # Explicit preferences are the operator's target list. Without one,
        # every registered/allowed target remains eligible, including the Mac.
        if contract["role"] == "training" and not contract["preferred_resources"]:
            candidates = sorted(
                candidates,
...
def report(files):
    from narrowgate.studio_research import from_summaries

    record = json.loads(files["execution.json"])
    environment = json.loads(files["environment.json"])
    research_result, research_issues = from_summaries(json.loads(files["summaries.json"]))
    return {
        "schema_version": "registered_execution_report.v1",
        "classification": CLASSIFICATION,
        "research_result": research_result,
        "research_result_issues": research_issues,
