...
jobs=[]
# Linux ps may truncate command arguments to COLUMNS, hiding the run marker.
# Double-wide output preserves the same full-argument matching on macOS/Linux.
processes=command(['ps','-ww','-axo','pid=,command=']).splitlines() if cfg.get('jobs') else []
