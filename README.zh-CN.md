...
市场身份始终使用 `venue:instrument:symbol`；显示 symbol 相同的 feed，在经过明确的 consensus 或 conversion 步骤之前不会合并。下表描述获取和传输 adapter，不是按供应商划分研究数据集，也不表示所有可选数据源都已在生产中启用。兼容观测经过验证转换后进入同一日级频道合同。受 Git 跟踪的公开模板默认关闭 `multi_market`、所有 external venue，以及独立 deep book。
...
| Historical archive | Binance Vision | USD-M `BTCUSDC` / `BTCUSDT`；指定的 spot symbol | USD-M individual `trades` 和 `metrics`；保留的参考频道聚合成交 | 逐笔成交用于日级并集及派生 bar/成交流；**不是历史 L2** | 不可以 | 按需下载工具；BTCUSDC perpetual 原生聚合已停用，改为本地生成成交流分组 |
| Historical archive | CryptoHFTData | Binance Futures `BTCUSDC` execution market | 需要认证的逐小时 price-level snapshot/delta 输入 | 统一日级盘口及派生 BBO/L2 的观测输入；缺失和陈旧区间保持显式 | 不可以 | 按需获取；当前转换与更新限制见[日级盘口合同](data/README.zh-CN.md#每天只维护一份融合盘口) |
| Historical archive | Tardis delivery | Binance Futures `BTCUSDC` execution market | 每日 `incremental_book_L2` 与原生 `book_ticker` 输入 | 同一日级盘口的兼容观测，不是另一类研究日期 | 不可以 | 导入 adapter；仍须检查转换、时钟与重建，获取完成本身不证明经济支持 |
...
只有 Binance USD-M `BTCUSDC` 是 execution market。所有 reference connector 均为只读；公开配置不会把 reference feed 自动转化为 quote policy。成交与盘口仍是不同频道：逐笔成交可以生成 bar 和成交流分组，但不能还原未观测到的报价或队列变化。补源并集要求相同市场/合约语义和可靠事件身份，不能仅按时间戳合并。历史 BTCUSDT bridge 使用右边界可见、freshness 有上限的官方 individual-trade 1s bar，不依赖另一套 orderbook；live 仍使用 BTCUSDT book ticker。
...
原始观测保存在 `NARROWGATE_RAW_DATA_ROOT`，生成产物保存在 `NARROWGATE_DATA_ROOT`，私有证据另由 `NARROWGATE_PRIVATE_EVIDENCE_ROOT` 定位。可重建 replay cache 保存在 `NARROWGATE_CACHE_ROOT` 下。显式设置的 `NARROWGATE_CACHE_ROOT` 优先；否则使用 `$XDG_CACHE_HOME/NarrowGate_BTCUSDC`，当 `XDG_CACHE_HOME` 未设置时回退到 `$HOME/.cache/NarrowGate_BTCUSDC`。公开说明不得假定 macOS 专用的 `~/Library/Caches` 路径。

原始行情和生成产物存放在这两个真实目录，禁止供应商别名和兼容软链接。原始日容器按频道跨日期统一命名和物理合同：字段名、列顺序、类型、单位与含义全部相同。盘口、成交与资金费是不同频道，不硬拼成同一 schema：

- `raw/binance_futures/BTCUSDC/YYYY-MM-DD/{incremental_book_L2,trades,funding}.parquet`
- `raw/binance_futures/BTCUSDT/YYYY-MM-DD/{trades,aggTrades}.parquet`（参考频道）

生成产物位于 `derived/`，例如：

- `binance_futures/BTCUSDC/YYYY-MM-DD/trade_aggregates_100ms.parquet`（按价格／方向自聚合的成交流，不是交易所原生消息）
- `bars_1s/BTCUSDC-1s-YYYY-MM-DD.parquet`（直接从并集逐笔生成）
...
当前维护的 BTCUSDC 数据集日历为 2025-08-01 至 2026-09-05 UTC，401 个日期都属于同一当前数据集。新研究事先声明连续区间，不按覆盖率、交易活跃度或结果挑选孤立“好日”；缺口和被阻塞的日期仍留在总分母中。有兼容真实观测时先补源，否则派生视图只能因果沿用最后有效状态，保留原观测时钟并增加年龄；没有前值则保持 unknown。不能凭空生成成交量、成交、新鲜报价或队列进度，也不能放松执行新鲜度/风险限制。

维护中的格式和布局见[当前日级数据合同](data/README.zh-CN.md#每天只维护一份融合盘口)与[路径约定](docs/path_conventions.md#market-data-tree)。401 日日历和统一 schema 不表示所有物理文件已完成转换或验收，应以当前任务逐日验证状态为准。数据可读性、支持的回放机制、经济证据，以及 previous-use、Development、Validation、holdout 使用权限必须分开。历史来源登记和实验保留原有含义，但不再是新研究默认的日期筛选规则。不要 glob 历史顶层 `bbo/`、`l2/` 迁移目录。
