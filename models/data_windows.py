...
def _runtime_loader_identity(params: dict[str, Any]) -> dict[str, Any]:
    ...
    return ({} if bounds is None else {
        ...
        "bounded_reader_identity": canonical_sha256([
            _callable_identity(function) for function in (
                bt._parquet_window_rows, bt._read_parquet_rows, bt._read_daily_trade_parquet,
                bt._normalize_individual_trade_frame, bt.load_bbo_data, bt.load_l2_data,
                bt._bounded_l2_arrays,
                bt._load_book_observations, bt.load_book_delivery_prefix,
            )
        ]),
