...
from models.quote_decomposition_tick import _diagnostic_counts, _mean_bool, _nullable_bool, _run_day  # noqa: E402
...
def _add_common_columns(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in ("markout_1s", "markout_5s", "markout_30s", "ev_30s"):
        out[column] = _num_col(out, column, default=np.nan)
        out[f"{column}_qty"] = out[column] * out["fill_qty_btc"]
        out[f"{column}_valid_qty"] = out["fill_qty_btc"].where(out[column].notna())
        if column.startswith("markout_"):
            status = out.get(f"{column}_status", pd.Series("legacy_unclassified", index=out.index))
            for state in ("censored", "delayed", "exact"):
                out[f"{column}_{state}"] = status.eq(state).astype(int)
            out[f"{column}_missing"] = (out[column].isna() & ~status.eq("censored")).astype(int)
    out["toxic_30s"] = _nullable_bool(out.get("toxic_30s", pd.Series(index=out.index, dtype=float)))
    out["positive_30s"] = out["ev_30s"].gt(0).astype(float).where(out["ev_30s"].notna())
...
def _order_evidence(orders: pd.DataFrame, fills: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if not fills.empty and "order_id" in fills:
        ...
        for column in ("markout_1s", "markout_5s", "markout_30s", "ev_30s", "toxic_30s"):
            orders[column] = orders.pop(f"{column}_fill")
    else:
...
def _diagnostic_aggregations() -> dict[str, tuple[str, Any]]:
    result = {}
    for key in ("markout_1s", "markout_5s", "markout_30s", "ev_30s"):
        result[f"{key}_valid_count"] = (key, "count")
        result[f"{key}_valid_qty_btc"] = (f"{key}_valid_qty", lambda s: s.sum(min_count=1))
        if key.startswith("markout_"):
            for state in ("censored", "delayed", "exact", "missing"):
                result[f"{key}_{state}_count"] = (f"{key}_{state}", "sum")
    return result


def _weighted_diagnostics(out: pd.DataFrame) -> None:
    for key in ("markout_1s", "markout_5s", "markout_30s", "ev_30s"):
        denom = out[f"{key}_valid_qty_btc"].replace(0.0, np.nan)
        name = "avg_ev_30s_usdc_per_btc" if key == "ev_30s" else f"avg_{key}"
        out[name] = out.pop(f"{key}_qty_sum") / denom
        out[f"{key}_unknown_count"] = out["fills"] - out[f"{key}_valid_count"]
...
def _fill_evidence(fills: pd.DataFrame, *, min_fills: int) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    def _agg(group_cols: list[str]) -> pd.DataFrame:
        out = (
            ...
            .agg(
                ...
                **_diagnostic_aggregations(),
                ...
                sum_ev_30s_usdc=("ev_30s_usdc", lambda s: s.sum(min_count=1)),
                positive_30s_rate=("positive_30s", "mean"),
                toxic_30s_rate=("toxic_30s", "mean"),
            )
            .reset_index()
        )
        _weighted_diagnostics(out)
        ...
        out["alpha_evidence"] = np.where(
            ...
            "positive_bucket",
            np.where(out["ev_30s_valid_count"].eq(0), "unknown", np.where(out["fills"] >= min_fills, "negative_or_mixed", "sparse")),
        )
...
def _bucket_stability(fill_daily: pd.DataFrame, *, min_fills: int) -> pd.DataFrame:
    ...
    valid_fills = pd.to_numeric(work.get("ev_30s_valid_count", work["fills"]), errors="coerce")
    work["support_day"] = valid_fills.ge(min_fills) & work["avg_ev_30s"].notna()
    work["positive_day"] = work["avg_ev_30s"].gt(0).astype(float).where(work["avg_ev_30s"].notna())
    work["ev_weight"] = valid_fills.where(work["avg_ev_30s"].notna())
    work["pos_weight"] = valid_fills.where(work["positive_30s_rate"].notna())
    work["weighted_ev_part"] = (
        pd.to_numeric(work["avg_ev_30s"], errors="coerce") * work["ev_weight"]
    )
    work["weighted_pos_part"] = (
        pd.to_numeric(work["positive_30s_rate"], errors="coerce") * work["pos_weight"]
    )
    ...
    for keys, group in work.groupby(["side", "bucket_family", "bucket"], dropna=False):
        ...
        pos_support = int((group["support_day"] & group["positive_day"].eq(1)).sum())
        ev_weight = group["ev_weight"].sum()
        pos_weight = group["pos_weight"].sum()
        weighted_ev = float(group["weighted_ev_part"].sum(min_count=1) / ev_weight) if ev_weight > 0 else np.nan
        weighted_pos = float(group["weighted_pos_part"].sum(min_count=1) / pos_weight) if pos_weight > 0 else np.nan
        support_ratio = pos_support / support if support else np.nan
        if pd.isna(weighted_ev):
            verdict = "unknown"
        elif support >= 3 and support_ratio >= 2 / 3 and weighted_ev > 0:
        ...
        rows.append({
            ...
            "bucket": bucket,
            "days": days,
            "unknown_days": int(group["positive_day"].isna().sum()),
    ...
    for day in all_days:
        ...
        rows.append({
            ...
            **_diagnostic_counts(day_fills),
            ...
            "avg_ev_30s": float(day_fills["ev_30s_qty"].sum(min_count=1) / day_fills["ev_30s_valid_qty"].sum()) if not day_fills.empty and day_fills["ev_30s_valid_qty"].sum() > 0 else np.nan,
            "avg_markout_30s": float(day_fills["markout_30s_qty"].sum(min_count=1) / day_fills["markout_30s_valid_qty"].sum()) if not day_fills.empty and day_fills["markout_30s_valid_qty"].sum() > 0 else np.nan,
            "sum_ev_30s_usdc": float(day_fills["ev_30s_usdc"].sum(min_count=1)) if not day_fills.empty else np.nan,
            "toxic_30s_rate": _mean_bool(day_fills.get("toxic_30s", pd.Series(dtype=float))),
