...
from models.tick_data_types import HistoricalBBOData, HistoricalL2Data, book_usable_mask
...
def _stitch_bbo(first: HistoricalBBOData, second: HistoricalBBOData) -> HistoricalBBOData:
    ...
    usable = (None if first.usable is None and second.usable is None else
              _append_arrays((first.ts_ms, book_usable_mask(first)),
                             (second.ts_ms, book_usable_mask(second)), role="BBO usable")[1])
    return HistoricalBBOData(*values, source="f05_native_cross_day_prefix_preserved", usable=usable,
                             **_stitch_observation_fields(first, second))
...
def _stitch_observation_fields(first, second):
    fields = {}
    for name, default, dtype in (("observation_ts_us", -1, np.int64), ("source_observed", False, bool)):
        arrays = [getattr(book, name, None) for book in (first, second)]
        if all(array is None for array in arrays):
            fields[name] = None
        else:
            arrays = [np.full(len(book.ts_ms), default, dtype=dtype) if array is None else array
                      for book, array in zip((first, second), arrays, strict=True)]
            fields[name] = _append_arrays((first.ts_ms, arrays[0]), (second.ts_ms, arrays[1]), role=name)[1]
    return fields
