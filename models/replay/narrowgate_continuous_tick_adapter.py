...
from models.tick_data_types import HistoricalBBOData, HistoricalL2Data, book_observation_fields, book_usable_mask
...
def _slice_timed_payload(payload: Any, start_ms: int, end_ms: int) -> Any:
    ...
    if isinstance(payload, HistoricalBBOData):
        ...
        return HistoricalBBOData(
            ...
            **book_observation_fields(payload, slice(left, right)),
        )
...
def _concat_timed_payloads(payloads: Sequence[Any]) -> Any:
    ...
    if not present:
        return None
    def selection(ts):
        # Daily slices are normally already strictly ordered and disjoint.
        # Avoid sorting and copying all four L2 matrices twice in that case.
        if len(ts) < 2 or np.all(ts[1:] > ts[:-1]):
            return slice(None), slice(None)
        order = np.argsort(ts, kind="stable")
        ordered = ts[order]
        return order, np.r_[ordered[1:] != ordered[:-1], True]
    def observation_fields(order, keep):
        return {name: np.concatenate([
            (getattr(payload, name) if getattr(payload, name, None) is not None else
             np.full(len(payload.ts_ms), -1 if name == "observation_ts_us" else False,
                     dtype=np.int64 if name == "observation_ts_us" else bool))
            for payload in present
        ])[order][keep] for name in ("observation_ts_us", "source_observed")}
    if isinstance(present[0], HistoricalBBOData):
        ...
        return HistoricalBBOData(
            ...
            usable=(None if all(payload.usable is None for payload in present) else
                    np.concatenate([book_usable_mask(payload) for payload in present])[order][keep]),
            **observation_fields(order, keep),
        )
...
def _concat_ml_payloads(payloads: Sequence[tuple[Any, ...]]) -> tuple[Any, ...]:
    ...
    if len(ready) < 2 or np.all(ready[1:] > ready[:-1]):
        return tuple(out)
