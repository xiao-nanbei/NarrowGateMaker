...
def _mean_bool(series: pd.Series) -> float:
    return float(_nullable_bool(series).mean())


def _nullable_bool(series: pd.Series) -> pd.Series:
    """CSV/object booleans must not turn missing or 'False' into True."""
...
def _diagnostic_counts(frame: pd.DataFrame) -> dict[str, int]:
    counts = {}
    for horizon in ("1s", "5s", "30s"):
        key = f"markout_{horizon}"
        values = pd.to_numeric(frame.get(key, pd.Series(index=frame.index, dtype=float)), errors="coerce")
        status = frame.get(f"{key}_status", pd.Series("legacy_unclassified", index=frame.index))
        counts[f"{key}_valid_count"] = int(values.notna().sum())
        counts[f"{key}_censored_count"] = int(status.eq("censored").sum())
        counts[f"{key}_missing_count"] = int((values.isna() & ~status.eq("censored")).sum())
        counts[f"{key}_delayed_count"] = int(status.eq("delayed").sum())
    return counts
...
def _bias_toxicity(fills: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        ...
        .agg(
            ...
            toxic_30s_rate=("toxic_30s", _mean_bool),
        )
        .reset_index()
