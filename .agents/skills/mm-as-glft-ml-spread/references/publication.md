...
## Historical alpha consolidation and current permissions

The 2026-09-13 consolidation under `version-alpha-20260913` was a historical operation. Current main has subsequent commits; ordinary work does not retain or rewrite a single mutable root. Preserve unrelated dirty bytes and save task-owned changes as ordinary local commits. Push, tag, merge or rewrite history only under explicit current-task authorization; no alpha label grants that authority.
