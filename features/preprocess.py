...
def _write_bar_metadata(
    ...
    *,
    ...
    metadata = {
        ...
        "trade_count_unit": "individual_execution" if data_type == "trades" else "native_aggregate_packet",
        "rows": int(rows),
...
def _select_daily_inputs(paths: list[Path], *, prefer_individual: bool) -> list[Path]:
    ...
    for path in sorted(paths):
        day = path.parent.name if path.suffix == ".parquet" else path.stem[-10:]
        individual = path.stem == "trades" or "-trades-" in path.name
        rank = (individual if prefer_individual else False, path.suffix == ".parquet")
        current = selected.get(day)
        if current is None:
            selected[day] = path
            continue
        current_individual = current.stem == "trades" or "-trades-" in current.name
...
def main():
    ...
    def accept_result(csv_path, result):
        ...
        if (args.cleanup_input and csv_path.suffix != ".parquet"
    ...
    if workers == 1:
        for csv_path in csv_files:
            data_type = "trades" if "-trades-" in csv_path.name or csv_path.stem == "trades" else "aggTrades"
            accept_result(
                csv_path,
                process_file(
                    csv_path,
