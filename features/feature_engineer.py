...
from data.unify_daily_fields import canonical_feature_frame  # noqa: E402
...
VERBOSE = False
# Inference-only preparation retains its complete source calendar. Historical
# label/training entrypoints still use the unchanged quality exclusion policy.
EXPLICIT_FEATURE_CALENDAR_DAYS: tuple[str, ...] = ()
...
def _compute_label_triplet(
    ...
    horizon_ns: int,
    ret_outcome_end_ns: Optional[np.ndarray] = None,
    vol_outcome_end_ns: Optional[np.ndarray] = None,
    ...
    for i in range(n):
        ...
        if window_len >= 2:
            ...
            if vol_outcome_end_ns is not None and np.isfinite(vol_label[i]):
                vol_outcome_end_ns[i] = ts_1s_ns[fill_stop - 1] + _TS_NS
        ...
        if not np.isfinite(ret_val):
            # An unknown markout is not a negative-direction observation.
            continue
        ret_label[i] = ret_val
        dir_label[i] = 1.0 if ret_val > 0.0 else 0.0
        if ret_outcome_end_ns is not None and np.isfinite(ret_val):
            # The markout uses the close of this left-labelled 1s bar, not its
            # opening timestamp. Both ret and dir depend on that same close.
            ret_outcome_end_ns[i] = ts_1s_ns[markout_idx] + _TS_NS
...
def _compute_toxicity_pair(
    ...
    ask_quote: np.ndarray,
    horizon_ns: int,
    bid_outcome_end_ns: Optional[np.ndarray] = None,
    ask_outcome_end_ns: Optional[np.ndarray] = None,
    ...
    for i in range(n):
        ...
        if bid_fill_idx != -1:
            ...
            if markout_idx < len(close_1s) and np.isfinite(close_1s[markout_idx]):
                tox_bid[i] = 1.0 if close_1s[markout_idx] < bid_quote[i] else 0.0
                if bid_outcome_end_ns is not None and np.isfinite(close_1s[markout_idx]):
                    bid_outcome_end_ns[i] = ts_1s_ns[markout_idx] + _TS_NS
        ...
        if ask_fill_idx != -1:
            ...
            if markout_idx < len(close_1s) and np.isfinite(close_1s[markout_idx]):
                ...
                if ask_outcome_end_ns is not None and np.isfinite(close_1s[markout_idx]):
                    ask_outcome_end_ns[i] = ts_1s_ns[markout_idx] + _TS_NS
...
def _prepare_1s_label_context(bars_1s: pd.DataFrame) -> tuple[np.ndarray, ...]:
    ...
    if not len(closes):
        raise ValueError("label context requires nonempty outcome Bars")
    diffs[0] = np.nan
...
def load_bars(date_filter: str = None, symbol: str = DEFAULT_SYMBOL,
    ...
    files = filter_paths_for_orderbook_quality(
        files, symbol, label="1s bar", explicitly_allowed_days=EXPLICIT_FEATURE_CALENDAR_DAYS,
    )
    ...
    bars = filter_frame_for_orderbook_quality(
        bars, symbol, label="1s bar", explicitly_allowed_days=EXPLICIT_FEATURE_CALENDAR_DAYS,
    )
...
def _load_taker_tempo_features(symbol: str, tag: str) -> Optional[pd.DataFrame]:
    paths = filter_paths_for_orderbook_quality(
        ...
        explicitly_allowed_days=EXPLICIT_FEATURE_CALENDAR_DAYS,
    )
    if not paths:
        return None
    ...
    for stem in ("bookTicker", "bbo"):
        ...
        if path.exists():
            ...
            if frame is not None and not frame.empty:
                return filter_frame_for_orderbook_quality(
                    frame, symbol, label="BBO",
                    explicitly_allowed_days=EXPLICIT_FEATURE_CALENDAR_DAYS,
                )
    return None
...
def _load_market_bars_for_tag(symbol: str, market_type: str,
                              day_tag: Optional[str]) -> Optional[pd.DataFrame]:
    bars_dir = market_bars_dir(ROOT, market_type)
    if market_type == PERP_MARKET and os.environ.get("MM_REFERENCE_BARS_DIR"):
        bars_dir = Path(os.environ["MM_REFERENCE_BARS_DIR"]).expanduser().resolve()
    if not day_tag:
        return None
...
def load_metrics(data_tag: str, symbol: str, *, start_day: Optional[str] = None) -> pd.DataFrame:
    """Read target plus available causal metric context; never future dates."""
    target = METRICS_DIR / f"{symbol}-metrics-{data_tag}.parquet"
    if not target.exists():
        path = target
        print(f"  [WARN] metrics 文件不存在: {path.name}")
        return None
    paths = [target]
    if start_day and start_day < data_tag:
        for day in pd.date_range(start_day, data_tag, inclusive="left"):
            path = METRICS_DIR / f"{symbol}-metrics-{day:%Y-%m-%d}.parquet"
            if path.is_file():
                paths.append(path)
    df = pd.concat([pd.read_parquet(path) for path in sorted(paths)]).sort_index()
...
def _l2_paths_for_tag(symbol: str, tag: str) -> list[Path]:
    ...
    if path.exists():
        return filter_paths_for_orderbook_quality(
            [path], symbol, label="L2", explicitly_allowed_days=EXPLICIT_FEATURE_CALENDAR_DAYS,
        )
    if len(tag) == 7:
        return filter_paths_for_orderbook_quality(
...
def _l2_observation_clock(path: Path, timestamps: np.ndarray, symbol: str):
    from models.backtest_tick import _load_book_observations
    return _load_book_observations(path, timestamps, "l2", symbol=symbol)


def _previous_l2_state(tag: str, symbol: str) -> Optional[dict[str, float]]:
    """Read only the latest valid D-1 snapshot; never borrow a future/day-gap seed."""
    start = pd.Timestamp(tag, tz="UTC")
    previous = (start - pd.Timedelta(days=1)).strftime("%Y-%m-%d")
    columns = ["timestamp", "bid_px_1", "ask_px_1", *[
        f"{side}_qty_{level}" for side in ("bid", "ask") for level in range(1, 11)
    ]]
    for path in reversed(_l2_paths_for_tag(symbol, previous)):
        parquet = pq.ParquetFile(path)
        for group in range(parquet.num_row_groups - 1, -1, -1):
            frame = parquet.read_row_group(group, columns=columns).to_pandas()
            if frame.empty:
                continue
            timestamps = _frame_ts_ms(frame)
            bid = frame["bid_px_1"].to_numpy(dtype=np.float64)
            ask = frame["ask_px_1"].to_numpy(dtype=np.float64)
            depth = frame[columns[3:]].to_numpy(dtype=np.float64).sum(axis=1)
            valid = (
                (timestamps >= int((start - pd.Timedelta(days=1)).timestamp() * 1000))
                & (timestamps < int(start.timestamp() * 1000))
                & np.isfinite(bid) & np.isfinite(ask) & np.isfinite(depth)
                & (bid > 0) & (ask > bid) & (depth > 0)
            )
            if valid.any():
                index = np.flatnonzero(valid)[-1]
                observations, _ = _l2_observation_clock(path, timestamps[index:index + 1], symbol)
                return {"timestamp_ms": int(timestamps[index]), "best_bid": float(bid[index]),
                        "best_ask": float(ask[index]), "total_depth": float(depth[index]),
                        "last_observation_timestamp_us": int(observations[0])}
    return None


def _load_l2_summary_1s(path: Path, *, previous_state=None,
                        unknown_initial_flow: bool = False) -> Optional[pd.DataFrame]:
    try:
        parquet_file = pq.ParquetFile(path)
        full_axis = _frame_ts_ms(parquet_file.read(columns=["timestamp"]).to_pandas())
        source_observation, source_update = _l2_observation_clock(path, full_axis, path.name.split("-l2-")[0])
        clock_bound = bool(len(source_observation) and np.all(source_observation > 0))
        if clock_bound and previous_state is not None and previous_state.get("last_observation_timestamp_us", -1) <= 0:
            previous_state = None  # A legacy tail does not establish source-clock continuation.
        if (clock_bound and previous_state is not None
                and previous_state.get("last_observation_timestamp_us", -1) > source_observation[0]):
            raise ValueError("execution L2 source observation clock regresses across UTC days")
        if (clock_bound and previous_state is not None
                and source_observation[0] == previous_state.get("last_observation_timestamp_us")):
            source_update[0] = False
        offset = 0
        state_frames = []
        prev_best_bid = None if previous_state is None else previous_state["best_bid"]
        prev_best_ask = None if previous_state is None else previous_state["best_ask"]
        prev_total_depth = None if previous_state is None else previous_state["total_depth"]
        unknown_initial_bucket = None
        ...
        for batch in parquet_file.iter_batches(batch_size=250_000):
            ...
            ts_ms = _frame_ts_ms(frame)
            observed_us = source_observation[offset:offset + len(frame)]
            observed_update = source_update[offset:offset + len(frame)] if clock_bound else np.ones(len(frame), dtype=bool)
            offset += len(frame)
            ...
            ts_ms = ts_ms[valid]
            observed_us = observed_us[valid]
            observed_update = observed_update[valid]
            if previous_state is not None and ts_ms[0] <= previous_state["timestamp_ms"]:
                raise ValueError("execution L2 continuation must precede the first current observation")
            bid_px = bid_px[valid]
            ...
            out = pd.DataFrame({
                ...
                "_last_observation_timestamp_us": observed_us,
                ...
                "_quote_flip_sum": ((best_bid != prev_bid) | (best_ask != prev_ask)).astype(np.float64) * observed_update,
                "_refresh_sum": _safe_divide(np.maximum(delta_depth, 0.0), prev_depth) * observed_update,
                "_cancel_sum": _safe_divide(np.maximum(-delta_depth, 0.0), prev_depth) * observed_update,
                "_snapshot_count": observed_update.astype(np.float64),
            })
            if prev_best_bid is None:
                if unknown_initial_flow:
                    unknown_initial_bucket = int(out.iloc[0]["bucket_ts"])
            ...
            reduced = out.groupby("bucket_ts", sort=False).agg({
                **{col: "last" for col in EXECUTION_L2_STATE_COLS},
                "_last_observation_timestamp_us": "last",
                "_quote_flip_sum": "sum",
                "_refresh_sum": "sum",
                "_cancel_sum": "sum",
                "_snapshot_count": "sum",
        ...
        reduced.loc[reduced["_snapshot_count"] == 0, EXECUTION_L2_FLOW_COLS] = np.nan
        if unknown_initial_bucket is not None:
            reduced.loc[unknown_initial_bucket, EXECUTION_L2_FLOW_COLS] = np.nan
        reduced.index = pd.to_datetime(reduced.index.astype(np.int64), unit="ms", utc=True)
        reduced.index.name = None
        result = reduced[[*EXECUTION_L2_FEATURE_COLS, "_last_observation_timestamp_us"]]
        result.attrs["last_l2_state"] = {
            "timestamp_ms": int(ts_ms[-1]), "best_bid": prev_best_bid,
            "best_ask": prev_best_ask, "total_depth": prev_total_depth,
            "last_observation_timestamp_us": int(observed_us[-1]),
        }
        result.attrs["observation_validation"] = {
            "path": str(path), "rows": len(full_axis), "clock_bound": clock_bound,
            "source_observed_rows": int(source_update.sum()) if clock_bound else None,
            "carried_rows": int((~source_update).sum()) if clock_bound else None,
            "max_observation_age_us": int(np.max(full_axis * 1000 - source_observation)) if clock_bound else None,
            "future_fill_violations": int(np.count_nonzero(source_observation > full_axis * 1000)) if clock_bound else None,
        }
        return result
    except Exception as exc:
...
def load_l2_summary_1s(tag: str, symbol: str) -> Optional[pd.DataFrame]:
    ...
    frames = []
    previous_state = _previous_l2_state(tag, symbol)
    for path in paths:
        frame = _load_l2_summary_1s(path, previous_state=previous_state, unknown_initial_flow=True)
        if frame is not None and not frame.empty:
            frames.append(frame)
            previous_state = frame.attrs["last_l2_state"]
    ...
    combined = filter_frame_for_orderbook_quality(
        combined, symbol, label="L2 summary",
        explicitly_allowed_days=EXPLICIT_FEATURE_CALENDAR_DAYS,
    )
    combined.attrs["observation_validation"] = [frame.attrs["observation_validation"] for frame in frames]
...
def add_execution_l2_features(df_10s: pd.DataFrame, dense_index_1s: pd.Index,
    ...
    # Absent snapshots and the first observation without a prior state are
    # unknown flow, not proof that no quote/depth change occurred.

    l2_10s = pd.concat([
        aligned[EXECUTION_L2_STATE_COLS].resample("10s").last(),
        aligned[EXECUTION_L2_FLOW_COLS].resample("10s").mean(),
    ], axis=1)
    unknown_flow = l2_1s[EXECUTION_L2_FLOW_COLS].isna().any(axis=1).resample("10s").max()
    unknown_buckets = unknown_flow.fillna(False).astype(bool)
    l2_10s.loc[unknown_buckets[unknown_buckets].index.intersection(l2_10s.index), EXECUTION_L2_FLOW_COLS] = np.nan
    if "_last_observation_timestamp_us" in l2_1s:
        observed = l2_1s["_last_observation_timestamp_us"].reindex(dense_index_1s, method="ffill")
        observed = observed.resample("10s").last().reindex(l2_10s.index)
        # The original feature axis is left-labelled; its information cutoff is
        # the right edge of the complete 10s bucket, not a new source timestamp.
        cutoff_us = l2_10s.index.as_unit("us").asi8 + 10_000_000
        age_us = cutoff_us - observed.to_numpy(dtype=np.float64)
        unknown = observed.isna().to_numpy() | (observed.to_numpy() <= 0)
        stale = (age_us > max_age_s * 1_000_000) if max_age_s > 0 else np.zeros(len(age_us), dtype=bool)
        invalid = unknown | (age_us < 0) | stale
        l2_10s.loc[invalid, EXECUTION_L2_STATE_COLS] = np.nan

    df_10s = df_10s.join(l2_10s.reindex(df_10s.index), how="left")
    df_10s.attrs["execution_l2_observation_validation"] = l2_1s.attrs.get("observation_validation", [])
    return df_10s
...
def add_labels(df: pd.DataFrame, bars_1s: pd.DataFrame,
               ...
               config_path: Optional[Path] = None, *,
               include_outcome_times: bool = False,
               decision_times: Optional[pd.DatetimeIndex] = None,
               decision_variance: Optional[np.ndarray] = None) -> pd.DataFrame:
    """Add existing labels, optionally recording their actual UTC dependency end.

    Outcome helpers are metadata, never predictors. They preserve the original
    horizon masks and make train/held-out day purging independent of head names.
    Public FeatureFrames supply their actual decision times, without the legacy
    left-labelled feature-period offset. Their already-visible variance can be
    supplied explicitly; otherwise only fully closed historical Bars are used.
    """
    ...
    if decision_times is None:
        ...
        if (len(decisions) != len(df) or decisions.tz is None
                or decisions.hasnans or not decisions.is_monotonic_increasing
                or decisions.has_duplicates):
            raise ValueError("decision times must be unique ordered timezone-aware rows")
        quote_time_ns = decisions.tz_convert("UTC").as_unit("ns").asi8
    start_idx = np.searchsorted(ts_1s_ns, quote_time_ns, side="left")
    last_hist_idx = np.searchsorted(ts_1s_ns + _TS_NS, quote_time_ns, side="right") - 1
    sigma_sq_now = np.full(len(df), np.nan)
    has_history = last_hist_idx >= 0
    sigma_sq_now[has_history] = sigma_sq_1s[last_hist_idx[has_history]]
    if decision_variance is not None:
        if decision_times is None:
            raise ValueError("visible decision variance requires explicit decision times")
        sigma_sq_now = np.asarray(decision_variance, dtype=np.float64)
        if sigma_sq_now.shape != (len(df),) or np.any(np.isinf(sigma_sq_now)) or np.any(sigma_sq_now < 0):
            raise ValueError("invalid visible decision variance")
    ...
    for h in LABEL_HORIZONS:
        ret_end = np.full(len(df), np.iinfo(np.int64).min, dtype=np.int64) if include_outcome_times else None
        vol_end = np.full(len(df), np.iinfo(np.int64).min, dtype=np.int64) if include_outcome_times else None
    ...
    for h in TOXICITY_HORIZONS:
        bid_end = np.full(len(df), np.iinfo(np.int64).min, dtype=np.int64) if include_outcome_times else None
        ask_end = np.full(len(df), np.iinfo(np.int64).min, dtype=np.int64) if include_outcome_times else None
...
def iter_public_feature_frames(input_stream):
    """Current shared input contract, not a re-interpretation of legacy Bars."""
    for tick in input_stream:
        if tick.feature_frame is not None:
            yield tick.feature_frame
...
def process_day(bars_1s: pd.DataFrame, day_tag: str, symbol: str,
    ...
    # --- metrics features ---
    metrics = load_metrics(data_lookup_tag, symbol, start_day=bars_1s.index.min().strftime("%Y-%m-%d"))
...
def _quality_boolean_series(values: pd.Series) -> pd.Series:
    """Parse explicit quality truth values; missing/unknown never mean pass."""
    def parse(value):
        if isinstance(value, (bool, np.bool_)):
            return bool(value)
        if isinstance(value, (int, float, np.integer, np.floating)):
            return bool(value) if value in (0, 1) else pd.NA
        if isinstance(value, str):
            return {"true": True, "false": False, "1": True, "0": False,
                    "1.0": True, "0.0": False}.get(value.strip().lower(), pd.NA)
        return pd.NA

    return values.map(parse).astype("boolean")


def _reference_bar_manifest(days, reference_symbol, proof_path=None):
    """Bind reference count units to actual daily Bar bytes, not their names.

    Canonical Bars can keep builder metadata in their existing calendar receipt
    instead of per-file sidecars. An absent proof stays UNKNOWN, never packet=1.
    """
    root = Path(os.environ.get("MM_REFERENCE_BARS_DIR", market_bars_dir(ROOT, PERP_MARKET))).expanduser().resolve()
    proof = None
    binding = None
    if proof_path:
        proof_path = Path(proof_path)
        proof = json.loads(proof_path.read_text(encoding="utf-8"))
        if (proof.get("schema") != "individual_reference_bars_calendar.v1"
                or proof.get("status") != "COMPLETED"):
            raise ValueError("reference Bar proof must be a completed individual calendar receipt")
        binding = {"path": str(proof_path), "sha256": _sha256_file(proof_path)}
    rows, missing = [], []
    for day in sorted(set(days)):
        path = root / f"{reference_symbol}-1s-{day}.parquet"
        if not path.is_file():
            missing.append(day)
            continue
        ...
        if proof is not None:
            record = proof.get("records", {}).get(day, {})
            declared = record.get("builder_metadata", {})
            if (record.get("day") != day or record.get("symbol") != reference_symbol
                    or record.get("status") != "VERIFIED" or record.get("readback_equal") is not True
                    or record.get("counts_and_last_event_verified") is not True
                    or record.get("output_sha256") != digest
                    or record.get("trade_count_sum") != declared.get("source_rows")):
                raise ValueError(f"reference Bar proof lacks matching count/output evidence: {day}")
            if metadata and any(metadata.get(key) != declared.get(key) for key in (
                    "output_sha256", "source_sha256", "trade_count_unit", "symbol", "utc_day")):
                raise ValueError(f"reference Bar sidecar/calendar conflict: {day}")
            metadata = declared
        unit = metadata.get("trade_count_unit") or "UNKNOWN"
        if metadata.get("output_sha256") and metadata["output_sha256"] != digest:
            raise ValueError(f"reference Bar metadata/output identity mismatch: {day}")
        if (metadata and unit != "UNKNOWN" or proof is not None) and (
                         metadata.get("output_sha256") != digest or metadata.get("utc_day") != day
                         or metadata.get("symbol") != reference_symbol or metadata.get("complete") is not True
                         or metadata.get("bar_interval") != "[t,t+1s)"
                         or metadata.get("causal_visible_at") != "t+1s"
                         or unit not in {"individual_execution", "native_aggregate_packet"}
                         or (proof is not None and unit != "individual_execution")
                         or (unit == "individual_execution" and metadata.get("source_data_type") != "trades")):
            raise ValueError(f"reference Bar metadata has incompatible units/clock/identity: {day}")
        rows.append({"day": day, "symbol": reference_symbol, "path": str(path), "sha256": digest,
                     "trade_count_unit": unit, "source_sha256": metadata.get("source_sha256"),
                     "source_rows": metadata.get("source_rows"),
                     "causal_visible_at": metadata.get("causal_visible_at", "UNKNOWN"),
                     "metadata_sha256": _sha256_file(sidecar) if sidecar.is_file() else None})
    return {"root": str(root), "symbol": reference_symbol, "daily_files": rows,
            "missing_days": missing, "calendar_proof": binding,
            "count_feature": "cv_ref_perp_trade_intensity_60s",
            "clock_scope": "real_individual_trade_clock_not_native_packet_clock_equivalence"}
...
def write_causal_feature_manifest(
    ...
    *,
    ...
    reference_bar_proof: Optional[Path] = None,
) -> Path:
    ...
    bar_paths = {
        path.stem.removeprefix(f"{symbol}-1s-"): path
        for path in BARS_DIR.glob(f"{symbol}-1s-*.parquet")
    }
    bar_inputs: dict[str, dict[str, object]] = {}
    for day, feature_path in sorted(feature_paths):
        ...
        source_days = []
        if day in bar_paths:
            for bar_path in _contiguous_warmup_paths(day, bar_paths, warmup_days=warmup_days):
                source_day = bar_path.stem.removeprefix(f"{symbol}-1s-")
                source_days.append(source_day)
                if source_day not in bar_inputs:
                    meta_path = bar_path.with_suffix(bar_path.suffix + ".meta.json")
                    meta = json.loads(meta_path.read_text(encoding="utf-8")) if meta_path.is_file() else {}
                    bar_sha = _sha256_file(bar_path)
                    if meta.get("output_sha256") and meta["output_sha256"] != bar_sha:
                        raise ValueError(f"bar metadata/output identity mismatch: {bar_path}")
                    bar_inputs[source_day] = {
                        "day": source_day, "path": str(bar_path), "sha256": bar_sha,
                        "metadata_sha256": _sha256_file(meta_path) if meta_path.is_file() else None,
                        "source_sha256": meta.get("source_sha256"),
                        "trade_count_unit": meta.get("trade_count_unit", "UNKNOWN"),
                        "causal_visible_at": meta.get("causal_visible_at", "UNKNOWN"),
                    }
        row["bar_input_days"] = source_days
        row["warmup_days_observed"] = max(0, len(source_days) - 1)
        daily_files.append(row)
        manifest_digest.update(
            f"{day}\0{feature_path.name}\0{stat.st_size}\0{content_sha256}\n".encode()
        )
    reference_days = {day for row in daily_files for day in row["bar_input_days"]}
    reference_days.update(day for day, _ in feature_paths)
    reference_source = _reference_bar_manifest(
        reference_days, reference_symbol,
        reference_bar_proof or os.environ.get("MM_REFERENCE_BAR_PROOF"),
    ) if market_stage not in {"single", "none", "off", "disabled"} and reference_symbol != symbol else None
    derived_datasets = []
    ...
    execution_l2_source: dict[str, object] = {
        ...
        "continuation": "adjacent_utc_day_last_valid_snapshot_only",
        "missing_initial_flow": "unknown_not_zero",
        "missing_state": "nan_after_max_exec_book_age_not_zero",
        "freshness_clock": "bound_last_observation_timestamp_us_at_10s_feature_cutoff",
        "carried_flow": "not_counted_as_source_update; unknown_without_observed_updates",
        "unbound_legacy_clock": "unknown_state_freshness_not_assumed",
    }
    ...
    if normalize_symbol(symbol) == "BTCUSDC":
        ...
        if daily_quality.is_file():
            ...
            quality_flags = (
                "cadence_schema_valid", "coverage_99_valid", "formal_eligible",
                "provider_sensitivity_replay_eligible", "exact_queue_policy_eligible",
            )
            parsed_quality = {
                name: _quality_boolean_series(selected_quality[name] if name in selected_quality
                    else pd.Series(pd.NA, index=selected_quality.index))
            ...
            } for name, values in parsed_quality.items()}
            invalid_days = selected_quality.loc[
                ~parsed_quality["cadence_schema_valid"].fillna(False), "day",
            ].tolist()
            if require_execution_l2 and invalid_days:
                raise RuntimeError(
                    "required normalized L2 cadence/schema failed or unknown for: "
                    + ", ".join(invalid_days[:5])
                )
            execution_l2_source.update(
                {
                    ...
                    "quality_boolean_counts": boolean_counts,
                    **{f"{name}_days": counts["true"] for name, counts in boolean_counts.items()},
                }
            )
    payload = {
        "schema_version": 3,
        ...
        "bar_source": {
            "root": str(BARS_DIR), "daily_files": list(bar_inputs.values()),
            "price_padding": "past_close_only_leading_unknown_no_backward_fill",
            "empty_second_counts": "zero_observed_events_in_retained_source_not_proof_of_capture_completeness",
        },
        "reference_bar_source": reference_source,
        "calendar_policy": (
            "retain_all_selected_source_days_no_economic_admission"
            if not labels_materialized else "historical_quality_filtered"
        ),
        "market_stage": str(market_stage),
        "reference_symbol": str(reference_symbol),
        "reference_bars_dir": str(Path(os.environ.get(
            "MM_REFERENCE_BARS_DIR", market_bars_dir(ROOT, PERP_MARKET),
        )).expanduser().resolve()),
        "split_mode": "chronological_good_day_with_embargo" if labels_materialized else "inference_only",
        "split": split,
        "sample_weight": {
...
def _materialize_feature_day(
    path: Path, *, paths_by_day: dict[str, Path], out_dir: Path, symbol: str,
    options: argparse.Namespace, config_path: Optional[Path], market_stage: str,
    reference_symbol: str, sample_weight_reference_date: str,
    calendar_days: tuple[str, ...],
    source_roots: Optional[dict[str, Path]] = None,
) -> tuple[str, Path, list[str] | None]:
    """Independent daily output with explicitly supplied past-only context."""
    global VERBOSE, EXPLICIT_FEATURE_CALENDAR_DAYS, DATA_DIR, BARS_DIR, METRICS_DIR, TRADE_FEATURE_DIR
    if source_roots is not None:
        DATA_DIR = Path(source_roots["data"])
        BARS_DIR = Path(source_roots["bars"])
        METRICS_DIR = Path(source_roots["metrics"])
        TRADE_FEATURE_DIR = Path(source_roots["tempo"])
    VERBOSE = bool(options.verbose)
    EXPLICIT_FEATURE_CALENDAR_DAYS = calendar_days
    tag = path.stem.replace(f"{symbol}-1s-", "")
    out_path = out_dir / f"features_{tag}.parquet"
    if out_path.exists() and not options.force:
        qprint(f"[SKIP] {out_path.name} 已存在")
        return tag, out_path, None
    warmup_paths = _contiguous_warmup_paths(tag, paths_by_day, warmup_days=options.warmup_days)
    bars_1s = pd.concat([_read_bars_utc(source) for source in warmup_paths]).sort_index()
    bars_1s = filter_frame_for_orderbook_quality(
        bars_1s, symbol, label="1s bar", explicitly_allowed_days=calendar_days,
    )
    if bars_1s.empty:
        raise ValueError(f"{tag}: no source bars remain for requested feature day")
    qprint(f"[...] {tag}: {len(bars_1s):,} source bars; causal warmup={len(warmup_paths) - 1}d")
    chunks = split_on_calendar_day_gaps(bars_1s, max_gap_s=FEATURE_SOURCE_MAX_GAP_S)
    feature_chunks = [process_day(
        chunk, tag, symbol, config_path=config_path, market_stage=market_stage,
        reference_symbol=reference_symbol,
        calendar_tag=tag if len(chunks) == 1 and len(warmup_paths) == 1 else None,
        output_day_tag=tag, sample_weight_reference_date=sample_weight_reference_date,
        sample_weight_lambda=options.lam, require_execution_l2=options.require_execution_l2,
        require_taker_tempo=options.require_taker_tempo, include_labels=not options.features_only,
    ) for chunk in chunks]
    features = pd.concat(feature_chunks).sort_index()
    features = filter_frame_for_orderbook_quality(
        features, symbol, label="feature", explicitly_allowed_days=calendar_days,
    )
    _assert_unique_time_index(features, f"features_{tag}")
    features.dropna(subset=["volatility_60s"], inplace=True)
    features = canonical_feature_frame(features)
    ...
    print(f"[OK] {out_path.name}: {len(features):,} rows; {out_path.stat().st_size / 1e6:.1f} MB", flush=True)
    del bars_1s, features, feature_chunks
    gc.collect()
    return tag, out_path, columns


def main():
    parser = argparse.ArgumentParser(description="特征工程: 1s bars → 10s 特征矩阵")
    parser.add_argument("--data-plan", type=Path, help="Common-input plan; computes shared frames without old labels")
    parser.add_argument("--data-output", type=Path, help="New bound common-input bundle directory")
    ...
    parser.add_argument("--workers", type=int, default=1,
                        help="Independent daily workers with explicit causal warmup (1-4)")
    args = parser.parse_args()
    if args.data_plan is not None or args.data_output is not None:
        if args.data_plan is None or args.data_output is None or args.file or args.days_file:
            parser.error("use --data-plan and --data-output together, without legacy day selectors")
        from data.runtime import derive_inputs
        result = derive_inputs(json.loads(args.data_plan.read_text()), args.data_output)
        print(json.dumps({"rows": {k: v["rows"] for k, v in result["files"].items()}, "labels": "not_run"}))
        return
    ...
    global VERBOSE, EXPLICIT_FEATURE_CALENDAR_DAYS
    VERBOSE = bool(args.verbose)
    EXPLICIT_FEATURE_CALENDAR_DAYS = ()
    ...
    if args.features_only:
        EXPLICIT_FEATURE_CALENDAR_DAYS = tuple(
            f.stem.replace(f"{symbol}-1s-", "") for f in files
    ...
    # Workers retain only their bounded context; one parent publishes the manifest.
    feature_paths = []
    sample_columns = None
    build_day = functools.partial(
        _materialize_feature_day, paths_by_day=paths_by_day, out_dir=out_dir, symbol=symbol,
        options=args, config_path=config_path, market_stage=market_stage,
        reference_symbol=reference_symbol, sample_weight_reference_date=sample_weight_reference_date,
        calendar_days=EXPLICIT_FEATURE_CALENDAR_DAYS,
        source_roots={"data": DATA_DIR, "bars": BARS_DIR, "metrics": METRICS_DIR, "tempo": TRADE_FEATURE_DIR},
    )
    daily_inputs = [f for f in files if _is_day_tag(f.stem.replace(f"{symbol}-1s-", ""))]

    def record(result):
        nonlocal sample_columns
        tag, path, columns = result
        feature_paths.append((tag, path))
        if sample_columns is None and columns is not None:
            sample_columns = columns

    if args.workers == 1:
        for path in daily_inputs:
            record(build_day(path))
    else:
        with concurrent.futures.ProcessPoolExecutor(max_workers=args.workers) as executor:
            pending = [executor.submit(build_day, path) for path in daily_inputs]
        ...
        try:
            ...
            try:
                for path in paths:
                    ...
                    end_ts = df.index[-1]
                    total_rows += len(df)
                    table = pa.Table.from_pandas(canonical_feature_frame(df), preserve_index=True)
                    if writer is None:
            ...
            canonical_feature_frame(combined).to_parquet(out, engine="pyarrow")
