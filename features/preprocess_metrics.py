...
from data_paths import data_root, raw_data_root  # noqa: E402
from data.unify_daily_fields import METRICS_COLUMNS, canonical_feature_frame  # noqa: E402
...
KEEP_COLS = list(METRICS_COLUMNS)
...
def main():
    parser = argparse.ArgumentParser(description="Preprocess daily raw metrics to feature-ready parquet")
    parser.add_argument("--symbol", type=str, default=DEFAULT_SYMBOL,
                        help=f"Symbol (default {DEFAULT_SYMBOL}; MM_SYMBOL also supported)")
    parser.add_argument("--file", type=str, default=None)
    parser.add_argument("--input-dir", type=Path, default=None,
                        help="Explicit legacy CSV input directory; default prefers canonical daily Parquet")
    ...
    csv_files = sorted((args.input_dir or RAW_DIR).glob(f"{symbol}-metrics-*.csv"))
    if args.input_dir is None:
        canonical = sorted((raw_data_root() / "history" / "binance_futures" / symbol).glob("*/metrics.parquet"))
        replaced = {path.parent.name for path in canonical}
        csv_files = [path for path in csv_files if path.stem[-10:] not in replaced] + canonical
    if not csv_files:
        print("No daily raw metrics files found")
        sys.exit(1)

    if args.file:
        csv_files = [f for f in csv_files if args.file in f.name or args.file == f.parent.name]

    ok = skip = rows_written = 0
    invalid: list[tuple[str, str]] = []
    for f in csv_files:
        # filename: BTCUSDT-metrics-2025-01-01.csv
        date_str = f.parent.name if f.suffix == ".parquet" else f.stem.replace(f"{symbol}-metrics-", "")
        if len(date_str) != 10:
        ...
        combined = pd.read_parquet(f) if f.suffix == ".parquet" else pd.read_csv(f, parse_dates=["create_time"])
        ...
        combined = canonical_feature_frame(combined)
