...
独立 UTC 质量日历包含请求起止日期内的每一天，包括未下载的头尾日。可筛选来源、市场、币对、数据集、节点、问题日及明确缺失的副本。文件可用性、审计状态、各任务适用性与节点副本核验分开展示；离线或未核验副本不会被默认为缺失或可用。展开记录只展示已有记录数、字节数、覆盖、缺口区间、时间、原因及证据范围。导出生成有界的 JSON 补数 / 复核清单，不执行下载、同步、审计或回放。由日期跳转到复盘，不会把当前质量目录绑定为旧结果的原始输入证明。
