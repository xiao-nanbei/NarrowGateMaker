...
登记任务可在已有 `summary_files` 中包含唯一一份 `studio_research_result.v1` JSON。完成后任务详情直接展示，无须 B0 手工导入或重新回放；旧摘要仍可查看。真实比较与合成示例分开，使用 `/api/research-comparison?left=<job-id>&right=<job-id>`。

摘要包含逻辑 `result_id`、`arm`、`model_id`、`currency`、布尔值 `economic_complete`，以及允许空值的 `metrics`：`pnl_before_funding`、有符号 `funding_cashflow`、`all_in_net_pnl`、正成本 `fee_cost`、`terminal_inventory`、`terminal_unrealized_pnl`、`fill_count`、`order_count`。缺失保持 null。完整记账要求资金费、手续费和终点记账齐全；净收益等于资金费前收益加资金费。资金费前收益已经包含手续费，不再次扣除。不完整结果的净收益必须为 null，但仍可查看。

比较要求以下逻辑身份非空且一致：`experiment_id`、`input_manifest_id`、`evaluation_contract_id`、`slice_manifest_id`、`split_manifest_id`、`latency_profile_id`、`accounting_contract_id`、`initial_state_id`、`runtime_contract_id` 和币种。它们引用已有冻结合同，不新增清单副本或审批层；评估合同绑定指标定义与评估日期。模型与组别可以不同。未知指标不计算差值，口径不一致则解释原因而不排名。声明校验不等于独立经济验证。

本次第一步尚未适配任意历史报告、自动登记当前训练、提供实时进度，或绑定完整订单／行情事件用于诊断复盘。缺少产物时不伪造复盘链接。既有 B0 上下文复盘保持原边界。新产出端需提供这份摘要，结果才能进入比较。
...
日历默认显示**训练 / 回测产物**，另有独立的**原始行情**和**待分类记录**页签。文件覆盖表统计登记文件组，与各用途质量结论分开；原始文件组可显示 23/24 小时文件这样的部分覆盖。原始文件在，不代表处理产物已经生成或检查。完整日历按钮使用登记起止日期（每次请求最多 730 天），日期明细每页 20 天。目录展示的是已登记数据集，不是磁盘里的每一个历史目录。连续回测应跨日保留执行状态；沿用前快照是缺口建模方式，不证明缺失的增量为零。
