...
export type QualityDateWindow = { start: string; end: string };

/** Initialize from the registered calendar once; never replace a user selection. */
export function initialQualityWindow(
  current: QualityDateWindow | null,
  calendar: { start_day: string; end_day: string } | null | undefined,
): QualityDateWindow | null {
  if (current || !calendar) return current;
  if (!utcDays(calendar.start_day, calendar.end_day).length) return null;
  return { start: calendar.start_day, end: calendar.end_day };
}
