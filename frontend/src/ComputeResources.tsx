...
function ResourceCard({ resource }: { resource: ComputeResource }) {
    ...
    <section className="panel compute-panel">
      <div className="section-heading">
        ...
        <span className="muted">
          资源状态与 worker 心跳分开记录 · 不自动开机或提交任务
        </span>
      </div>
      {report && (
      ...
      <details className="worker-diagnostics">
        <summary>
          合成演示进程登记 · {workers.length} 个（不是主机数量）
        </summary>
        <p>
          下列心跳只表示 demo worker
        ...
        {!workers.length && <p>当前没有合成 worker 登记。</p>}
      </details>
    </section>
  );
}
