...
test("quality defaults wait for the catalog and use all 401 registered UTC days", () => {
  assert.equal(initialQualityWindow(null, undefined), null);
  assert.equal(initialQualityWindow(null, null), null);
  const current = initialQualityWindow(null, {
    start_day: "2025-08-01",
    end_day: "2026-09-05",
  });
  assert.deepEqual(current, { start: "2025-08-01", end: "2026-09-05" });
  const days = utcDays(current.start, current.end);
  assert.equal(days.length, 401);
  assert.equal(days.at(-1), "2026-09-05");
  assert.equal(days.includes("2026-09-06"), false);
});

test("catalog refresh preserves both initialized and manually selected date windows", () => {
  const refreshed = { start_day: "2025-08-01", end_day: "2026-09-08" };
  for (const current of [
    { start: "2025-08-01", end: "2026-09-05" },
    { start: "2026-04-01", end: "2026-04-30" },
    { start: "2026-04-01", end: "" },
    { start: "", end: "" },
  ]) {
    assert.equal(initialQualityWindow(current, refreshed), current);
    assert.equal(initialQualityWindow(current, null), current);
  }
});

test("invalid catalog dates remain unselected instead of inventing a current-day range", () => {
  for (const calendar of [
    { start_day: "2026-09-06", end_day: "2026-09-05" },
    { start_day: "2026-02-30", end_day: "2026-09-05" },
  ]) {
    assert.equal(initialQualityWindow(null, calendar), null);
  }
});
